...
Conversation entries preserve original Session entry IDs, parents, and leaf identity while omitting system/instruction, memory, and skill bodies. An update retains the visible prefix through `afterEntryId` and replaces its suffix; it can represent a branch change without any Run event. Application clients restore user/tool/assistant rows from this conversation and fetch a fresh snapshot when the previous leaf or prefix is inconsistent. [Snapshot bounds](./server.md#snapshot-limits) reject oversized projections instead of truncating them.
