...
import { applied, at, entry, proposal, scope, update } from "./harness-fixtures"
...
describe("Refinement.applyProposal accepts only an authored proposal", () => {
  ...
  it("refuses a forged proposal whose cast erased the brand at runtime", () => {
    const forged = Schema.decodeUnknownSync(HarnessEntry.RefinementProposal)(
      forgedJson,
    ) as unknown as HarnessEntry.AuthoredRefinementProposal
