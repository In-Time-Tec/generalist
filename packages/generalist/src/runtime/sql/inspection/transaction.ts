...
export const withConsistentSnapshot: {
  ...
  SqlClient.makeWithTransaction({
    ...
    savepoint: (connection, id) =>
      connection.executeUnprepared(`SAVEPOINT generalist_inspection_${id}`, [], undefined).pipe(Effect.asVoid),
    ...
    rollbackSavepoint: (connection, id) =>
      connection
        .executeUnprepared(`ROLLBACK TO SAVEPOINT generalist_inspection_${id}`, [], undefined)
        .pipe(Effect.asVoid),
  })(effect),
)
