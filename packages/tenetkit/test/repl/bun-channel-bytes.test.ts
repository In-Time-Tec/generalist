...
import { emptyAccumulator, ingest, terminal } from "../../src/repl/repl/bun-runtime.js"
...
it.effect("marks truncated stderr on a stopped cell once", () =>
  Effect.sync(() => {
    ...
    expect(outcome?.failure?._tag).toBe("tenetkit/repl/CellExecutionFailed")
    if (outcome?.failure?._tag !== "tenetkit/repl/CellExecutionFailed") return
