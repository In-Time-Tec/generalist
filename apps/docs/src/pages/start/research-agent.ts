...
export const researchAgent = definePage({
  ...
  content: [
    ...
    p(
      "The run is waiting rather than failed. Each open wait has an opaque ",
      code("RunWaiting.wait.waitId"),
      "; a turn can produce several waits, so never construct or predict these IDs. ",
      ...
      ". This script reads the current wait from the event stream and sends that exact ID back through the route. Run it again for each later ",
      code("RunWaiting"),
      " event:",
    ),
