...
export const introduction = definePage({
  ...
  title: "What is TenetKit",
  navTitle: "What is TenetKit",
  group: "Start",
  description:
    "TenetKit is a TypeScript framework for building AI agents on Effect: agents are plain values, runs are typed event streams, and every seam is an Effect service with a deterministic test layer.",
  content: [
    p(
      "TenetKit is a TypeScript framework for building AI agents on Effect, built on ",
      code("effect/unstable/ai"),
      ", and it covers what an agent needs beyond a model call: turn iteration, permissions, human approval as typed suspension, steering, and deterministic test seams. An agent is a plain value: a name, instructions, a toolkit, and a turn policy. Run it directly through tenetkit for process-local work, or use tenetkit/runtime for addressable runs, durable waits, canonical event replay, and database-backed recovery.",
    ),
    p(
    ...
    h2("when-to-use-tenetkit", "When to use TenetKit"),
    bullets(
      "Process-local agents: CLIs, scripts, servers, and tests can run tenetkit without external infrastructure.",
      "Human-in-the-loop flows: approval-gated tools suspend the run as a typed error carrying a resume token.",
      "Live chat: tenetkit/transport projects semantic Runtime-owned RunEvents over SSE and WebSocket, with a headless FoldKit chat model for the browser.",
      "Deterministic CI: scripted models and test layers on every seam make agent behavior assertable.",
    ),
    h2("non-goals", "Non-goals"),
    p(
      "TenetKit is not a general-purpose workflow engine, project scaffold, or hosted platform. tenetkit/runtime owns agent-run durability and storage adapters; wider application orchestration and deployment remain yours.",
    ),
    h2("where-runtime-fits", "Where tenetkit/runtime fits"),
    p(
      "tenetkit/runtime composes the core agent loop into finite, addressable runs. It owns idempotent admission, persisted RunEvents, waits and signals, cancellation, inspection, and recovery. Use its memory layer for local development, SQLite for durable single-process execution, or PostgreSQL/MySQL for durable multi-worker execution. ",
