...
The documentation site is a code-first Foldkit + StyleX app built from this directory. Its pages are authored TypeScript, not a Markdown tree: [`src/content.ts`](src/content.ts) is the content model, and its displayed snippets live beside it under `src/snippets/`.
...
`bun run dev` starts the Vite dev server. Edit `src/content.ts` to change sections; code blocks import `?raw` documentation snippets.
...
This runs the Vite client and SSR builds.
...
- [`openapi.json`](openapi.json): the checked-in transport schema.
