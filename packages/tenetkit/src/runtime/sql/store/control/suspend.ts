...
const persistWaits = (runId: string, waits: SuspendInput["waits"], openedAt: string) =>
  Effect.gen(function* () {
    ...
    for (const [authoredOrder, requested] of waits.entries()) {
      ...
      if (prior === undefined) {
        yield* sql`
          INSERT INTO tenetkit_run_waits (run_id, wait_id, authored_order, reason, status, response_json, opened_at, closed_at)
...
const appendWaits = (hub: EventHub, loaded: DecodedRun, inserted: SuspendInput["waits"], openedAt: string) =>
  Effect.gen(function* () {
    ...
    if (inserted.length === 0) {
      yield* sql`UPDATE tenetkit_runs SET status = 'waiting', updated_at = ${openedAt} WHERE run_id = ${loaded.runId}`
    }
  })
