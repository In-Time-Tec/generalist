...
The default turn policy is `Policy.forever`: Generalist imposes no follow-up-turn count, and a turn with no pending tool results still completes the run naturally. A finite cap is an explicit author choice via `Policy.recurs(n)`; cancellation, budgets, and tool governance remain independent controls.
