...
import { make as makeJournal, type Journal, type State } from "../../src/durability/internal/journal.js"
...
type ActionKind = "decide" | "send" | "publish" | "publish-lost" | "timeout" | "cancel" | "reconcile" | "retry" | "ack" | "request-delete"
type Action = { readonly kind: ActionKind; readonly actor: number }
type Mutation = "none" | "overwrite" | "gap" | "stale-rebase" | "ignore-fence" | "identity-only" | "bad-digest" | "premature-ack" | "delete"
...
const initial = (): Model => ({
  writers: Array.from({ length: 2 }, (): Writer => ({ at: 0, attempts: 0, phase: "read", draft: null, evidence: null, checkedMissing: false })),
  requests: [], slots: [], accepted: [], acknowledgements: [], rejected: [], deletionRequested: false,
})
  ...
  model.writers.forEach((writer, actor) => {
    ...
    switch (writer.phase) {
      ...
      case "retry":
        ...
        if (writer.attempts < bounds.attempts || model.slots.some((slot) => slot?.command.id === command.id)) actions.push({ kind: "decide", actor })
        break
      case "prepared": actions.push({ kind: "send", actor }); break
      case "waiting": actions.push({ kind: "timeout", actor }, { kind: "cancel", actor }); break
      case "unknown":
        if (!writer.checkedMissing || model.slots[writer.draft!.slot] != null) actions.push({ kind: "reconcile", actor })
        if (writer.checkedMissing && writer.attempts < bounds.attempts) actions.push({ kind: "retry", actor })
        break
      case "confirmed": actions.push({ kind: "ack", actor }); break
    }
  })
  model.requests.forEach((request, actor) => {
    if (request.pending) actions.push({ kind: "publish", actor }, { kind: "publish-lost", actor })
  })
  if (!model.deletionRequested && model.slots.some((slot) => slot !== null)) actions.push({ kind: "request-delete", actor: 0 })
  return actions
}
...
const step = (previous: Model, program: Program, action: Action, mutation: Mutation = "none"): Model => {
  const model: Model = {
    ...
    slots: previous.slots.slice(), accepted: previous.accepted.slice(),
    acknowledgements: previous.acknowledgements.slice(), rejected: previous.rejected.slice(),
  }
  ...
  switch (action.kind) {
    case "decide": {
      ...
      if (command.generation !== generation && mutation !== "ignore-fence" && !(mutation === "stale-rebase" && writer.draft !== null)) {
    ...
    case "retry": writer.phase = "read"; break
    case "ack":
      model.acknowledgements.push({ command, receipt: (writer.evidence ?? writer.draft)!.receipt, evidence: writer.evidence })
      finish(writer)
      break
  }
  return model
...
const violations = (model: Model): Array<string> => {
  ...
  model.accepted.forEach((record, index) => {
    ...
    if (record.beforeGeneration !== generation || record.beforeTotal !== total) failures.add("fresh-deterministic-decision")
    if (record.command.kind === "takeover") generation += 1
    else total += record.command.amount
    const expected: Receipt = {
      commandId: record.command.id, inputDigest: inputDigest(record.command), slot: index, generation, total,
    }
    if (JSON.stringify(record.receipt) !== JSON.stringify(expected)) failures.add("exact-receipt-identity-digest-and-result")
  })
  if (model.slots.length !== model.accepted.length || model.slots.some((slot) => slot === null)) failures.add("contiguous-prefix")
  for (const acknowledgement of model.acknowledgements) {
    const { command, receipt, evidence } = acknowledgement
    if (evidence === null || !model.accepted.some((record) => record.token === evidence.token)) failures.add("no-ack-on-unresolved-publication")
    const original = model.accepted.find((record) => record.command.id === command.id)
    if (original === undefined || receipt.commandId !== command.id || receipt.inputDigest !== inputDigest(command) || JSON.stringify(receipt) !== JSON.stringify(original.receipt)) {
      failures.add("exact-receipt-identity-digest-and-result")
    }
    if (evidence !== null && (evidence.command.id !== command.id || inputDigest(evidence.command) !== inputDigest(command))) failures.add("exact-receipt-identity-digest-and-result")
  }
  return [...failures]
}
  ...
  for (let index = 0; index < nodes.length; index += 1) {
    ...
    for (const action of nextActions) {
      ...
      if (visited.size >= bounds.states) {
        throw new Error(`Model state guard exhausted; NOT an exhaustive result: ${JSON.stringify({ program, mutation, bounds, transitions, trace: [...traceTo(nodes, index), action] })}`)
      }
      visited.add(key)
...
const replay = (program: Program, trace: ReadonlyArray<Action>, mutation: Mutation = "none") => {
  ...
  for (const action of trace) {
    if (!enabled(state, program).some((candidate) => candidate.kind === action.kind && candidate.actor === action.actor)) {
...
describe("independent bounded numbered-slot protocol model", () => {
  for (const scenario of cases) {
    it(`enumerates every enabled interleaving: ${scenario.name}`, () => {
      ...
      evidence(scenario.name, { assumptions: "atomic-create, strong-read/list, immutable-payloads, retained-slots, no-fairness", bounds, program: scenario.program, ...result })
      if (result.counterexample !== null) throw new Error(JSON.stringify({ program: scenario.program, ...result.counterexample }))
      // Coverage is about modeled transitions, not incidental implementation calls.
      for (const kind of ["publish", "publish-lost", "timeout", "cancel", "reconcile", "retry", "ack", "request-delete"] as const) {
  ...
  it("retains slots against a cancelled delayed writer; deletion permits recreation", () => {
    ...
    const trace: Array<Action> = [
      { kind: "decide", actor: 0 }, { kind: "send", actor: 0 }, { kind: "cancel", actor: 0 },
      { kind: "decide", actor: 1 }, { kind: "send", actor: 1 }, { kind: "publish", actor: 1 },
      { kind: "ack", actor: 1 }, { kind: "request-delete", actor: 0 }, { kind: "publish-lost", actor: 0 },
    ]
    ...
    expect(recreated.accepted.map((record) => [record.slot, record.command.id])).toEqual([[0, "b"], [0, "a"]])
  ...
  it("reconciles a fresh same-ID retry without cancelling the original provider request", () => {
    ...
    const prefix: Array<Action> = [
      { kind: "decide", actor: 0 }, { kind: "send", actor: 0 }, { kind: "timeout", actor: 0 },
      { kind: "reconcile", actor: 0 }, { kind: "retry", actor: 0 },
      { kind: "decide", actor: 0 }, { kind: "send", actor: 0 },
    ]
    ...
    for (const winner of [0, 1]) {
      const trace: Array<Action> = [
        ...
        { kind: "reconcile", actor: 0 }, { kind: "ack", actor: 0 },
      ...
      expect(settled.acknowledgements.map((ack) => ack.receipt)).toEqual([{
        commandId: "a", inputDigest: '["add",0,1]', slot: 0, generation: 0, total: 1,
      }])
  ...
  it("generates only modeled command and transport lifecycles from reproducible seeds", () => {
    const seeds = [1, 7, 19, 42, 99, 257, 1024, 4093, 65537, 0x12345678, 0x5eed, 0x7fffffff, 0x80000001, 0xabcdef01, 0xfffffffe, 0xffffffff]
    const results = seeds.map((seed) => {
      ...
      return { seed, actions: trace.length, accepted: state.accepted.length, pending: state.requests.filter((request) => request.pending).length, enabledAtEnd: enabled(state, program).length }
    })
    evidence("seeded-model-only-lifecycles", { commandsPerWriter: 2, maxActions: 64, maxAttempts: bounds.attempts, results })
  })
})
...
const expectedOutcome = (accepted: ReadonlyArray<Command>) => {
  ...
  return { receipts, state: { generation: last?.generation ?? 0, total: last?.total ?? 0 }, sequence: String(accepted.length - 1) }
}
...
describe("numbered-slot model / real JournalEngine boundary", () => {
  ...
  it.effect("rejects a delayed old-generation decision after takeover before accepting new-generation work", () =>
    Effect.gen(function* () {
      ...
      const pause = yield* bucket.faults.pauseNextCreate(firstSlot)
      const pending = yield* submit(old, add("stale", 100)).pipe(Effect.flip, Effect.forkChild({ startImmediately: true }))
      yield* pause.entered
