  ...
  Effect.gen(function* () {
    ...
    const released = yield* sql.onDialectOrElse({
      mysql: () =>
        Effect.gen(function* () {
          yield* sql`
            UPDATE generalist_runs SET owner_worker_id = NULL, lease_expires_at = NULL, updated_at = ${updated}
      ...
      orElse: () =>
        sql<{ readonly run_id: string }>`
          UPDATE generalist_runs SET owner_worker_id = NULL, updated_at = ${updated}
          WHERE run_id = ${input.runId}
            AND owner_worker_id = ${input.ownerId}
            AND attempt_fence = ${input.attemptFence}
          RETURNING run_id
  ...
  Effect.gen(function* () {
    ...
    if (run.status === "queued") {
      ...
      const links = yield* sql<{ readiness: string }>`
        SELECT readiness FROM generalist_run_links WHERE child_run_id = ${run.runId} LIMIT 1
      `
