...
describe("model instrumentation", () => {
  it.effect("disables the SDK response-id fallback so each Generalist attempt invokes the provider once", () =>
    Effect.gen(function* () {
      ...
      const model = yield* LanguageModel.make({
        generateText: (options) => {
          generateRawCalls += 1
          const parts: Array<Response.PartEncoded> = [Response.makePart("text", { text: "generated" }), finishPart]
          return options.previousResponseId === undefined ? Effect.succeed(parts) : Effect.fail(incrementalRejected)
        },
        streamText: (options) => {
          streamRawCalls += 1
      ...
      const wrapped = yield* instrument(model, { emit, turn: 0 })
  ...
  it.effect("emits provider-native finish metadata without normalizing it", () =>
    Effect.gen(function* () {
      ...
      const wrapped = yield* instrument(languageModel({ streamText: () => Stream.make(providerFinish) }), {
        emit,
        turn: 0,
      })
  ...
  it.effect("fails the call on terminal classification without scheduling retries", () =>
    Effect.gen(function* () {
      const { events, emit } = makeCollector()
      const wrapped = yield* instrument(languageModel({ generateText: () => Effect.fail(terminalError) }), {
        emit,
        turn: 0,
        resilience: makeResilience({
          retrySchedule: Schedule.exponential("100 millis"),
  ...
  it.effect("instrumenting an already instrumented model is idempotent", () =>
    Effect.gen(function* () {
      ...
      const once = yield* instrument(model, { emit, turn: 0 })
      const twice = yield* instrument(once, { emit, turn: 0 })
  ...
  it.effect("a nested run re-instruments the base model so one call never emits into two runs", () =>
    Effect.gen(function* () {
      ...
      const outerInstrumented = yield* instrument(model, { emit: outer.emit, turn: 0 })
      const innerInstrumented = yield* instrument(outerInstrumented, { emit: inner.emit, turn: 0 })
