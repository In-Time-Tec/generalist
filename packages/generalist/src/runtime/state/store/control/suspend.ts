...
type SuspendResult = Effect.Effect<RuntimeState, RunNotFound | RunTerminal | RuntimeUnavailable, PreparedObservation>
...
const suspensionTokens = (suspension: SuspendInput["suspension"]): ReadonlyArray<string> => {
  if (suspension._tag === "generalist/runtime/ToolSuspended") return [suspension.token]
...
const appendWaits = (state: RuntimeState, runId: string, inserted: ReadonlyArray<RunWait>) =>
  Effect.gen(function* () {
    let next = state
...
const reconcileChildren = (state: RuntimeState, runId: string, tokens: ReadonlyArray<string>) =>
  Effect.gen(function* () {
...
const reconcileGroups = (state: RuntimeState, runId: string, suspension: SuspendInput["suspension"]) =>
  Effect.gen(function* () {
    let reconciled = state
    for (const owned of groupWaitsFromSuspension(suspension)) {
      ...
      const transitioned = closeWait(reconciled, {
        ...
        status: "signaled",
        resolution,
        closedAt: yield* preparedOccurredAt,
      })
...
export const suspend: {
  (input: SuspendInput): (state: RuntimeState) => SuspendResult
  (state: RuntimeState, input: SuspendInput): SuspendResult
} = Function.dual(2, (state: RuntimeState, input: SuspendInput) =>
  Effect.gen(function* () {
    ...
    let waiting: RuntimeState = yield* appendWaits(inserted.state, run.runId, inserted.inserted)
