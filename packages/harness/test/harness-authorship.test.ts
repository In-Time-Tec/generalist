...
import { Authorship, HarnessEntry, HarnessState, Refinement } from "../src/index"
import { applied, at, create, entry, proposal, scope, update } from "./harness-fixtures"
...
const authored = (edits: ReadonlyArray<unknown>): unknown => ({ id: "model-1", at: at(2), edits })
...
describe("Authorship.authorProposal", () => {
  ...
  it.effect("refuses a create that pins a revision", () =>
    Effect.gen(function* () {
      ...
      expect(failure._tag).toBe("@batonfx/harness/AuthorshipRejected")
  ...
  it.effect("refuses an excess proposal property", () =>
    Effect.gen(function* () {
      const failure = yield* Authorship.authorProposal({
        ...(authored([{ _tag: "Delete", kind: "memory", id: "target" }]) as Record<string, unknown>),
        trusted: true,
      }).pipe(Effect.flip)
      expect(failure.reason).toBe("malformed")
    }),
