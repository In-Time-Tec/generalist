...
`@batonfx/transport` exposes the Runtime-owned `RunEvent` stream over SSE and WebSocket. Transport encodes events, carries exclusive replay cursors, reconnects observers, and dispatches explicit transport commands. It does not execute Agents, persist events, or own Run state.

- `RunEvent` is the only streamed lifecycle vocabulary. SSE event IDs and WebSocket reconnect cursors equal the persisted event `sequence`.
...
- Terminal facts are streamed exactly as persisted. Transport does not append `Ended`, status, failure, or snapshot frames.
- Producers encode with the canonical Runtime schema. Observers validate common RunEvent identity and cursor fields while retaining unknown future event tags.
