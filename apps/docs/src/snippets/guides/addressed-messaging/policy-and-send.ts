...
const messagingPolicy = Messaging.makePolicy({
  ...
  discover: (sender) =>
    Effect.succeed(
      [...(linkedThreads.get(sender.sessionId) ?? [])].map((sessionId) => AgentDirectory.sessionAddress(sessionId)),
    ),
})
...
export const ping = (input: {
  ...
  Runtime.Runtime.use((runtime) =>
    runtime.sendMessage({
      ...
      to: AgentDirectory.sessionAddress(input.targetSessionId),
