...
import { Address, ChildRuns, ExecutionHost, ExecutableResolver, Runtime, RunStore } from "../src/index.js"
...
const inspectFourChildReferences = (filename: string, expectedMarkers: ReadonlyArray<string>) => {
  ...
  const committed = readRows(
    ...
    (row) =>
      ...
      row.payload !== null &&
      "_tag" in row.payload &&
      row.payload._tag === "ModelResponseCommitted",
  )
  const sessions = readRows(
    database,
  ...
  const messages = sessions.filter(
    (row) =>
      typeof row.payload === "object" &&
      row.payload !== null &&
      "_tag" in row.payload &&
      row.payload._tag === "Message",
  )
  const compactions = sessions.filter(
    (row) =>
      typeof row.payload === "object" &&
      row.payload !== null &&
      "_tag" in row.payload &&
      row.payload._tag === "Compaction",
  )
  expect(messages).toHaveLength(42)
  expect(compactions).toHaveLength(1)
  const messageJson = messages.map((row) => JSON.stringify(row.payload)).join("\n")
  const requests = messageJson.match(/linear-child-\d-request/g)?.sort() ?? []
  const observations = messageJson.match(/observed:linear-child-\d-model-\d{2}/g)?.sort() ?? []
  ...
  const childOperations = operations.filter((row) =>
    (row.payload as { readonly sessionId: string }).sessionId.startsWith("fanout:"),
  )
  ...
  for (const row of operations) {
    ...
    expect(reference).toEqual({
      ...
      sessionParentId: expect.any(String),
  ...
  const preserved = referencedPayloads.flatMap((payload) => JSON.stringify(payload).match(markerPattern) ?? []).sort()
  expect(preserved).toEqual([...expectedMarkers].sort())
  expect(
    new Set(
      operations.map((row) => {
        const payload = row.payload as { readonly sessionId: string; readonly sessionEntryId: string }
    ...
    yield* Effect.scoped(
      provideScoped(
        ...
        Effect.gen(function* () {
          ...
          expect(yield* store.snapshot(admitted.parentRunId)).toMatchObject({
            outcome: {
              _tag: "Succeeded",
              result: {
                text: "linear-parent-model-01",
                turns: 2,
                session: { sessionId: "session:linear-four-child", leafId: expect.any(String) },
              },
            },
          })
        }),
