...
<Warning>
These instructions target Generalist 0.65.0. Use the [repository examples](/start/examples) to run directly from a checkout. Do not substitute an older release and assume the same APIs.
</Warning>

**Terminal**

```bash
bun add effect@4.0.0-rc.112 generalist@0.65.0
```
...
```bash
npm install effect@4.0.0-rc.112 generalist@0.65.0
pnpm add effect@4.0.0-rc.112 generalist@0.65.0
```

<Warning title="Pin the Effect release candidate">
Generalist 0.65.0 targets `effect@4.0.0-rc.112`. Effect AI APIs can change between release candidates. Use the documented version, and install optional `@effect/ai-*` and platform packages at the matching version.
</Warning>
...
| ------------ | ------- | ---------------------------------------------------------------------------------------------------------------------- |
| `generalist` | 0.65.0  | Node 22+ and Bun 1.4+: agent loop, object-backed Runtime, exact feature imports, and Cloudflare/Rivet compute adapters |
