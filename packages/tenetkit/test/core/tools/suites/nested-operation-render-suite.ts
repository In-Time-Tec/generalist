...
it.effect("drops a projection the schema refuses rather than failing the operation", () =>
  Effect.gen(function* () {
    const malformedRender = yield* Schema.decodeEffect(Schema.fromJsonString(Schema.Json))(
      '{"_tag":"Artifact","path":"/w/a.png","mimeType":"image/png","byteSize":4096,"width":0}',
    )
    const data = yield* NestedOperation.progressData({
      kind: "attach",
      ordinal: 0,
      status: "succeeded",
      render: Schema.decodeUnknownOption(NestedOperation.Render)(malformedRender).pipe(Option.getOrUndefined),
    })
