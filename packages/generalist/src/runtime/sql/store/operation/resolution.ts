...
const resolveOperationEffect = (
  ...
  Effect.gen(function* () {
    ...
    if (input.resolution._tag === "Succeeded") {
      yield* sql`
        UPDATE generalist_run_operations SET status = 'succeeded', result_json = ${encodeJsonValue(input.resolution.value)},
      ...
      yield* sql`
        UPDATE generalist_run_operations SET status = 'failed', error_json = ${encodeJsonValue(input.resolution.error)},
          resolution_idempotency_key = ${input.idempotencyKey}, resolution_json = ${resolutionJson}, finished_at = ${finished}
        WHERE run_id = ${input.runId} AND operation_id = ${input.operationId} AND status = 'unknown'
      `
    } else {
      yield* sql`
        UPDATE generalist_run_operations SET status = 'requested', result_json = NULL, error_json = NULL,
    ...
    if (clearLease) {
      ...
      yield* sql`
        UPDATE generalist_runs SET status = ${runStatus}, owner_worker_id = NULL, updated_at = ${finished}
        WHERE run_id = ${input.runId} AND status = 'needs-resolution'
      `
    }
  })
