...
function hydratePayload(payload: EntryPayload): HydratedPayload {
  ...
  return {
    ...
    content: payload.content.map((part) => {
      switch (part.type) {
        case "tool-call": return Response.makePart("tool-call", part)
        case "tool-result": return Response.makePart("tool-result", part)
...
const isPayload = (input: unknown): input is HydratedPayload =>
...
export const SessionEntryCodec: Schema.Codec<Entry, unknown> = StoredEntry.pipe(Schema.decodeTo(
...
const AppendPayload = EntryPayload.pipe(Schema.refine((payload): payload is AppendPayload => payload._tag !== "Compaction"))
export const SessionAppendInputCodec: Schema.Codec<AppendInput, unknown> = AppendPayload.pipe(Schema.decodeTo(
