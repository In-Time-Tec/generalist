...
const journalUnsafe = (state: RuntimeState, run: StoredRun): Journal => {
...
export const journal: {
  (runId: string): (state: RuntimeState) => Effect.Effect<Journal, RunNotFound | RuntimeUnavailable>
  (state: RuntimeState, runId: string): Effect.Effect<Journal, RunNotFound | RuntimeUnavailable>
...
const illegal = (state: RuntimeState, run: StoredRun, action: string) =>
...
export const retry: {
  ...
  if (decision._tag !== "RetryOperation") {
    return Effect.fail(illegal(state, run, "retry"))
  }
  const operations = new Map(state.operations)
  const runtimeOperation = operations.get(operationMapKey(input.runId, decision.operationId))
  if (
    ...
    operations.set(operationMapKey(input.runId, decision.operationId), retried)
    ...
    const key = `${input.runId}\0${decision.operationId}`
    ...
    return appendAction(
      ...
      input.runId,
      input.operator,
      { _tag: "Retry", operationId: decision.operationId },
    )
  }
  const runs = new Map(state.runs)
  const { ownerId: _, ...released } = run
...
} = Function.dual(2, (state: RuntimeState, input: ResolveUnknownInput) =>
  Effect.gen(function* () {
