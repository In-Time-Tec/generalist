...
export const structuredOutput = Prose.definePage({
  ...
  content: [
    ...
    Prose.p(
      ...
      Prose.code("schema"),
      ". ",
      Prose.code("generateObject"),
      " folds the stream and returns ",
      Prose.code("result.value"),
      " already decoded to the schema's type:",
    ),
    Prose.codeBlock({ label: "extract.ts", source: extract, expectedOutput: extractExpected }),
    Prose.callout(
      "info",
      "Two model paths",
      "The loop turns use ",
    ...
    Prose.codeBlock({ label: "stream-object.ts", source: streamObject, expectedOutput: streamObjectExpected }),
    Prose.h2("name-and-prompt", "3. Name the output and tune the terminal prompt"),
    Prose.p(
      "Two options shape the terminal turn. ",
      Prose.code("objectName"),
