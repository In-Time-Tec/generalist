...
const admission = (key: string) => ({
  message: { id: `message:${key}`, to: address, sessionId: `session:${key}`, prompt: Prompt.make("recover the exact accepted work"),
    idempotencyKey: key, correlationId: `correlation:${key}`, metadata: {} },
...
describe("object Runtime canonical mutations", () => {
  it.effect("recovers an admitted run and its host session on a fresh independent layer", () =>
    Effect.gen(function* () {
      ...
      const admitted = yield* Effect.scoped(Effect.gen(function* () {
  ...
  it.effect("an indeterminate budget extension can be resubmitted after restart without double charging", () =>
    Effect.gen(function* () {
      ...
      expect(history.filter((event) => event._tag === "BudgetExtended").map((event) => event.delta)).toEqual([{ tokens: 100 }])
      expect(yield* recovered.extendBudget({ ...command, delta: { tokens: 200 } }).pipe(Effect.flip)).toMatchObject({ reason: "input-conflict" })
    }).pipe(Effect.scoped, Effect.provide(BunCrypto.layer)),
  )
  ...
  it.effect("session appends survive restart and stale run/session authority cannot write after transfer", () =>
    Effect.gen(function* () {
      ...
      const entry = yield* writer.append({ _tag: "Message", message: Prompt.make("accepted").content[0]! }, { id: "entry-1", expectedLeafId: null })
      const second = yield* openActive(yield* bucket.connect, "owner-b")
      expect(yield* second.claimExecution({ runId: run.runId, ownerId: "owner-b", commandId: "claim-b-live" }).pipe(Effect.flip)).toMatchObject({ _tag: "generalist/runtime/RuntimeUnavailable" })
      ...
      expect(yield* writer.append({ _tag: "Message", message: Prompt.make("stale").content[0]! }, { id: "entry-stale", expectedLeafId: entry.id }).pipe(Effect.flip)).toMatchObject({ reason: "conflict" })
  ...
  it.effect("replays one generated Session append by command identity and rejects a divergent retry", () =>
    Effect.gen(function* () {
      ...
      const claim = yield* store.claimExecution({ runId: run.runId, ownerId: "owner-append", commandId: "claim-append" })
      ...
      const options = { commandId: "append-replay-1", expectedLeafId: null }
      const first = yield* writer.append(entry, options)
      expect(yield* writer.append(entry, options)).toEqual(first)
      const conflict = yield* Effect.flip(
        writer.append({ ...entry, message: Prompt.make("different logical write").content[0]! }, options),
      )
  ...
  it.effect("fresh ownership preserves an uncertain never-replay operation as Unknown", () =>
    Effect.gen(function* () {
      ...
      const claim = yield* first.claimExecution({ runId: run.runId, ownerId: "owner-a", commandId: "claim-a" })
      const operation = yield* first.recordOperation({ ...claim, operationKey: "external-payment", kind: "tool", inputDigest: "payment-input", input: { amount: 5 }, replayPolicy: "never", attempt: 1 })
      ...
      expect(yield* recovered.getOperation({ runId: run.runId, operationId: operation.operationId })).toMatchObject({ status: "unknown", replayPolicy: "never" })
