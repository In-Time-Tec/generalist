...
const claim = (
  ...
  SubscriptionRef.modify(state, (current) => {
    ...
    const selected = script[current.cursor]
    const step = available && selected !== undefined ? normalizeStep(selected) : undefined
    return [
      { step, request },
      {
...
export const toolCall: {
  ...
  (name: string, params: typeof Schema.Unknown.Type, options: ToolCallOptions = {}) => {
    let part: ToolCallPart = {
      _tag: "ToolCall",
      name,
      params: Schema.decodeUnknownSync(Schema.Unknown)(params),
      providerExecuted: options.providerExecuted ?? false,
    }
    if (options.id !== undefined) part = { ...part, id: options.id }
    return part
  },
)
...
const isStepOptionsLike = (value: typeof Schema.Unknown.Type): value is StepOptions =>
...
export const object: {
  ...
  (value: typeof Schema.Unknown.Type, options: StepOptions = {}) => ({
    _tag: "Object",
    value,
    ...options,
  }),
