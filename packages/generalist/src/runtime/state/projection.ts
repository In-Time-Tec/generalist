...
export const emptySession = (): RuntimeSession => ({
  entries: new Map(),
  order: [],
  leaf: null,
  counter: 0,
...
export interface RuntimeState {
  ...
  readonly sessions: ReadonlyMap<string, RuntimeSession>
  ...
  readonly workers: ReadonlyMap<string, { readonly expiresAt: number; readonly incarnation: string }>
...
export const emptyState = (input: {
  readonly addressBindings: RuntimeState["addressBindings"]
  readonly subscriberQueueCapacity: number
}): RuntimeState => ({
  closed: false,
  nextRunCounter: 1,
...
export const runWaits: {
  (runId: string): (state: RuntimeState) => ReadonlyArray<RunWait>
  (state: RuntimeState, runId: string): ReadonlyArray<RunWait>
} = Function.dual(
  2,
  (state: RuntimeState, runId: string): ReadonlyArray<RunWait> =>
