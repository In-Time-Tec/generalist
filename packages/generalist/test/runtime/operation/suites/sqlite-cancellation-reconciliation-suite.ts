...
it.effect("SQLite startup reconciles a poisoned running cancellation and stale claim", () => {
  ...
  return Effect.gen(function* () {
    ...
    const canonical = yield* Schema.decodeUnknownEffect(
      Schema.NullOr(Schema.Struct({ cancellation_requested: Schema.optional(Schema.Unknown) })),
    )(db.query("SELECT cancellation_requested FROM generalist_runs WHERE run_id = ?").get(runId))
    expect(canonical?.cancellation_requested).toBe(1)
    db.run("UPDATE generalist_runs SET status = 'running', cancellation_requested = 'true' WHERE run_id = ?", [
      childRunId,
    ])
    db.close()
