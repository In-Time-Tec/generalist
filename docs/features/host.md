...
Update or remove an item using its observed `expectedRevision` and a new `commandId`. An edit or removal addresses the earliest pending entry matching both `id` and `expectedRevision`. A `session.message` and a `session.submit` accepted under separate command identities can legitimately share one caller command id; each accepted entry keeps its own Run and promotion, and matching entries are addressed in FIFO order. An update can replace that item's Agent selection with `agent`; omitting it retains the item's pinned selection. Editing does not move the item in FIFO order or change the active Run. If an edit, removal, or promotion wins first, a stale revision fails `SessionQueueConflict`; reload the queue instead of silently retrying against a newer revision.
