...
license: MIT
compatibility: Uses Generalist's pinned Bun and workspace-local Mintlify CLI.
metadata:
  author: mintlify
  version: "1.0"
  source: https://github.com/mintlify/docs/blob/716e447d08e7016c461800a41402d15328e79dea/skill.md
  revision: 716e447d08e7016c461800a41402d15328e79dea
  adaptation: Repository paths, commands, generated API ownership, and opt-in tooling.
---
...
Do not automatically run `mint index`, install an MCP server, or globally install/update the CLI. Use the committed workspace tools. Ask before introducing external tooling.
...
Read `docs/docs.json`. Preserve existing page URLs and the Start / Build / Operate / Reference navigation. `docs/api/` and its navigation are generated by `scripts/docs-api.ts`; regenerate rather than hand-editing API signatures. Design decisions are secondary references, not onboarding requirements.
...
- `bun run dev` - Preview the docs; in an orb use a supervised service and portal
- `bun run docs:build` - Validate the site and internal links
- `bun run docs:api:check` - Check generated API freshness
- `bun run readme:check` - Check public install versions, typecheck snippets, and execute supported tutorial blocks
- `bun node_modules/prettier/bin/prettier.cjs --check <paths>` - Check changed-file formatting

Inspect rendered desktop/mobile and light/dark states after content or navigation changes. Exercise navigation, keyboard focus, and code copy; inspect the captured screenshots before claiming visual verification. Local preview does not prove deployed search works.
...
- [ ] Run `bun run docs:build` to validate pages and links
- [ ] Run `bun run readme:check` and report exactly which snippets ran
