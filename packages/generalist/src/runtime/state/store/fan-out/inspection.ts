...
export const inspectFanOut: {
  (fanOutId: string): (state: RuntimeState) => Effect.Effect<FanOutInspection, FanOutNotFound | RuntimeUnavailable>
  (state: RuntimeState, fanOutId: string): Effect.Effect<FanOutInspection, FanOutNotFound | RuntimeUnavailable>
} = Function.dual(2, (state: RuntimeState, fanOutId: string) => {
