...
One object-native durability engine serves application-owned hosts through local-directory or S3 storage. An entrypoint existing is not provider qualification: this report does not claim live AWS S3 support without accepted remote evidence. SQL drivers and an alternate memory Runtime are not supported.
...
The committed report manually records capabilities established by a complete shared object-native Runtime suite. The test-only simulator is not restart-safe production storage. Transport unit tests and signed local HTTP fixtures provide local evidence, not remote-provider certification.
...
The committed [`hosts-report.json`](./hosts-report.json) uses schema version 1 for the current object-native contract. It is manually maintained retained evidence, does not contain a source revision, and does not promote remote gates. Remote evidence must be reviewed and integrated explicitly.
