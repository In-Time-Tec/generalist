...
for (const backend of ["memory", "sqlite"] as const) {
  ...
  layer(
    (backend === "memory" ? Runtime.layerMemory(options) : SqliteRuntime.layerSqlite({ ...options, filename })).pipe(
      Layer.provide(fixture.resolverLayer),
    ),
  )(`${backend} gives each Program fan-out child only its required registrations`, (it) => {
