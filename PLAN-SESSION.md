...
PostgreSQL now has dialect-native Session storage and atomic completed, interrupted, and handoff conversation commits; PostgreSQL schema version 4 removes `transcript_json`. MySQL Session storage and the same atomic conversation transactions remain unfinished parity work. Its current execution schema still carries `transcript_json`; it must gain dialect-native Session tables/transactions and then delete that column and its compatibility paths.
