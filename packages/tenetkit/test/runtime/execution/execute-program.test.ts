...
describe("durable Agent Programs", () => {
    ...
    suite.effect("authorizes once and resumes exactly", () =>
      Effect.gen(function* () {
        ...
        const wrong = yield* runtime
          ...
          .pipe(Effect.flip)
        expect(wrong._tag).toBe("tenetkit/runtime/ApprovalMismatch")
        ...
        const conflict = yield* runtime
          ...
          .pipe(Effect.flip)
        expect(conflict._tag).toBe("tenetkit/runtime/ApprovalMismatch")
        ...
        const stale = yield* runtime
          ...
          .pipe(Effect.flip)
        expect(stale._tag).toBe("tenetkit/runtime/ApprovalStale")
  ...
  it.live("resolves a crashed non-idempotent Program operation without redispatch", () => {
    const verify = Effect.gen(function* () {
      ...
      ).toMatchObject({ _tag: "tenetkit/runtime/OperationResolutionConflict" })
