...
import { HarnessState, HarnessStore, Refinement } from "../../src/harness/index.js"
import { applied, create, entry, proposal, scope } from "./fixtures.js"

layer(HarnessStore.layerMemory)("HarnessStore.layerMemory", (test) => {
  ...
  test.effect("returns the exact saved state", () =>
    Effect.gen(function* () {
      const store = yield* HarnessStore.HarnessStore
      const state = HarnessState.make({ scope, entries: [entry({ id: "a", kind: "memory" })] })
  ...
  test.effect("keeps scopes independent", () =>
    Effect.gen(function* () {
      const store = yield* HarnessStore.HarnessStore
      yield* store.save(
        HarnessState.make({ scope: "isolated-a", entries: [entry({ id: "a", kind: "memory", scope: "isolated-a" })] }),
      )
      yield* store.save(
        HarnessState.make({ scope: "isolated-b", entries: [entry({ id: "b", kind: "skill", scope: "isolated-b" })] }),
      )
      expect(HarnessState.allEntries(yield* store.load("isolated-a")).map((value) => value.id)).toEqual(["a"])
      expect(HarnessState.allEntries(yield* store.load("isolated-b")).map((value) => value.id)).toEqual(["b"])
    }),
  )
  ...
  test.effect("round-trips a full apply through the store", () =>
    Effect.gen(function* () {
      ...
      expect(HarnessState.snapshotId(reloaded)).toBe(result.event.after)
...
const fixed = HarnessState.make({ scope, entries: [entry({ id: "fixed", kind: "prompt" })] })

layer(
  HarnessStore.layerTest({
    load: () => Effect.succeed(fixed),
    save: (state) =>
      Effect.fail(
        HarnessStore.HarnessStoreError.make({ reason: "unwritable", scope: state.scope, message: "read only" }),
      ),
  }),
)("HarnessStore.layerTest", (test) => {
...
describe("HarnessStore.HarnessStoreError", () => {
  it("encodes as a tagged boundary error", () => {
    const failure = HarnessStore.HarnessStoreError.make({ reason: "unwritable", scope, message: "disk full" })
    expect(Schema.encodeSync(HarnessStore.HarnessStoreError)(failure)).toMatchObject({
      reason: "unwritable",
      scope,
      message: "disk full",
    })
