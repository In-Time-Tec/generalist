...
export const handle: Handle = Effect.gen(function* () {
  ...
  const writeEvent = (event: RunEvent) =>
    (event._tag === "ModelResponseCommitted" || event._tag === "ModelResponseInterrupted"
      ...
      Effect.catchTag("generalist/transport/WireCodecFailed", (error) =>
  ...
  const attach = (runId: string, cursor: number | undefined) =>
    SynchronizedRef.modifyEffect(state, (current) => {
      ...
      return runtime.events(options).pipe(
        ...
        Effect.catchTags({
          "generalist/transport/WireCodecFailed": () => Effect.void,
          "generalist/runtime/SubscriberLagged": (error) => close(4000, `lagged:${error.lastDeliveredSequence}`),
          "generalist/runtime/CursorExpired": (error) => close(4001, `cursor-expired:${error.earliestSequence}`),
  ...
  const handleText = (text: string) =>
    decodeCommand(text).pipe(
      ...
      Effect.catchTags({
        "generalist/transport/WireCodecFailed": () => close(1003, "malformed-command"),
        "generalist/transport/NotAttached": () => close(1008, "not-attached"),
        "generalist/transport/RunMismatch": () => close(1008, "run-mismatch"),
        "generalist/runtime/RunNotFound": () => close(4004, "run-not-found"),
        "generalist/runtime/RuntimeUnavailable": () => close(1011, "runtime-unavailable"),
      }),
    )
