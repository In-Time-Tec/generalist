import { Module, type Request } from "../../core/program/code-executor.js"
...
const compareModules = (left: Module, right: Module): number => {
  if (left.name < right.name) return -1
  if (left.name > right.name) return 1
  return 0
}

const validateImports = (module: Module, names: ReadonlySet<string>): void => {
...
export const normalize = (request: Request): NormalizedSource => {
  initSync()
