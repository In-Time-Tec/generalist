...
import {
  ...
  layerClient,
} from "../../../src/unstable/providers/openai-account.js"
import { classifyFailure, layer as openAiLayer } from "../../../src/ai/provider/openai.js"
import type { OpenAIAccountAuth } from "../../../src/unstable/providers/openai-account-auth.js"
...
const sse = <Event>(...events: ReadonlyArray<Event>) => events.map((event) => `data: ${stringify(event)}\n\n`).join("")
const completedFrame = (response: typeof responseBody | typeof textResponseBody = responseBody) =>
...
const streamingOnlyClient = (requests: Array<CapturedRequest>, body: BodyInit = completedFrame()) =>
  HttpClient.make((request, url) => {
    ...
    const streaming = Schema.is(Schema.Struct({ stream: Schema.Literal(true) }))(decoded)
