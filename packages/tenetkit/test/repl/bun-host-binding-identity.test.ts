...
const whoami = {
  ...
  output: Out,
  failure: Fail,
  handle: () => Effect.map(ToolContext.ToolContext, (context) => ({ session: context.sessionId })),
}
...
layer(platform, liveOptions)("Bun kernel host bindings", (it) => {
  ...
  it.effect("answers a cell's host request with that cell's own Session identity", () =>
    withPool({
      overrides: { modules: [{ name: "probe", operations: [whoami] } as unknown as HostBindingRegistry.Module] },
      use: ({ pool }) =>
        Effect.gen(function* () {
