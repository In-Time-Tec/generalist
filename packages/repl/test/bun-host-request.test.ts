...
class SlowFailure extends Schema.TaggedErrorClass<SlowFailure>()("@batonfx/repl/test/SlowFailure", {
  ...
  operations: [
    {
      ...
      failure: SlowFailure,
      handle: (request) =>
        Deferred.succeed(input.arrived, undefined).pipe(
          Effect.andThen(Deferred.await(input.gate)),
          Effect.as((request as { readonly value: number }).value * 2),
        ),
    },
  ],
})
  ...
  operations: [
    {
      ...
      failure: SlowFailure,
      handle: (input) => Effect.succeed(String((input as { readonly text: string }).text).toUpperCase()),
    },
    {
      name: "boom",
      input: Schema.Struct({}),
