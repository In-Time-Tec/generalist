...
export const settlementNotifications = (input: {
  ...
  Effect.gen(function* () {
    ...
    const parents = yield* sql<{ session_id: string }>`
      SELECT session_id FROM generalist_runs WHERE run_id = ${input.parentRunId}
    `
    ...
    const rows = yield* sql<NotificationRow>`
      SELECT sequence, admitted_at_millis, metadata_json FROM generalist_messages
...
export const admitChildSettlement = (input: {
  ...
  Effect.gen(function* () {
    ...
    const existing = yield* sql<{ entry_id: string }>`
      SELECT entry_id FROM generalist_messages WHERE entry_id = ${notificationId}
    `
    if (existing.length > 0) return
    const member = yield* sql<{ child_run_id: string }>`
      SELECT m.child_run_id FROM generalist_fan_out_members m
      JOIN generalist_fan_outs f ON f.fan_out_id = m.fan_out_id
    ...
    const highest = yield* sql<{ next_sequence: number | string }>`
      SELECT COALESCE(MAX(sequence), -1) + 1 AS next_sequence FROM generalist_messages
    ...
    yield* sql`
      INSERT INTO generalist_messages (
...
export const admitChildSettlementFromEventId = (input: {
  ...
  Effect.gen(function* () {
    ...
    const rows = yield* sql<{ event_json: string }>`
      SELECT event_json FROM generalist_run_events WHERE event_id = ${input.terminalEventId}
    `
    if (rows[0] !== undefined) {
