...
): Effect.Effect<MemberCancellation, PayloadTooLarge | RuntimeUnavailable, PreparedObservation> =>
  Effect.gen(function* () {
...
export const applyRemainder: {
  ...
  ): (state: RuntimeState) => Effect.Effect<AppliedRemainder, RuntimeUnavailable>
  (
    state: RuntimeState,
    fanOut: StoredFanOut,
    members: ReadonlyArray<FanOutMemberResult>,
