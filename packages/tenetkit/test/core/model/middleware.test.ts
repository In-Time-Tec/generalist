  ...
  ItLayer.make(it, "rejects duplicate transformed tool-call IDs before the duplicate executes", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        expect("error" in outcome && outcome.error._tag).toBe("tenetkit/core/DuplicateToolCallId")
        if ("error" in outcome && outcome.error._tag === "tenetkit/core/DuplicateToolCallId") {
  ...
  ItLayer.make(it, "rejects non-adjacent and provider-executed duplicate IDs with tool-call positions", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        if (failure._tag === "tenetkit/core/DuplicateToolCallId") {
    ...
    () =>
      [
        ...
        Effect.gen(function* () {
          ...
          if (failure._tag === "tenetkit/core/MiddlewareViolation") {
  ...
  ItLayer.make(it, "middleware failure: transformPrompt failure fails the run before the model is called", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        expect(failure._tag === "tenetkit/core/AgentError" && failure.message).toBe("prompt middleware boom")
      }),
    ] as const
  })
})
