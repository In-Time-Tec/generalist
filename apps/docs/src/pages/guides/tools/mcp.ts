...
export const mcp = definePage({
  ...
  content: [
    ...
    p(
      ...
      " remains the lower-level client API. The usual ",
      code("connect"),
      " API opens the connection, lists the tools once, and closes the client when its Effect scope ends. Discovered tool names are prefixed with the server name: a ",
      code("search"),
      " tool on the ",
      code("files"),
      " server becomes ",
