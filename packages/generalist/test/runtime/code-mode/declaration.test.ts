...
describe("CodeMode declaration", () => {
  ...
  it("rejects duplicate names and tools outside the Agent toolkit", () => {
    ...
    expect(duplicateTool).toMatchObject({ field: "tools", reason: "duplicate-name", declarationName: tool.name })
    ...
    expect(duplicateAgent).toMatchObject({ field: "agents", reason: "duplicate-name", declarationName: "child" })
    ...
    expect(duplicateStep).toMatchObject({ field: "steps", reason: "duplicate-name", declarationName: "same_step" })
    ...
    expect(absent).toMatchObject({ field: "tools", reason: "not-agent-tool", declarationName: otherTool.name })
  })
  ...
  it("rejects unstable versions, invalid executor identity, and every invalid limit", () => {
    ...
    for (const field of budgetFields) {
      ...
      for (const value of values) {
        ...
        expect(failure).toMatchObject({ field: "budget", reason: "limit-invalid", declarationName: field })
      }
    }
  })
})
