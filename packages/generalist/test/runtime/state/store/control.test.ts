...
const admitWaitWithClaimedChild = (waitId: string) =>
  Effect.gen(function* () {
    ...
    yield* store.claimExecution({
      commandId: `${child.runId}:control:claim`,
      runId: child.runId,
      ownerId: objectWorkerId,
    })
    const parentClaim = yield* store.claimExecution({
      commandId: `${parent.runId}:control:suspend:claim`,
      runId: parent.runId,
      ownerId: objectWorkerId,
    })
    yield* store.suspend({
...
const duplicateResponseAfterCancellation = (waitId: string) =>
  Effect.gen(function* () {
    ...
    yield* runtime.cancel({ commandId: `${runId}:control:cancel`, runId, reason: "stop" })
    expect((yield* runtime.inspect(runId)).status).toBe("cancelling")

    yield* runtime.respond({ runId, waitId, resolution })
    yield* store.resume({ commandId: `${runId}:control:resume`, runId, waitId, resolution })
