/** Mutable last-send clock driving conversation cache escalation. */
export interface SendClock {
  readonly idleSince: (now: number) => number | undefined
}

/** One run's last-send tracker; gaps above the escalation threshold move the conversation boundary to one hour. */
