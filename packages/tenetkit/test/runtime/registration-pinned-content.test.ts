...
import { AgentManifest, ExecutableManifest as CoreExecutableManifest, Pins } from "tenetkit"
...
describe("pinned content in executable registrations", () => {
  ...
  it.effect("fails typed on a wrong codec", () =>
    Effect.gen(function* () {
      const failure = yield* validate([...others, registrationFor({ codec: "@tenetkit/other" })]).pipe(Effect.flip)
      expect(failure._tag).toBe("tenetkit/runtime/ExecutableRegistrationInvalid")
