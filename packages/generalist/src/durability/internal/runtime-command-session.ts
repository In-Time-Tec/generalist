...
import { SessionAppendInputCodec, SessionEntryCodec } from "./runtime-state/session.js"
...
export const commands = {
  reserveEntryId: { tag: "session.reserveEntryId", input: reserveInput, receipt: Schema.String,
    identity: ([claim, commandId]: typeof reserveInput.Type) => JSON.stringify([claim.session.sessionId, claim.session.epoch, commandId]),
  },
  append: { tag: "session.append", input: appendInput, receipt: SessionEntryCodec,
    identity: ([claim, , options]: typeof appendInput.Type) => JSON.stringify([claim.session.sessionId, claim.session.epoch,
      "id" in options ? ["entry", options.id] : ["command", options.commandId]]),
  },
  appendCheckpoint: { tag: "session.appendCheckpoint", input: checkpointInput, receipt: CheckpointAppend,
    identity: ([claim, checkpoint]: typeof checkpointInput.Type) => JSON.stringify([claim.session.sessionId, claim.session.epoch, checkpoint.id]),
  },
  setLeaf: { tag: "session.setLeaf", input: leafInput, receipt: Schema.Void,
    identity: ([claim, , commandId]: typeof leafInput.Type) => JSON.stringify([claim.session.sessionId, claim.session.epoch, commandId]),
  },
} as const
