...
- A root Run that has admitted children cannot be rewound. The request fails with `RuntimeUnavailable` (`run <id> has initial children`) before any state change, so the Run keeps its prior status, children, and retained history.
