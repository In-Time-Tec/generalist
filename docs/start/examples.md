...
Choose the behavior you need to prove: a tool call in the current process, recovery after closing a Runtime, or an approval round trip through a browser. These examples run from the workspace, so you can try the current code without installing a registry release.
