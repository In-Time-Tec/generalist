...
const toEntry = (state: RuntimeState, run: StoredRun): DirectoryEntry => {
...
export const directory: {
  (runId: string): (state: RuntimeState) => Effect.Effect<DirectoryEntry, RunNotFound | RuntimeUnavailable>
  (state: RuntimeState, runId: string): Effect.Effect<DirectoryEntry, RunNotFound | RuntimeUnavailable>
...
export const resolveAddress: {
  (address: Address): (state: RuntimeState) => Effect.Effect<DirectoryEntry, AddressNotFound | RuntimeUnavailable>
  (state: RuntimeState, address: Address): Effect.Effect<DirectoryEntry, AddressNotFound | RuntimeUnavailable>
} = Function.dual(2, (state: RuntimeState, address: Address) =>
  Effect.gen(function* () {
...
export const listRelated: {
  (
    runId: string,
  ): (state: RuntimeState) => Effect.Effect<ReadonlyArray<DirectoryEntry>, RunNotFound | RuntimeUnavailable>
  (state: RuntimeState, runId: string): Effect.Effect<ReadonlyArray<DirectoryEntry>, RunNotFound | RuntimeUnavailable>
} = Function.dual(2, (state: RuntimeState, runId: string) =>
...
export const settlementNotifications: {
  ...
  }): (state: RuntimeState) => Effect.Effect<ReadonlyArray<Notification>, RunNotFound | RuntimeUnavailable>
  (
    state: RuntimeState,
...
export const admitChildSettlement: {
  ...
  }): (state: RuntimeState) => Effect.Effect<RuntimeState, never, PreparedObservation>
  (
    state: RuntimeState,
    input: { readonly parent: StoredRun; readonly child: StoredRun; readonly event: RunEvent },
  ): Effect.Effect<RuntimeState, never, PreparedObservation>
} = Function.dual(
  2,
  (state: RuntimeState, input: { readonly parent: StoredRun; readonly child: StoredRun; readonly event: RunEvent }) =>
    Effect.gen(function* () {
      ...
      const entry = observationEntry({
        ...
        admittedAtMillis: yield* preparedOccurredAtMillis,
      })
