...
`connect` scopes one MCP connection, one discovered toolkit, its handlers, and the matching TenetKit executor. Releasing the scope closes the connection. Calls preserve interruption and optional finite timeouts; the connection adds no detached work, queue, or retry policy.
