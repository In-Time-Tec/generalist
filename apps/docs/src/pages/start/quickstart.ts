import step1 from "virtual:source/src/snippets/quickstart/step-1.sh"
import step2 from "virtual:source/src/snippets/quickstart/step-2.ts"
import step2Expected from "virtual:source/src/snippets/quickstart/step-2.expected.txt"
import step3 from "virtual:source/src/snippets/quickstart/step-3.ts"
import step3Expected from "virtual:source/src/snippets/quickstart/step-3.expected.txt"
import step4 from "virtual:source/src/snippets/quickstart/step-4.ts"
import step4Expected from "virtual:source/src/snippets/quickstart/step-4.expected.txt"
import step5 from "virtual:source/src/snippets/quickstart/step-5.ts"
...
export const quickstart = definePage({
  ...
  content: [
    ...
    callout(
      ...
      "A Generalist run always needs a model. A tool-calling run also needs the Effect AI handler layer for its toolkit and an explicit authorization policy — Permissions plus Approvals — so who can run what is never an implicit default. ToolExecutor, middleware, memory, skills, compaction, and steering are optional seams: absent means default behavior. ",
    ...
    bullets(
      ...
      [
        code("Permissions.layerAllowAll"),
        " + ",
        code("Approvals.layerAutoApprove"),
        ": required whenever the agent has tools. The run fails fast without them, so authorization is always a conscious choice.",
      ],
      [
    ...
    callout(
      "info",
      "The layerModel pattern",
      "Every provider leaf exports ",
      code("layerModel({ model })"),
      ", a LanguageModel layer over its client. Provide it to the run with ",
      code("Effect.provide"),
      " — no registry, no selection strings. Swapping to a real model is the same shape: ",
      code("layerConfig"),
      " builds the client, ",
      code("layerModel"),
      " pins the model (see ",
    ...
    bullets(
      ["Connect a real model: ", link("/docs/guides/providers", "How to provide model providers"), "."],
      [
