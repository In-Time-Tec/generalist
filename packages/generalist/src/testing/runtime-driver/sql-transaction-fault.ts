  ...
  it.live(`${options.name} rolls back interrupted transaction work and releases its connection`, () =>
    withClient(
      ...
      Effect.gen(function* () {
        ...
        yield* sql`DROP TABLE IF EXISTS generalist_transaction_fault_probe`
        yield* sql`CREATE TABLE generalist_transaction_fault_probe (id INTEGER PRIMARY KEY, value INTEGER NOT NULL)`
        const inserted = yield* Deferred.make<void>()
        const transaction = yield* sql
          .withTransaction(
            Effect.gen(function* () {
              yield* sql`INSERT INTO generalist_transaction_fault_probe VALUES (1, 1)`
        ...
        yield* sql`DROP TABLE generalist_transaction_fault_probe`
      }),
    ),
  )
  ...
  it.live(`${options.name} interrupts a transaction blocked on a row lock without applying its write`, () =>
    withClient(
      ...
      Effect.gen(function* () {
        ...
        yield* sql`DROP TABLE IF EXISTS generalist_transaction_fault_probe`
        yield* sql`CREATE TABLE generalist_transaction_fault_probe (id INTEGER PRIMARY KEY, value INTEGER NOT NULL)`
        yield* sql`INSERT INTO generalist_transaction_fault_probe VALUES (1, 0)`
        const locked = yield* Deferred.make<void>()
        const release = yield* Deferred.make<void>()
        const holder = yield* sql
          .withTransaction(
            Effect.gen(function* () {
              yield* sql`UPDATE generalist_transaction_fault_probe SET value = value + 1 WHERE id = 1`
        ...
        const waiting = yield* sql
          .withTransaction(
            Effect.gen(function* () {
              ...
              yield* sql`UPDATE generalist_transaction_fault_probe SET value = value + 10 WHERE id = 1`
            }),
          )
          .pipe(Effect.forkChild)
        ...
        yield* sql.withTransaction(sql`UPDATE generalist_transaction_fault_probe SET value = value + 1 WHERE id = 1`)
        expect(
          yield* sql<{ value: number }>`SELECT value FROM generalist_transaction_fault_probe WHERE id = 1`,
        ).toEqual([{ value: 2 }])
        yield* sql`DROP TABLE generalist_transaction_fault_probe`
