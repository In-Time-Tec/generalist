...
import { AgentProgram, ExecutableManifest, Pins, ProgramBindings, ProgramCapabilities, SandboxExecutor } from "tenetkit"
      ...
      suite.effect(`rejects ${name} before journal access`, () =>
        Effect.gen(function* () {
          ...
          expect(result.outcome).toMatchObject({
            _tag: "Failed",
            error: { _tag: "tenetkit/core/ProgramSchemaFailure" },
          })
  ...
  it.effect("rejects result excess fields and replay divergence", () => {
    ...
    return provideScoped(
      ...
      execute(excess.address).pipe(
        Effect.tap(({ outcome }) =>
          Effect.sync(() =>
            expect(outcome).toMatchObject({ _tag: "Failed", error: { _tag: "tenetkit/core/ProgramSchemaFailure" } }),
          ),
        ),
      ),
    ).pipe(
            ...
            Effect.tap(({ outcome }) =>
              Effect.sync(() =>
                expect(outcome).toMatchObject({
                  _tag: "Failed",
                  error: { _tag: "tenetkit/core/ProgramReplayDivergence" },
                }),
              ),
            ),
          ),
