import "./protocol-model-suite.js"
import "./journal-cache-suite.js"
...
const slot = (sequence: string) => `${prefix}commits/${sequenceName(sequence)}.json`
...
const countOf = (state: State) => Option.getOrElse(Schema.decodeUnknownOption(Schema.Finite)(state.count), () => 0)
const increment = (state: State) => {
  const count = countOf(state) + 1
  return Effect.succeed({ patches: [{ op: "set" as const, path: ["count"], value: count }], receipt: { count } })
}
const sealedBytes = (record: Json) =>
  Effect.gen(function* () {
    const crypto = yield* Crypto.Crypto
    const digest = Encoding.encodeHex(yield* crypto.digest("SHA-256", yield* bytes(record)))
    return yield* bytes({ digest, record })
  })
...
describe("immutable numbered object journal", () => {
  it.effect("serializes independent writers and reevaluates only the losing deterministic reducer", () =>
    Effect.gen(function* () {
      ...
      const pending = yield* first
        .commit({ id: "first", input: null }, (state) => {
          observed.push(countOf(state))
  ...
  it.effect("returns the winning original receipt for concurrent identical commands without reevaluation", () =>
    Effect.gen(function* () {
      ...
      let evaluations = 0
      const pending = yield* first
        .commit({ id: "same", input: { b: 2, a: 1 } }, (state) => {
          evaluations += 1
          return increment(state)
        })
        .pipe(Effect.forkChild({ startImmediately: true }))
  ...
  it.effect("reconciles a lost successful PUT response from the exact attempted slot", () =>
    Effect.gen(function* () {
      ...
      const fresh = yield* open(yield* bucket.connect)
      expect(yield* fresh.commit({ id: "lost-response", input: null }, () => Effect.fail("must not evaluate"))).toEqual(
        { count: 1 },
      )
  ...
  it.effect(
    "returns indeterminate when a successful PUT cannot be reconciled, then recovers its receipt after restart",
    () =>
      Effect.gen(function* () {
        const bucket = yield* makeSimulator()
        const journal = yield* open(bucket)
        ...
        const fresh = yield* open(yield* bucket.connect)
        expect(yield* fresh.commit({ id: "unknown", input: [1] }, () => Effect.fail("must not evaluate"))).toEqual({
          count: 1,
        })
        expect(yield* bucket.store.read(slot("1"), { maxBytes: 1024 * 1024 })).toBeUndefined()
      }).pipe(Effect.provide(BunCrypto.layer)),
  ...
  it.effect("does not retry an unconfirmed missing attempted slot inside the original commit", () =>
    Effect.gen(function* () {
      ...
      expect((yield* journal.commit({ id: "retry", input: null }, increment).pipe(Effect.flip)).reason).toBe(
        "indeterminate",
      )
  ...
  it.effect(
    "recovers snapshots, deltas, and original receipts through paginated fresh-client discovery without trusting hints",
    () =>
      Effect.gen(function* () {
        ...
        expect(yield* fresh.commit({ id: "command-0", input: 0 }, () => Effect.fail("must not evaluate"))).toEqual({
          count: 1,
        })
        expect((yield* fresh.commit({ id: "command-0", input: 99 }, increment).pipe(Effect.flip)).reason).toBe(
          "input-conflict",
        )
        for (let index = 0; index < 7; index += 1)
          expect(yield* bucket.store.read(slot(String(index)), { maxBytes: 1024 * 1024 })).toBeDefined()
        expect(yield* bucket.store.read(slot("7"), { maxBytes: 1024 * 1024 })).toBeUndefined()
      }).pipe(Effect.provide(BunCrypto.layer)),
  )

  it.effect(
    "acknowledges a committed command after snapshot failure but blocks extension until the recovery boundary publishes",
    () =>
      Effect.gen(function* () {
        const bucket = yield* makeSimulator()
        const journal = yield* open(bucket, { snapshotEvery: 2 })
        ...
        const pending = yield* journal
          .commit({ id: "one", input: null }, increment)
          .pipe(Effect.forkChild({ startImmediately: true }))
        yield* pause.entered
        ...
        expect(yield* fresh.read).toMatchObject({ sequence: "1", state: { count: 2 } })
        yield* bucket.faults.failNextCreate({ phase: "before" })
        expect((yield* journal.commit({ id: "two", input: null }, increment).pipe(Effect.flip)).reason).toBe(
          "transport",
        )
  ...
  it.effect("refuses a retained-history gap even when a later snapshot can reconstruct the state", () =>
    Effect.gen(function* () {
      ...
      expect((yield* fresh.commit({ id: "after-gap", input: null }, increment).pipe(Effect.flip)).reason).toBe(
        "corruption",
      )
      expect(yield* bucket.store.read(slot("1"), { maxBytes: 1024 * 1024 })).toBeUndefined()
  ...
  it.effect("checks SHA-256 independently of an unchanged opaque ETag before any mutation", () =>
    Effect.gen(function* () {
      ...
      const corrupt = yield* bytes({ ...envelope, record: { ...envelope.record, receipt: "modified" } })
  ...
  it.effect("rejects unsupported versions even when their canonical checksum is valid", () =>
    Effect.gen(function* () {
      ...
      const envelope = yield* decode(Envelope, yield* parse(object.bytes), "corruption")
      yield* bucket.faults.corrupt(slot("0"), yield* sealedBytes({ ...envelope.record, version: 2 }))
      expect((yield* journal.commit({ id: "next", input: null }, increment).pipe(Effect.flip)).reason).toBe(
        "unsupported-version",
      )
      expect(yield* bucket.store.read(slot("1"), { maxBytes: 1024 * 1024 })).toBeUndefined()
    }).pipe(Effect.provide(BunCrypto.layer)),
  ...
  it.effect("bounds state plus retained receipts and individual commits during publication and recovery", () =>
    Effect.gen(function* () {
      ...
      const stateError = yield* stateLimited
        .commit({ id: "large-state", input: null }, () =>
          Effect.succeed({
            patches: [{ op: "set" as const, path: ["large"], value: "x".repeat(300) }],
            receipt: null,
          }),
        )
        .pipe(Effect.flip)
      expect(stateError.reason).toBe("limit")
      const receiptError = yield* stateLimited
        .commit({ id: "large-receipt", input: null }, () => Effect.succeed({ patches: [], receipt: "x".repeat(300) }))
        .pipe(Effect.flip)
      expect(receiptError.reason).toBe("limit")
      const commitLimited = yield* open(bucket, { maxCommitBytes: 100 })
      expect(
        (yield* commitLimited.commit({ id: "large-commit", input: null }, increment).pipe(Effect.flip)).reason,
      ).toBe("limit")
      expect(yield* bucket.store.read(slot("0"), { maxBytes: 1024 * 1024 })).toBeUndefined()
  ...
  it.effect("keeps path segments isolated and permits array replacement but not array traversal", () =>
    Effect.gen(function* () {
      ...
      yield* journal.commit({ id: "array", input: null }, () =>
        Effect.succeed({
          patches: [{ op: "set" as const, path: ["array"], value: [1, 2] }],
          receipt: null,
        }),
      )
      expect(
        (yield* journal
          .commit({ id: "index", input: null }, () =>
            Effect.succeed({
              patches: [{ op: "set" as const, path: ["array", "0"], value: 3 }],
              receipt: null,
            }),
          )
          .pipe(Effect.flip)).reason,
      ).toBe("encoding")
      yield* journal.commit({ id: "replace", input: null }, () =>
        Effect.succeed({
          patches: [{ op: "set" as const, path: ["array"], value: [3] }],
          receipt: null,
        }),
      )
      const fresh = yield* open(yield* bucket.connect, {
  ...
  it.effect("validates snapshot bytes before using their state and retained receipts", () =>
    Effect.gen(function* () {
      ...
      const envelope = yield* decode(Envelope, yield* parse(snapshot.bytes), "corruption")
      yield* bucket.faults.corrupt(
        snapshotKey,
        yield* bytes({
          ...envelope,
          record: { ...envelope.record, state: { count: 99 } },
        }),
      )
      const fresh = yield* open(yield* bucket.connect, { snapshotEvery: 2 })
      expect((yield* fresh.read.pipe(Effect.flip)).reason).toBe("corruption")
  ...
  it.effect("rejects malformed committed transitions even when their checksum is valid", () =>
    Effect.gen(function* () {
      ...
      yield* bucket.faults.corrupt(
        ...
        yield* sealedBytes({
          ...envelope.record,
          patches: [{ op: "increment", path: ["count"], value: 1 }],
        }),
      )
      expect((yield* journal.read.pipe(Effect.flip)).reason).toBe("corruption")
      expect((yield* journal.commit({ id: "next", input: null }, increment).pipe(Effect.flip)).reason).toBe(
  ...
  it.effect("applies ordered nested object patches without changing an earlier returned head", () =>
    Effect.gen(function* () {
      const bucket = yield* makeSimulator()
      const journal = yield* open(bucket)
      yield* journal.commit({ id: "initial", input: null }, () =>
        Effect.succeed({
          patches: [{ op: "set" as const, path: ["object"], value: { keep: 1, remove: 2 } }],
          receipt: null,
        }),
      )
      const earlier = yield* journal.read
      yield* journal.commit({ id: "nested", input: null }, () =>
        Effect.succeed({
          patches: [
      ...
      expect(
        (yield* fresh
          .commit({ id: "missing", input: null }, () =>
            Effect.succeed({
              patches: [{ op: "remove" as const, path: ["object", "missing"] }],
              receipt: null,
            }),
          )
          .pipe(Effect.flip)).reason,
  ...
  it.effect("reconstructs with read-only credentials and never probes or publishes snapshots from make or read", () =>
    Effect.gen(function* () {
      const bucket = yield* makeSimulator()
      let creates = 0
      const reader = yield* makeJournal({ ...identity, snapshotEvery: 2 }).pipe(
      ...
      expect(yield* reader.commit({ id: "zero", input: null }, () => Effect.fail("must not evaluate"))).toEqual({
        count: 1,
      })
      expect(creates).toBe(0)
  ...
  it.effect("stops deterministic conflict retries at the configured boundary without accepting a losing command", () =>
    Effect.gen(function* () {
      ...
      const observed: Array<number> = []
      const pending = yield* losing
        .commit({ id: "contended", input: null }, (state) => {
          observed.push(countOf(state))
          return increment(state)
        })
  ...
  it.effect("compares canonical logical state independently of commit and receipt history", () =>
    Effect.gen(function* () {
      ...
      yield* journal.commit({ id: "receipt-only", input: null }, () =>
        Effect.succeed({ patches: [], receipt: "acknowledged" }),
      )
      const fresh = yield* open(yield* bucket.connect, { snapshotEvery: 2 })
      const second = yield* fresh.read
      const crypto = yield* Crypto.Crypto
      const expectedDigest = Encoding.encodeHex(
        yield* crypto.digest("SHA-256", new TextEncoder().encode('{"count":1}')),
      )
  ...
  it("preserves decimal slot identity beyond safe integers and the initial padding width", () => {
    ...
    expect(nextSequence("99999999999999999999")).toBe("100000000000000000000")
    expect(sequenceFromName(sequenceName("100000000000000000000"))).toBe("100000000000000000000")
    expect(["100000000000000000000", "99999999999999999999"].toSorted(compareSequence)).toEqual([
      "99999999999999999999",
      "100000000000000000000",
    ])
    expect(sequenceFromName("000000000000000000000")).toBeUndefined()
  })
})
