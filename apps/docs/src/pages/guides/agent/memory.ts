...
export const memory = definePage({
  ...
  content: [
    p(
      ...
      " of ",
      code("{ agent, subject }"),
      " that you choose. Generalist never derives memory identity from session ids or users.",
    ),
    p(
    ...
    callout(
      ...
      " to ",
      code("WorkingMemory.layer"),
      " with a closed LanguageModel layer (for example a provider's ",
      code("layerModel"),
      ") to fold messages that fall out of the window into a running summary instead of dropping them. Pass ",
      code("summarize: {}"),
      " to use the LanguageModel provided where the layer is built; the layer then carries that requirement.",
    ),
