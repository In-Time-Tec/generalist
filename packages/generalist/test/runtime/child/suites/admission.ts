    ...
    it.live("carries the originating operation and ordinal into the child-tree events", () =>
      provide(
        Effect.gen(function* () {
          ...
          // Correlation must be readable from Generalist's own events, never by parsing the cell source.
    ...
    it.live("orders children of one cell by their host-assigned ordinal", () =>
      provide(
        Effect.gen(function* () {
          ...
          expect(ordinals.toSorted((left, right) => left - right)).toEqual([0, 1, 2])
