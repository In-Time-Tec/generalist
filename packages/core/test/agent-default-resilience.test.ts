...
describe("Agent default model resilience", () => {
  it.effect("retries provider 429 and 5xx failures with backoff inside one turn", () =>
    Effect.gen(function* () {
      const model = scriptedModel((calls) =>
        calls === 1 ? Stream.fail(rateLimit) : calls === 2 ? Stream.fail(unavailable) : success,
      )
  ...
  it.effect("fails with the typed provider cause after the default retry budget is exhausted", () =>
    Effect.gen(function* () {
      ...
      expect(model.calls()).toBe(3)
      expect(finalFailure._tag).toBe("@batonfx/core/AgentError")
      if (finalFailure._tag === "@batonfx/core/AgentError") expect(finalFailure.cause).toBe(unavailable)
    }),
  )
  ...
  const expectNoRetry = (terminal: AiError.AiError) =>
    Effect.gen(function* () {
      ...
      expect(model.calls()).toBe(1)
      expect(finalFailure._tag).toBe("@batonfx/core/AgentError")
      if (finalFailure._tag === "@batonfx/core/AgentError") expect(finalFailure.cause).toBe(terminal)
    })
