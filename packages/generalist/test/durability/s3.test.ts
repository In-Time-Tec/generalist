/* oxlint-disable anti-slop/no-runtime-typeof, anti-slop/require-safety-comment-for-type-assertion, effecttsgo/async-function, effecttsgo/new-promise, effecttsgo/node-builtin-import, effecttsgo/strict-boolean-expressions, eslint/no-promise-executor-return, typescript/no-unsafe-argument, typescript/no-unsafe-type-assertion -- This transport suite deliberately drives Node's Promise-only HTTP server and AWS SDK streaming boundary, including stalled Promise and partial-SDK-body fixtures. */
...
const objectResponse = (bytes: Uint8Array, contentLength = bytes.byteLength): GetObjectCommandOutput => ({
  ...
  Body: { transformToWebStream: () => new Response(new Uint8Array(bytes)).body! } as NonNullable<
...
describe("S3 object durability transport", () => {
  it("addresses a virtual-host custom endpoint and signs with refreshed session credentials", async () => {
    ...
    try {
      ...
      expect(await Effect.runPromise(store.read("雪 +%", { maxBytes: 1024 }))).toEqual({
    ...
    await withServer(
      ...
      async (endpoint) => {
        ...
        expect(requests.map((request) => request.path)).toEqual(
          Array(3).fill("/durability-test/commits/%E9%9B%AA%20%2B%3F%23%25/literal%252F.json"),
        )
  ...
  it("does not retry a committed PUT whose acknowledgement is lost and permits explicit reconciliation", async () => {
    ...
    await withServer(
      ...
      async (endpoint) => {
        ...
        expect(await Effect.runPromise(store.read("commits/0000", { maxBytes: 1024 }))).toEqual({
          bytes,
          etag: '"committed"',
        })
      },
  ...
  it("distinguishes a missing object from a missing bucket and authorization failure", async () => {
    await withServer(
      ...
      async (endpoint) => {
        ...
        expect((await Effect.runPromise(Effect.flip(store.read("missing-bucket", { maxBytes: 1024 })))).reason).toBe(
          "invalid-response",
        )
        expect((await Effect.runPromise(Effect.flip(store.read("forbidden", { maxBytes: 1024 })))).reason).toBe(
          "authentication",
        )
      },
    )
  ...
  it("classifies a throttled create without retrying or weakening its condition", async () => {
    ...
    await withServer(
      ...
      async (endpoint) => {
        const store = await Effect.runPromise(make({ ...connection, endpoint }))
        expect((await Effect.runPromise(Effect.flip(store.create("commit", new Uint8Array([1]))))).reason).toBe(
          "rate-limit",
        )
  ...
  it("does not mistake a conditional-write/delete race for an existing committed object", async () => {
    await withServer(
      (_request, response) => xmlError(response, 409, "ConditionalRequestConflict"),
      async (endpoint) => {
        const store = await Effect.runPromise(make({ ...connection, endpoint }))
        expect((await Effect.runPromise(Effect.flip(store.create("commit", new Uint8Array([1]))))).reason).toBe(
          "unavailable",
        ...
        await withServer(
          (_request, response) => {
            ...
            response.writeHead(307, {
              location: `${unrelatedEndpoint}/credential-target`,
  ...
  it("rejects incompatible provider or injected-client guarantees during initialization", async () => {
    const badProvider = await Effect.runPromise(
      Effect.flip(
        make({
          ...connection,
    ...
    const missingGuarantees = await Effect.runPromise(
      Effect.flip(
        make({
    ...
    const retryingClient = await Effect.runPromise(
      Effect.flip(
        make({
          ...connection,
          client: injected({
    ...
    const redirectingClient = await Effect.runPromise(
      Effect.flip(
        make({
          ...connection,
          client: injected({
  ...
  it("rejects credential-bearing endpoint URLs without sending a request", async () => {
    const error = await Effect.runPromise(
      Effect.flip(
        make({
          ...connection,
          endpoint: "https://username:password@objects.example.test",
        }),
      ),
    )
    expect(error.reason).toBe("invalid-response")
  ...
  it("rejects truncated bytes instead of accepting a corrupt canonical object", async () => {
    ...
    expect((await Effect.runPromise(Effect.flip(store.read("commit", { maxBytes: 1024 })))).reason).toBe(
      "invalid-response",
    )
  })
  ...
  it("does not treat an unsatisfiable range as a missing object", async () => {
    await withServer(
      (_request, response) => xmlError(response, 416, "InvalidRange"),
      async (endpoint) => {
        const store = await Effect.runPromise(make({ ...connection, endpoint }))
        expect(
          (
    ...
    const store = await Effect.runPromise(
      make({
        ...
        client: injected({
          getObject: async () => {
            reads += 1
            return objectResponse(new Uint8Array())
          },
        }),
      }),
    )
  ...
  it("rejects dot-only path segments rather than reading or writing a normalized sibling key", async () => {
    ...
    expect((await Effect.runPromise(Effect.flip(store.read("commits/../other", { maxBytes: 1024 })))).reason).toBe(
      "invalid-response",
    )
    expect((await Effect.runPromise(Effect.flip(store.create("commits/./other", new Uint8Array())))).reason).toBe(
      "invalid-response",
    )
  })
    ...
    await withServer(
      async (request, response) => {
        if (request.method === "PUT") {
          ...
          if (!deletionAuthorization.includes("Credential=maintenance-access-key/"))
            return xmlError(response, 403, "AccessDenied")
          object = undefined
      ...
      async (endpoint) => {
        ...
        const maintenance = await Effect.runPromise(
          makeMaintenance({
            ...connection,
            endpoint,
            credentials: { ...credentials, accessKeyId: "maintenance-access-key" },
          }),
        )
