...
const scheduleToolCalls = (terminal: "cancelled" | "failed") => {
  ...
  return scopedWith(layer)(
    Effect.gen(function* () {
      ...
      const claim = yield* store.claimExecution({ commandId: `runtime-state-session-terminalization-${terminal}-claim-1`, runId: receipt.runId, ownerId: objectWorkerId })
      ...
      yield* store.startOperation({ commandId: `runtime-state-session-terminalization-${terminal}-startOperation-1`, ...claim, operationId: modelOperation.operationId })
      const calls = [
      ...
      yield* store.startOperation({ commandId: `runtime-state-session-terminalization-${terminal}-startOperation-2`, ...claim, operationId: decoy.operationId })
      ...
      for (const call of calls.slice(0, 3)) {
        ...
        yield* store.startOperation({ commandId: `runtime-state-session-terminalization-${terminal}-startOperation-${operation.operationId}`, ...claim, operationId: operation.operationId })
      ...
      yield* store.expireRunningOperation({ commandId: `runtime-state-session-terminalization-${terminal}-expireRunningOperation-1`, ...claim, operationId: operations[2]!.operationId })
      const terminalFailure = Errors.AgentExecutionFailure.make({ message: "execution failed" })
      if (terminal === "cancelled") {
        yield* runtime.cancel({ commandId: `runtime-state-session-terminalization-${terminal}-cancel-1`, runId: receipt.runId, reason: "user cancelled" })
        yield* runtime.cancel({ commandId: `runtime-state-session-terminalization-${terminal}-cancel-1`, runId: receipt.runId, reason: "user cancelled" })
...
it.effect("rejects successful Run settlement while its Session still has an unresolved tool call", () =>
  scopedWith(objectLayer)(
    Effect.gen(function* () {
      ...
      const claim = yield* store.claimExecution({ commandId: "runtime-state-session-terminalization-test-ts-claim-2", runId: receipt.runId, ownerId: objectWorkerId })
      const session = yield* store.claimedSessionStore(claim)
      if (Option.isNone(session)) return yield* Effect.die("expected Session store")
      const prefix = yield* session.value.append(
      ...
      yield* store.startOperation({ commandId: `runtime-state-session-terminalization-test-ts-startOperation-${operation.operationId}`, ...claim, operationId: operation.operationId })
      yield* store.commitModelResponse({
        ...claim,
