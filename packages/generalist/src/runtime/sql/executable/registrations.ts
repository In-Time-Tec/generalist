  ...
  Effect.gen(function* () {
    ...
    const storedDigest = (pin: string) =>
      sql<RegistrationRow>`
        ...
        FROM generalist_executable_registrations WHERE pin = ${pin}
    ...
    for (const registration of registrations) {
      ...
      const inserted = yield* Effect.exit(sql`
        INSERT INTO generalist_executable_registrations (pin, codec, version, payload_json, registration_digest)
...
export const associateRegistrations: {
  ...
  Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient
    for (const registration of registrations) {
      yield* sql`INSERT INTO generalist_run_registrations (run_id, pin) VALUES (${runId}, ${registration.pin})`
    }
  }),
)
...
export const loadRegistrations = (runId: string) =>
  Effect.gen(function* () {
    ...
    const rows = yield* sql<RegistrationRow>`
      ...
      FROM generalist_runs run
      JOIN generalist_run_registrations link ON link.run_id = run.run_id
      JOIN generalist_executable_registrations registration ON registration.pin = link.pin
      WHERE run.run_id = ${runId}
      ORDER BY registration.pin
    `
    return yield* Effect.forEach(rows, (row) =>
      Effect.gen(function* () {
        const registration = yield* Schema.decodeEffect(Schema.fromJsonString(ExecutableRegistration), {
