...
it.effect("rejects malformed and inactive calls before any handler runs", () => {
  ...
  return Effect.gen(function* () {
    ...
    expect(malformed._tag).toBe("tenetkit/core/InvalidToolCallParameters")
    ...
    expect(inactive).toMatchObject({ _tag: "tenetkit/core/FrameworkFailure", stage: "authorization" })
    expect(handlerCalls).toBe(0)
    ...
    expect(providerExecuted).toMatchObject({ _tag: "tenetkit/core/FrameworkFailure", stage: "authorization" })
    expect(handlerCalls).toBe(0)
...
it.effect("enforces authorization, deadline, and tool-call budget before handler entry", () => {
  ...
  return Effect.gen(function* () {
    ...
    expect(denied).toMatchObject({ _tag: "tenetkit/core/FrameworkFailure", stage: "authorization" })
    expect(handlerCalls).toBe(0)
...
it.effect("resumes exact approval identity and completed replay without duplicating the handler", () => {
  ...
  return Effect.gen(function* () {
    ...
    if (suspension._tag !== "tenetkit/core/AgentSuspended" || waitingCheckpoint === undefined) {
    ...
    expect(mismatched).toMatchObject({ _tag: "tenetkit/core/AgentError" })
    expect(handlerCalls).toBe(0)
