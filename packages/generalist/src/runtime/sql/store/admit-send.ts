    ...
    `
      ...
      if (lane === undefined) {
        yield* sql.onDialectOrElse({
          pg: () => sql`
            INSERT INTO generalist_lanes (session_id, accepted_sequence, queue_json, head_run_id)
            VALUES (${input.message.sessionId}, ${acceptedSequence}, ${encodeQueue(queue)}, ${runId})
          `,
          orElse: () => sql`
            INSERT INTO generalist_lanes (session_id, accepted_sequence, queue_json)
