# Generalist And Rika Linear Durable History Plan

## Purpose

Replace growing durable state copies with one canonical Session message log, small execution references, bounded semantic events, and incremental Rika views. Generalist releases first. Rika then pins the published Generalist release and publishes through its canonical workflow.
...
Rika projection tables and interactive ThreadView state can be deleted and rebuilt from Generalist events plus referenced Session entries. A patch validates its own revision and changed keys; it does not revalidate, copy, map, and sort the full view.
...
- Bump every Runtime store schema version. Old compact-incompatible execution databases fail the existing checksum gate; Rika archives its rebuildable Generalist database through the existing recovery path.
- Keep Rika product Threads, Turns, and transcript projections. They rebuild when Generalist execution state is replaced.
...
6. Release Generalist and verify packages, checksums, isolated Bun/npm installs, restart, child resume, failure settlement, and linear byte growth.
7. Pin the published Generalist train in Rika. Resolve response references in the Generalist adapter.
...
- Rika cursor: terminal product state is published only after matching Generalist terminal status and exact cursor.
...
- Another Rika batching layer: Generalist would still pay the storage and encoding cost before batching.
...
(37.5M uncached input tokens, cacheWrite median 0). Generalist never placed `cache_control` breakpoints,
