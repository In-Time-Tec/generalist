---
name: tenetkit-release
description: Produces and audits TenetKit release assets from one exact detached commit, with checksums and fresh Bun and npm consumers. Use when preparing or recovering a TenetKit release.
---

# TenetKit release proof
...
Set `TENETKIT_RELEASE_COMMIT` to the requested ref or full SHA. Set `TENETKIT_RELEASE_ARTIFACT_DIR` when `release` is not the desired output directory. Run from any checkout of this repository:

```bash
set -euo pipefail

repository="$(git rev-parse --show-toplevel)"
commit="$(git rev-parse --verify "${TENETKIT_RELEASE_COMMIT:-HEAD}^{commit}")"
artifacts="${TENETKIT_RELEASE_ARTIFACT_DIR:-$repository/release}"
...
diff -u \
  <(printf '%s\n' \
    ...
    "tenetkit-${version}.tgz" \
    "tenetkit-cloudflare-${version}.tgz" \
    "tenetkit-mysql-${version}.tgz" \
    "tenetkit-pg-${version}.tgz" | sort) \
...
`bun run test` skips PostgreSQL or MySQL tests when `TENETKIT_DATABASE_URL` or `TENETKIT_MYSQL_URL` is absent. Report either missing variable as a gap; do not call that run full driver proof.
