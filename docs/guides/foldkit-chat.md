...
Restore a conversation when a user reconnects, then show new messages as they arrive. `generalist/unstable/foldkit` adapts committed Session snapshots and `HostEvent` frames into FoldKit's Elm architecture. `Connection.Connection` owns snapshot-first observation and connection facts; `Chat` is a headless submodel with a pure `update` that restores and updates display state. It ships no styled components; rendering stays yours.
