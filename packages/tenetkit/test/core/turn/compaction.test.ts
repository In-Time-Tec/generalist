...
import { Compaction, Session, ToolOutput } from "../../../src/core/index"
...
type ToolResultValue = Prompt.ToolResultPart["result"]
const encodeJsonValue = (value: ToolResultValue): string =>
...
const toolResult = (id: string, result: ToolResultValue): Prompt.Message =>
  Prompt.makeMessage("tool", {
    content: [Prompt.makePart("tool-result", { id, name: "echo", isFailure: false, providerExecuted: false, result })],
  })

const successfulToolResultValue = (prompt: Prompt.Prompt): ToolResultValue => {
  for (const message of prompt.content) {
    const content = Schema.decodeUnknownOption(Schema.Array(Prompt.Part))(message.content)
...
const OutputPaths = Schema.Struct({ outputPaths: Schema.Array(Schema.String) })
const outputPaths = (value: ToolResultValue): ReadonlyArray<string> | undefined => {
  const decoded = Schema.decodeUnknownOption(OutputPaths)(value)
  return Option.isSome(decoded) ? decoded.value.outputPaths : undefined
}
