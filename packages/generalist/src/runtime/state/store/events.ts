...
export const retainedSession = ({ state, sessionId }: { readonly state: RuntimeState; readonly sessionId: string }) => {
...
export const toInspection: {
...
} = Function.dual(2, (state: RuntimeState, run: StoredRun): RunInspection => {
  const optionals = () => {
  ...
  return {
    ...retainedSession({ state, sessionId: run.message.sessionId }),
    runId: run.runId,
    status: run.status,
...
export const inspectRun: {
  (runId: string): (state: RuntimeState) => Effect.Effect<RunInspection, RunNotFound | RuntimeUnavailable>
  (state: RuntimeState, runId: string): Effect.Effect<RunInspection, RunNotFound | RuntimeUnavailable>
} = Function.dual(
  2,
  (state: RuntimeState, runId: string): Effect.Effect<RunInspection, RunNotFound | RuntimeUnavailable> => {
...
export const followTreeChanges: {
  (
    rootRunId: string,
  ): (stateRef: SynchronizedRef.SynchronizedRef<RuntimeState>) => Stream.Stream<void, RunNotFound | RuntimeUnavailable>
  (
    stateRef: SynchronizedRef.SynchronizedRef<RuntimeState>,
    rootRunId: string,
  ): Stream.Stream<void, RunNotFound | RuntimeUnavailable>
} = Function.dual(2, (stateRef: SynchronizedRef.SynchronizedRef<RuntimeState>, rootRunId: string) =>
  Stream.unwrap(
    Effect.gen(function* () {
...
export const shutdownStore = (stateRef: SynchronizedRef.SynchronizedRef<RuntimeState>): Effect.Effect<void> =>
