...
import { RunStore as RunStoreTag } from "../../src/runtime/run/store.js"
...
import type { ExecutionClaim, Service as RunStore } from "../../src/runtime/run/store.js"
...
const openActive = (client: Client, workerId: string) => Effect.gen(function* () {
  const context = yield* Layer.build(Durability.layerRunStore({ ...options, workerId }).pipe(
    Layer.provide(Layer.succeed(ObjectStore, client.store)),
  ))
  yield* Durability.activate.pipe(Effect.provide(context))
  return yield* RunStoreTag.pipe(Effect.provide(context))
})
const admission = (key: string, isProgram = false) => ({
  message: {
    id: `message:${key}`, to: isProgram ? programAddress : address, sessionId: `session:${key}`,
    prompt: Prompt.make("retain accepted evidence"), idempotencyKey: key, correlationId: key, metadata: {},
  },
...
const call = Schema.decodeSync(Response.ToolCallPart("payment", Schema.Unknown))({
  type: "tool-call", id: "payment", name: "payment", params: { amount: 5 }, providerExecuted: false,
})
...
const commitLeaf = (store: RunStore, claim: ExecutionClaim, label: string, parentId: string | null) =>
  Effect.gen(function* () {
    ...
    const operation = yield* store.recordOperation({
      ...claim, operationKey, kind: "model", inputDigest: label, input: { label }, replayPolicy: "never", attempt: 0,
    })
    ...
    const unsigned = {
      operationId: operationKey, turn: 0, modelCallId: `call:${label}`, modelAttemptId: `attempt:${label}`,
      attempt: 0, sessionParentId: parentId, replayFromHistory: false,
      content: Schema.encodeSync(Schema.Array(Response.TextPart))(response.content), finishReason: "stop" as const,
      budgetCharge: 0,
    }
    const completed = yield* store.commitModelResponse({
      ...
      event: {
        _tag: "ModelResponseCommitted", turn: 0, operationKey, modelCallId: `call:${label}`,
        modelAttemptId: `attempt:${label}`, attempt: 0, response, budgetCharge: 0, digest: digest(unsigned),
      },
    })
...
describe("object branch evidence", () => {
  it.effect("A-B-A rewind retains incurred spend and an external receipt without reviving either prior writer", () =>
    Effect.gen(function* () {
      ...
      yield* first.emitAgentEvent({ ...ownerA, commandId: "payment-cost", event: { _tag: "ToolExecutionStarted", turn: 0, call } })
      const payment = yield* first.recordOperation({
        ...ownerA, operationKey: "external-payment", kind: "tool", inputDigest: "payment-5", input: { amount: 5 },
        replayPolicy: "never", attempt: 0,
      })
      yield* first.startOperation({ ...ownerA, operationId: payment.operationId, commandId: "payment-dispatch" })
      yield* first.completeOperation({ ...ownerA, operationId: payment.operationId, outcome: { _tag: "Succeeded", value: { receipt: "paid-5" } } })
      ...
      expect(yield* recovered.getOperation({ runId: run.runId, operationId: payment.operationId })).toMatchObject({
        status: "succeeded", result: { receipt: "paid-5" }, replayPolicy: "never",
      })
      for (const writer of [writerA, writerB]) {
        expect(yield* writer.append({ _tag: "Message", message: Prompt.make("stale").content[0]! }, {
          id: "stale", expectedLeafId: a.leaf,
        }).pipe(Effect.flip)).toMatchObject({ reason: "conflict" })
      }
      ...
      const replay = yield* recovered.startOperation({ ...ownerC, operationId: payment.operationId, commandId: "payment-replay" })
      expect(replay).toMatchObject({ status: "succeeded", result: { receipt: "paid-5" } })
    }).pipe(Effect.scoped, Effect.provide(BunCrypto.layer)),
  )

  it.effect("fork reserves only current capacity and rewind cannot refund its reservation or duplicate inherited costs", () =>
    Effect.gen(function* () {
      const bucket = yield* makeSimulator()
      ...
      expect((yield* history(recovered, run.runId)).filter((event) => event._tag === "RunForked" && event.role === "source")).toHaveLength(1)
    }).pipe(Effect.scoped, Effect.provide(BunCrypto.layer)),
  )

  it.effect("Program forks replay retained outcomes while fresh rewind identities consume the separately reserved grant", () =>
    Effect.gen(function* () {
      const bucket = yield* makeSimulator()
      ...
      const execute = (current: RunStore, runId: string, label: string) => Effect.gen(function* () {
        ...
        const output = yield* runProgram(program, claimed.message.prompt).pipe(Effect.provideService(ProgramRunner, makeProgramRunner({
          claim, claimed, store: current, executor: fixture.executor, handlers: fixture.handlers,
        })))
  ...
  it.effect("forking a settled child reserves from the ancestor that received its unused allowance", () =>
    Effect.gen(function* () {
      ...
      const owner = yield* store.claimExecution({ runId: parent.runId, ownerId: "parent", commandId: "ancestor-claim" })
      const childInput = admission("settled-child")
      const child = yield* store.admitProgramChild({
        ...owner, childRunId: "settled-child", invocationId: "child-invocation",
        message: childInput.message, executableRef: executable.ref, executableManifest: executable.manifest,
        registrations: childInput.registrations,
      })
      const childStore = yield* openActive(yield* bucket.connect, "child")
      const childOwner = yield* childStore.claimExecution({ runId: child.runId, ownerId: "child", commandId: "child-claim" })
      yield* childStore.complete({
        ...childOwner,
        commandId: `${child.runId}:complete`,
        result: { text: "done", output: "done", turns: 0, session: { sessionId: childInput.message.sessionId, leafId: null } },
      })
  ...
  it.effect("settled-child rewind requires a new ancestor grant without erasing the earlier incurred cost", () =>
    Effect.gen(function* () {
      ...
      const owner = yield* store.claimExecution({ runId: parent.runId, ownerId: "parent", commandId: "rewind-parent" })
      const childInput = admission("rewind-child")
      const child = yield* store.admitProgramChild({
        ...owner, childRunId: "rewind-child", invocationId: "rewind-invocation",
        message: childInput.message, executableRef: executable.ref, executableManifest: executable.manifest,
        registrations: childInput.registrations,
      })
      const childStore = yield* openActive(yield* bucket.connect, "child")
      const original = yield* childStore.claimExecution({ runId: child.runId, ownerId: "child", commandId: "rewind-child" })
      yield* childStore.emitAgentEvent({ ...original, commandId: "first-child-cost", event: { _tag: "ToolExecutionStarted", turn: 0, call } })
      yield* childStore.complete({
        ...original, result: { text: "done", output: "done", turns: 0, session: { sessionId: childInput.message.sessionId, leafId: null } },
        commandId: `${child.runId}:complete`,
      })
      yield* store.releaseExecution(owner)
  ...
  it.effect("Program forks share the source concurrency pool across independent owners", () =>
    Effect.gen(function* () {
      ...
      yield* store.reserveProgramOperation({
        ...owner, programPin: program.pinned.pin, budget: program.pinned.manifest.budget,
        operation: "holder", authoredOperation: "holder", kind: "step", capability: "hold",
        inputDigest: "hold", input: null, replay: "idempotent", reservation: { activeSlots: 1 },
      })
      ...
      const target = yield* independent.claimExecution({ runId: "pool-fork", ownerId: "target", commandId: "pool-target" })
      const reservation = {
        ...target, programPin: program.pinned.pin, budget: program.pinned.manifest.budget,
        operation: "future", authoredOperation: "future", kind: "tool" as const, capability: "echo",
        inputDigest: "future", input: "value", replay: "non-idempotent" as const,
        reservation: { toolCalls: 1, activeSlots: 1 },
      }
      expect(yield* independent.reserveProgramOperation(reservation).pipe(Effect.flip)).toMatchObject({ dimension: "concurrency", limit: 1 })
      const replacementStore = yield* openActive(yield* bucket.connect, "replacement")
      const replacement = yield* replacementStore.claimExecution({ runId: run.runId, ownerId: "replacement", commandId: "pool-release" })
      yield* replacementStore.settleProgramOperation({
        ...replacement, operation: "holder", outcome: { _tag: "Succeeded", value: "released" }, releaseSlots: 1,
      })
