...
- Every model stream part that survives `ModelMiddleware` is part of one ordered authoritative response. Baton emits it as `ModelPart { turn, part }`, uses it for text accumulation and tool dispatch, and commits that same transformed sequence to Chat history. Text is accumulated from authoritative `text-delta` parts across all turns into `Completed.text`. Authoritative `finish` parts also flow through as `ModelPart`s. Every transformed `tool-call` ID, including provider-executed calls, must be unique within that response. Baton checks IDs in stream order before exposing each call to events, authorization, dispatch, or history; a duplicate fails with `DuplicateToolCallId` and the zero-based positions of the first and duplicate calls among transformed tool-call parts. Earlier calls may finish once and their completed results are checkpointed before the duplicate failure propagates, while the duplicate and all later parts initiate no work. The registry is response-local, so a later model turn or independent run may reuse an ID.
...
- After all framework calls for a completed turn finish, Baton appends their tool-result parts to Chat exactly once in deterministic call order. That checkpoint precedes Session synchronization, Memory retention, policy evaluation, persisted-chat saving, and `TurnCompleted`. The next model turn reads the results from Chat history and does not append them through its new prompt. A policy stop retains the same checkpoint, and failure or interruption before a successful outcome never manufactures a result.
...
- If `pendingToolResults` is non-empty, the loop evaluates `policy.decide(info)` exactly once after the results are checkpointed in Chat. `Continue` selects `overrides` for the next model call: model layer and active tools affect that call, while instructions are prepended once as a system message to its prompt and then retained by `Ai.Chat` in transcript history. If `Steering` is present, Baton drains steering input after the policy continues, emits `SteeringDrained { turn, queue: "steering", count }`, and uses the drained prompts as new input after the checkpointed tool results. `Stop { reason: TurnLimit }` fails with `TurnLimitExceeded { turn, limit, pending }`; every other `Stop` fails with `TurnPolicyStopped { turn, reason, pending }`. Pending results remain attached as the complete stop checkpoint and are never silently dropped. A failed policy Effect propagates its exact `TurnPolicyError`, and steering does not bypass a stop.
...
- **`DuplicateToolCallId`** carries `{ id, firstIndex, duplicateIndex }` for a model/protocol violation in one authoritative transformed response. Positions are zero-based among transformed `tool-call` parts. It is detected before the duplicate is emitted, authorized, executed, routed, approved, or persisted.
...
**Save points.** Each streamed turn commits its authoritative transformed response to Chat and then saves a persisted Chat when the stream scope closes. Completed framework tool results are checkpointed into Chat and explicitly saved before turn observers and `TurnCompleted`. Baton additionally issues one explicit `chat.save` in two places: (a) after the final turn, before emitting `Completed`; (b) before propagating `AgentSuspended`, so a suspended conversation's history — including the pending transformed tool call and every completed sibling result — survives to the resume. Raw model parts are never saved through a competing Chat stream path. `PersistenceError`/`AiError` from explicit `save` or `getOrCreate` map to `AgentError` with the cause message and `cause` value.
