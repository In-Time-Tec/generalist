...
import { assistantAddress, textPrompt } from "../../tenetkit/test/runtime/helpers.js"
...
const database = postgresDatabase("stranded-delivery")
 ...
 */
describeBackend("postgres stranded delivery", () => {
  layer(database.provision(postgresLayer(database.url)), { excludeTestServices: true })("pending predicate", (it) => {
    it.effect("returns a message to pending when its bound Run dies without consuming it", () =>
      Effect.gen(function* () {
