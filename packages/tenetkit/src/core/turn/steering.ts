import { Effect, Schema, Scope, TxQueue, TxRef } from "effect"
  ...
  Effect.gen(function* () {
    ...
    const observeDrain = (queue: QueueName, drained: Drained) =>
      Effect.annotateCurrentSpan({
        "tenetkit.agent.run_id": runId,
        "tenetkit.agent.inbox.queue": queue,
        "tenetkit.agent.inbox.drained_entries": drained.count,
        "tenetkit.agent.inbox.drained_bytes": drained.bytes,
        "tenetkit.agent.inbox.remaining_entries": drained.remaining,
        "tenetkit.agent.inbox.pending_bytes": drained.pendingBytes,
      }).pipe(
        Effect.andThen(
          drained.firstSequence === undefined
            ? Effect.void
            : Effect.annotateCurrentSpan("tenetkit.agent.inbox.first_sequence", drained.firstSequence),
        ),
        Effect.andThen(
          drained.lastSequence === undefined
            ? Effect.void
            : Effect.annotateCurrentSpan("tenetkit.agent.inbox.last_sequence", drained.lastSequence),
        ),
      )
    ...
    const drain = (queue: QueueName, lane: Lane): Effect.Effect<ReadonlyArray<Input>> =>
      Effect.tx(drainTransaction(lane)).pipe(
        ...
        Effect.withSpan("TenetKit.Agent.inbox.drain", {
          attributes: { "tenetkit.agent.run_id": runId, "tenetkit.agent.inbox.queue": queue },
        }),
      )
    ...
    const close = (reason: "execution-exit" | "scope") =>
      Effect.tx(
        ...
        Effect.tap((closed) =>
          Effect.annotateCurrentSpan({
            "tenetkit.agent.run_id": runId,
            "tenetkit.agent.inbox.close_reason": reason,
            "tenetkit.agent.inbox.already_closed": closed.alreadyClosed,
            "tenetkit.agent.inbox.discarded_steering_entries": closed.steeringEntries,
            "tenetkit.agent.inbox.discarded_follow_up_entries": closed.followUpEntries,
            "tenetkit.agent.inbox.discarded_bytes": closed.bytes,
          }),
        ),
        Effect.asVoid,
        Effect.withSpan("TenetKit.Agent.inbox.close", {
          attributes: { "tenetkit.agent.run_id": runId, "tenetkit.agent.inbox.close_reason": reason },
        }),
      )
    ...
    const offer = (queue: QueueName, input: Input): Effect.Effect<Receipt, InboxFull | RunClosed> => {
      ...
      return Effect.gen(function* () {
        ...
        yield* Effect.annotateCurrentSpan({
          "tenetkit.agent.run_id": runId,
          "tenetkit.agent.inbox.queue": queue,
          "tenetkit.agent.inbox.outcome": outcome._tag.toLowerCase(),
          "tenetkit.agent.inbox.pending_entries": outcome.size,
          "tenetkit.agent.inbox.pending_bytes": outcome.totalBytes,
          "tenetkit.agent.inbox.offered_bytes": bytes,
        })
        ...
        Effect.withSpan("TenetKit.Agent.inbox.offer", {
          attributes: {
            "tenetkit.agent.run_id": runId,
            "tenetkit.agent.inbox.queue": queue,
            "tenetkit.agent.inbox.overflow": lane.policy.onFull,
          },
        }),
      )
    }
    ...
    const complete = Effect.tx(
      ...
      Effect.tap((completion) =>
        completion._tag === "Closed"
          ? Effect.annotateCurrentSpan({
              "tenetkit.agent.run_id": runId,
              "tenetkit.agent.inbox.outcome": "closed",
            })
          : observeDrain(completion.queue, completion).pipe(
              Effect.andThen(
                Effect.annotateCurrentSpan({
                  "tenetkit.agent.inbox.outcome": "pending",
                  "tenetkit.agent.inbox.queue": completion.queue,
                }),
              ),
            ),
      ),
      Effect.withSpan("TenetKit.Agent.inbox.complete", {
        attributes: { "tenetkit.agent.run_id": runId },
      }),
    )
