...
- `@generalist/cloudflare/durable-objects` reconstructs hibernating WebSocket handling from a bounded serialized `{ runId, cursor }` attachment. No resident subscription, fiber, or timer is required. Every host-invoked flush pages authoritative Runtime history strictly after that cursor with bounded page fuel, sends in sequence order, and persists the cursor only after a successful send; malformed attachments and commands close the socket without resetting replay state.
