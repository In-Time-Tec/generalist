...
import {
  ...
  type Request,
  type Result,
} from "../core/program/sandbox-executor.js"

/** @experimental Small provider seam used by the public SandboxExecutor conformance suite. */
export interface Options {
  readonly name: string
  readonly layer: Layer.Layer<SandboxExecutor>
...
const provide = <A, E>(
  options: Options,
  use: (executor: Interface) => Effect.Effect<A, E, Scope.Scope>,
): Effect.Effect<A, E> =>
  Effect.scoped(
    Effect.flatMap(Layer.build(options.layer), (context) =>
      use(Context.get(context, SandboxExecutor)).pipe(Effect.onExit(() => options.assertClean ?? Effect.void)),
    ),
  )
 ...
 */
export const providerConformance = (options: Options): void => {
  interface CircularInput {
    self?: CircularInput
  }

  describe(`${options.name} SandboxExecutor provider conformance`, () => {
