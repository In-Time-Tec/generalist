        ...
        ModelMiddleware.layer([
          {
            transformPrompt: (prompt) =>
              Effect.sync(() => {
                ...
                const marker = recalled?.options["generalist/memory"]
                if (recalled !== undefined && authored !== undefined) {
                  Reflect.deleteProperty(recalled.options, "generalist/memory")
                  Object.assign(authored.options, { "generalist/memory": marker })
                }
                return prompt
              }),
          },
        ...
        Compaction.layerTest({
          maybeCompact: (request) =>
            Effect.sync(() => {
              ...
              if (recalled !== undefined) Reflect.deleteProperty(recalled.options, "generalist/memory")
              return Option.some({
