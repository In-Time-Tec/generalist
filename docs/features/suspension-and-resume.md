...
Every response, approval, signal, timer, child, external child, fan-out, or cancellation close must prove one `open -> terminal` transition before appending its durable event. An identical duplicate is read-only success; a conflicting duplicate is typed failure; terminal history cannot reopen. A caller-issued close that finds no open wait fails typed and stays uncommitted, so an exact retry under the same caller identity can still apply once the wait exists—`signal` fails `WaitNotOpen` for a name that is not open. Runtime removes suspension only in the durable transition that commits completion, failure, cancellation, pending steering, or another pending terminal outcome.
