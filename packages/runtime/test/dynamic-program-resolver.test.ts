...
import { Errors, ExecutableRegistration, ExecutableResolver } from "../src/index.js"
...
const fixture = (options: { readonly revision?: string; readonly toolName?: string } = {}) => {
  ...
  const check = (
    ...
    request.registration.codec !== "test" || request.registration.version !== "1"
      ? Effect.fail(
          invalid(`unsupported registration codec ${request.registration.codec}@${request.registration.version}`),
        )
      : (request.registration.payload as { readonly fixture: string }).fixture !== revision
  ...
  const reconstruction: ExecutableResolver.ProgramReconstruction = {
    ...
    codec: (request) =>
      check(request).pipe(
        Effect.as(request.boundary === "input" ? (Prompt.Prompt as Schema.Codec<any, any>) : Schema.String),
      ),
    tool: (request) =>
      check(request).pipe(
        Effect.as(
    ...
    services: (request) =>
      Effect.acquireRelease(Effect.succeed(Layer.empty as Layer.Layer<any>), () =>
...
describe("ExecutableResolver.makeDynamic", () => {
  ...
  it.effect("fails typed when the persisted set carries a registration the executable does not require", () =>
    Effect.gen(function* () {
      ...
      expect(failure).toMatchObject({ _tag: "@batonfx/runtime/ExecutableRegistrationInvalid" })
      expect(String((failure as { readonly message: string }).message)).toMatch(/not required by executable/)
    }),
  )
  ...
  it.effect("fails typed on an unsupported registration codec version", () =>
    Effect.gen(function* () {
      ...
      expect(failure).toMatchObject({ _tag: "@batonfx/runtime/ExecutableRegistrationInvalid" })
      expect(String((failure as { readonly message: string }).message)).toMatch(/unsupported registration codec/)
    }),
  )
  ...
  it.effect("fails typed when a persisted registration payload changed", () =>
    Effect.gen(function* () {
      ...
      expect(failure).toMatchObject({ _tag: "@batonfx/runtime/ExecutableRegistrationInvalid" })
      expect(String((failure as { readonly message: string }).message)).toMatch(/registration payload changed/)
    }),
  )
  ...
  it.effect("fails typed when a reconstructed binding leaves the admitted manifest closure", () =>
    Effect.gen(function* () {
      ...
      expect(failure).toMatchObject({ _tag: "@batonfx/runtime/ExecutableRegistrationInvalid" })
      expect(String((failure as { readonly message: string }).message)).toMatch(/binding/)
    }),
  )
  ...
  it.effect("fails typed instead of throwing when the reference is not owned by the manifest", () =>
    Effect.gen(function* () {
      ...
      expect(failure).toMatchObject({ _tag: "@batonfx/runtime/ExecutablePinMissing", ref })
    }),
  )
})
