...
- the loop seams: `ToolExecutor`, `ToolContext`, `ToolOutputStore`, `Permissions`, `Steering`, `Compaction`, `ModelResilience`, `Approvals`, and `TurnPolicy` (a plain value, not a service);
...
Baton does not own (deferred, see ADR-0001): UI helpers, generic memory abstractions, evals, model-judge guardrails or detection heuristics, multi-agent/handoffs, durability of any kind. Baton owns ergonomic **Guardrail** combinators over `ModelMiddleware`, but no separate guardrail subsystem. Baton owns a standalone **Instructions** registry and **Session** event-log seam, but no filesystem context loaders, memory sources, skills catalog source, or durable/addressable session implementation. Baton owns a **chat persistence seam** (`RunOptions.persistence`, see below) but no persistence _implementation_ — consumers provide upstream `Chat.Persistence` layers. Baton also owns a non-durable **tool output spill seam** (`ToolOutputStore`, see below); durable blob stores remain host-side.
