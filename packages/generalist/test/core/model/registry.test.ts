...
describe("ModelRegistry", () => {
  it("infers requirement-free data-first, data-last, and empty registry layers", () => {
    ...
    const inference: Assert<Equal<typeof dataFirst, Layer.Layer<ModelRegistry.ModelRegistry>>> = true
    ...
    () =>
      [
        ...
        Effect.gen(function* () {
          ...
          expect(failure._tag).toBe("generalist/core/LanguageModelNotRegistered")
          if (failure._tag === "generalist/core/LanguageModelNotRegistered") {
  ...
  ItLayer.make(it, "attaches a registration tool compiler to only the active model", () => {
    const compiler: ModelRegistry.ToolJsonSchemaCompiler = (tool) =>
      Schema.decodeUnknownEffect(Schema.String)(tool.name).pipe(
