  ...
  it.effect("fails typed and closes the connection when discovery schemas contain non-JSON values", () =>
    Effect.gen(function* () {
      ...
      for (const malformedDiscoverySchema of schemas) {
        ...
        expect(error).toBeInstanceOf(MCPClient.MCPConnectionFailed)
        expect(error._tag).toBe("generalist/mcp/MCPConnectionFailed")
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        for (const error of errors) {
          ...
          expect(error._tag).toBe("generalist/mcp/MCPToolCallFailed")
          expect(error.server).toBe("calc")
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        if (Option.isSome(outcome)) {
          expect(outcome.value._tag).toBe("generalist/mcp/MCPToolCallFailed")
