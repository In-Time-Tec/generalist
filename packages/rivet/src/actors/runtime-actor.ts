...
const SendInput = Schema.Struct({
  ...
  to: Address.Address,
  from: Schema.optionalKey(Address.Address),
...
const actionInputSchemas = {
  runtime: {
    ...
    resolveOperation: Schema.toStandardSchemaV1(Schema.Tuple([OperationResolution.ResolveOperationInput])),
...
type RuntimeActions = {
  readonly runtime: {
    ...
    ) => Promise<Effect.Success<ReturnType<Runtime.Service["send"]>>>
    readonly signal: (c: RuntimeActionContext, input: Runtime.SignalInput) => Promise<void>
    readonly respond: (c: RuntimeActionContext, input: Runtime.RespondInput) => Promise<void>
    readonly cancel: (c: RuntimeActionContext, input: Runtime.CancelInput) => Promise<void>
    readonly resolveOperation: (
      c: RuntimeActionContext,
      input: OperationResolution.ResolveOperationInput,
    ) => Promise<void>
    readonly inspect: (
      c: RuntimeActionContext,
      runId: string,
    ) => Promise<Effect.Success<ReturnType<Runtime.Service["inspect"]>>>
...
const runAction = async <A, E>(c: Context, effect: (runtime: Runtime.Service) => Effect.Effect<A, E>): Promise<A> => {
  const host = requireHost(c)
  const result = await c.keepAwake(
    host.runtime.runPromise(Effect.flatMap(Runtime.Runtime, effect), { signal: c.abortSignal }),
  )
  await arm(c)
  return result
}
...
const makeRuntimeOwner = (actorId: string) =>
  Effect.gen(function* () {
    ...
    if (incarnation === undefined) {
      return yield* Errors.RuntimeUnavailable.make({ message: "Rivet actor incarnation allocation returned no row" })
    }
...
export const makeRuntimeActor = (options: RuntimeActorOptions): RuntimeActorDefinition => {
  ...
  return actor({
    ...
    actions: {
      runtime: {
        send: (c, input: Runtime.SendInput) => runAction(c, (runtime) => runtime.send(input)),
        signal: (c, input: Runtime.SignalInput) => runAction(c, (runtime) => runtime.signal(input)),
        respond: (c, input: Runtime.RespondInput) => runAction(c, (runtime) => runtime.respond(input)),
        cancel: (c, input: Runtime.CancelInput) => runAction(c, (runtime) => runtime.cancel(input)),
        resolveOperation: (c, input: OperationResolution.ResolveOperationInput) =>
          runAction(c, (runtime) => runtime.resolveOperation(input)),
        inspect: (c, runId: string) => {
          const host = requireHost(c)
          return c.keepAwake(
            host.runtime.runPromise(
              Effect.flatMap(Runtime.Runtime, (runtime) => runtime.inspect(runId)),
              {
                signal: c.abortSignal,
              },
            ),
