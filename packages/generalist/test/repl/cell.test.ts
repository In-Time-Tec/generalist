...
describe("Cell failure taxonomy", () => {
  ...
  it("tags every failure under the package scope", () => {
    for (const tag of Cell.failureTags) expect(tag.startsWith("generalist/repl/")).toBe(true)
  })

  it("rejects an unknown failure tag", () => {
    expect(() => Schema.decodeUnknownSync(Cell.CellFailure)({ _tag: "generalist/repl/Nope", message: "x" })).toThrow()
  })
