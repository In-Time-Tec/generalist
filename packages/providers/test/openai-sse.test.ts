...
import { normalizeResponsesSse, withOpenAi } from "@batonfx/providers/openai"
...
describe("withOpenAi stream error normalization", () => {
  it.effect("surfaces a nested server_error as a decoded error part instead of a schema failure", () =>
    Effect.gen(function* () {
      ...
      const parts = yield* ModelRegistry.stream(
        ...
        Stream.provide(
          withOpenAi({ model: "gpt-test", apiKey: Config.succeed(Redacted.make("test-key")) }).pipe(
