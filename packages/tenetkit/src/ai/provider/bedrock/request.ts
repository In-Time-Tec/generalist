...
const fileBlock = Effect.fnUntraced(function* (part: {
  ...
  if (documentType === undefined) return yield* fail(`unsupported file MIME type: ${part.mediaType}`)
...
const partBlocks = Effect.fnUntraced(function* (part: PromptPart, role: "user" | "assistant", finalPrefill: boolean) {
  ...
  if (part.type === "text") {
    ...
    if (role === "assistant") return yield* fail("Bedrock Converse accepts files only in user messages")
    content.push(
      yield* fileBlock({
