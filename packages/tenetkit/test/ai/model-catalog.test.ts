...
describe("ModelCatalog", () => {
  testLayer(ModelCatalog.layer())((test) => {
    test.effect("requires a known bundled model", () =>
      Effect.gen(function* () {
        const metadata = yield* ModelCatalog.get({ provider: "openai", model: "gpt-4o-mini" })
    ...
    test.effect("fails typed when required metadata is missing", () =>
      Effect.gen(function* () {
        const failure = yield* Effect.flip(ModelCatalog.get({ provider: "missing", model: "none" }))
    ...
    test.effect("returns undefined from find when metadata is missing", () =>
      Effect.gen(function* () {
        const metadata = yield* ModelCatalog.find({ provider: "missing", model: "none" })
  ...
  const override: ModelCatalog.Metadata = {
    provider: "openai",
    model: "gpt-4o-mini",
    contextWindow: 42,
    maxOutput: 7,
  ...
  testLayer(ModelCatalog.layer([override]))((test) => {
    test.effect("lets overrides shadow bundled metadata by provider and model", () =>
      Effect.gen(function* () {
        const metadata = yield* ModelCatalog.get({ provider: "openai", model: "gpt-4o-mini" })
        const all = yield* ModelCatalog.list()
