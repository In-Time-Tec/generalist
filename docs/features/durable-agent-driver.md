...
- `make` creates a root budget; `reserveChild` consumes one child slot and a grant, `narrowChild` refunds the reduction, `charge` deducts use, and `refundUnused` returns remaining child capacity. Malformed grants and widening fail `RunBudgetInvalid`; schedule-boundary exhaustion is `Exhausted`, never mid-provider-stream.
- Inline `AgentTool.asTool` and `delegateTool` nesting reserves and refunds child budget through the active driver. Runtime child/group/spawn admission still follows root-pinned `TreePolicy`; typed `AgentTool.fanOut` additionally reserves journal-backed shares from the parent Runtime budget and refunds unused capacity when each member settles.
