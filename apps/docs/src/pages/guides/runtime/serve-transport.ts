...
export const serveTransport = definePage({
  ...
  content: [
    ...
    p(
      ...
      ". ",
      code("Runtime.start"),
      " takes the registered Agent value and typed input, then returns a handle containing the stable ",
      code("runId"),
      ", typed ",
      code("await"),
      ", and replay-then-live ",
      code("events"),
      ". ",
      code("Runtime.events"),
      " also replays semantic and lifecycle events after an optional exclusive cursor, then follows live events. Provider fragments never enter this durable stream.",
    ),
