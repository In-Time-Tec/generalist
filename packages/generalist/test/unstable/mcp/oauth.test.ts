...
import { layer as layerMcpHttpClient } from "../../../src/unstable/mcp/client/http.js"
...
const configuration: OAuth.Configuration = {
  ...
  clientMetadata: {
    client_name: "Generalist OAuth test",
    redirect_uris: ["https://app.example/oauth/callback"],
  },
  clientInformation: { client_id: "generalist-test" },
}
...
describe("OAuth", () => {
  ...
  it("exposes typed denied, expired, pending, and provider errors", () => {
    expect(OAuth.OAuthDenied.make({ reason: "access_denied" })._tag).toBe("generalist/mcp/OAuthDenied")
    expect(OAuth.OAuthExpired.make({ server: "server" })._tag).toBe("generalist/mcp/OAuthExpired")
    expect(OAuth.OAuthPending.make({ authorizationUrl: "https://auth.example" })._tag).toBe(
      "generalist/mcp/OAuthPending",
    )
    const provider = OAuth.OAuthProviderError.make({ server: "server", operation: "refresh", message: "failed" })
    expect(provider._tag).toBe("generalist/mcp/OAuthProviderError")
