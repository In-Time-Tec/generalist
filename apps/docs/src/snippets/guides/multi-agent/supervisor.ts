...
const modelLayer = Layer.effect(
  ...
  Ai.LanguageModel.make({
    ...
    streamText: (options) => {
      ...
      if (isChildTurn) {
        return Stream.make(
          Ai.Response.makePart("text-delta", { id: "assistant", delta: "Refund for order 42 issued." }),
        )
      }
      supervisorCalls += 1
      return supervisorCalls === 1
        ...
        : Stream.make(
            Ai.Response.makePart("text-delta", { id: "assistant", delta: "Billing handled it: refund issued." }),
          )
    },
  }),
)
