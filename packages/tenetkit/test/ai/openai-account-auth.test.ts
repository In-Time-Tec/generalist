...
import { credentialsFromAccountAuth } from "../../src/ai/provider/openai.js"
import {
  AuthError,
  authorizationUrl,
  clientId,
...
const digest = (_algorithm: string, data: Uint8Array) =>
  Effect.promise(() =>
    globalThis.crypto.subtle.digest("SHA-256", data as Uint8Array<ArrayBuffer>).then((value) => new Uint8Array(value)),
  )
...
describe("OpenAI account authorization protocol", () => {
  it.effect("creates exact deterministic PKCE values", () =>
    Effect.gen(function* () {
      ...
      expect(String(pkce.state)).not.toContain(Redacted.value(pkce.state))
    }).pipe(
