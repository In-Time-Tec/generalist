...
export const lockRun = (runId: string) =>
  Effect.gen(function* () {
    ...
    yield* sql.onDialectOrElse({
      pg: () => sql`SELECT run_id FROM generalist_runs WHERE run_id = ${runId} FOR UPDATE`,
      mysql: () => sql`SELECT run_id FROM generalist_runs WHERE run_id = ${runId} FOR UPDATE`,
      orElse: () => Effect.void,
    })
  })
...
export const loadRun = (runId: string) =>
  Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient
    const rows = yield* sql<RunRow>`SELECT * FROM generalist_runs WHERE run_id = ${runId}`
    const row = rows[0]
...
export function loadEventsAfter(...args: [string, number] | [number]) {
  ...
  return Effect.gen(function* () {
    ...
    const rows = yield* sql<EventRow>`
      SELECT * FROM generalist_run_events
...
export function loadRunWait(...args: [string, string?]) {
  ...
  return Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient
    const rows =
      yield* sql<WaitRow>`SELECT * FROM generalist_run_waits WHERE run_id = ${runId} AND wait_id = ${waitId!}`
    const row = rows[0]
...
export const loadRunWaits = (runId: string): Effect.Effect<ReadonlyArray<RunWait>, SqlError, SqlClient.SqlClient> =>
  Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient
    const rows =
      yield* sql<WaitRow>`SELECT * FROM generalist_run_waits WHERE run_id = ${runId} ORDER BY authored_order ASC, wait_id ASC`
...
export const loadRunWaitsByStatus: {
  ...
  Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient
    const rows =
      yield* sql<WaitRow>`SELECT * FROM generalist_run_waits WHERE run_id = ${runId} AND status = ${status} ORDER BY authored_order ASC, wait_id ASC`
    return yield* Effect.forEach(rows, (row) => loadRunWait(runId, row.wait_id)).pipe(
      Effect.map((waits) => waits.filter((wait): wait is RunWait => wait !== undefined)),
    )
  }),
...
export function promoteHead(...args: [EventHub, string] | [string]) {
  ...
  return Effect.gen(function* () {
    ...
    const lanes = yield* sql<{ queue_json: string }>`
      SELECT queue_json FROM generalist_lanes WHERE session_id = ${sessionId}
    `
    const lane = lanes[0]
    ...
    yield* sql`UPDATE generalist_runs SET attempt_fence = ${attempt} WHERE run_id = ${headId} AND attempt_fence = ${head.attemptFence}`
...
export function removeFromLane(...args: [string, string] | [string]) {
  ...
  return Effect.gen(function* () {
    ...
    const lanes = yield* sql<{ queue_json: string; accepted_sequence: number }>`
      SELECT queue_json, accepted_sequence FROM generalist_lanes WHERE session_id = ${sessionId}
    `
    const lane = lanes[0]
    if (lane === undefined) return
    const queue = decodeQueue(lane.queue_json).filter((id) => id !== runId)
    if (queue.length === 0) {
      yield* sql`DELETE FROM generalist_lanes WHERE session_id = ${sessionId}`
    } else {
      yield* sql.onDialectOrElse({
        pg: () => sql`
          UPDATE generalist_lanes SET queue_json = ${encodeQueue(queue)}, head_run_id = ${queue[0]!}
          WHERE session_id = ${sessionId}
        `,
        orElse: () => sql`
          UPDATE generalist_lanes SET queue_json = ${encodeQueue(queue)}
          WHERE session_id = ${sessionId}
        `,
      })
    }
...
export const settleParent: {
  ...
  Effect.gen(function* () {
    ...
    const existing = yield* sql<{ child_run_id: string }>`
      SELECT child_run_id FROM generalist_run_links
    ...
    if (isQueuedRun(currentParent) && !(yield* hasUnsettledChild(parent.runId))) {
      ...
      yield* sql`UPDATE generalist_runs SET attempt_fence = ${attempt} WHERE run_id = ${parent.runId}`
    ...
    const running = yield* sql<{ fan_out_id: string }>`
      SELECT fan_out_id FROM generalist_fan_outs WHERE parent_run_id = ${parent.runId} AND status = 'running' LIMIT 1
    `
...
export const insertRun = (input: {
  ...
  Effect.gen(function* () {
    ...
    if (input.runId === input.rootRunId) {
      yield* sql`INSERT INTO generalist_tree_roots (root_run_id) VALUES (${input.runId})`
    }
  })
