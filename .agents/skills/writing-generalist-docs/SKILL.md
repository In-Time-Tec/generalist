...
Run `bun run docs:build` and formatting checks for changed paths. The site is the Foldkit + StyleX app in `docs/`: `bun run dev` serves it locally and `docs/src/content.ts` is the content model. For rendered content or navigation, inspect the dev server output including desktop/mobile light/dark screenshots plus keyboard navigation and code-copy behavior. Report checks and limitations without creating a second documentation toolchain.
