...
export const seamsAsServices = definePage({
  ...
  content: [
    ...
    p(
      "Because every seam is a service, a host can replace any of them without core importing host code. An in-process CLI provides a model and the handler layers for its local tools. @batonfx/runtime provides the same model/tool surface plus durable implementations of execution concerns (a database-backed ",
      code("SessionStore"),
      ", a blob-backed ",
      code("ToolOutputStore"),
      ", persisted RunEvents, wait-backed approvals) and the loop cannot tell the difference. That native composition is the subject of ",
