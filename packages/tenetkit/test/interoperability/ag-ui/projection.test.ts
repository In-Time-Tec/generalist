...
import { Errors, ExecutableManifest, RunEvent } from "tenetkit/runtime"
import { project, projectModelResponse, stateSnapshot } from "../../../src/interoperability/ag-ui/projection.js"
  ...
  it.effect("projects every committed semantic text, reasoning, and tool unit with deterministic lifecycles", () =>
    Effect.gen(function* () {
      ...
      expect(events).toEqual([
        ...
        { type: "TOOL_CALL_ARGS", toolCallId: "tool-1", delta: '{"q":"tenetkit"}' },
  ...
  it.effect("rejects removed transport fragments, malformed Runtime events, and validates snapshots", () =>
    Effect.gen(function* () {
      ...
      expect(fragmentFailure).toMatchObject({
        _tag: "tenetkit/ag-ui/EventInvalid",
        source: "runtime",
      })

      const failure = yield* project({ _tag: "RunCompleted" }, "thread-1").pipe(Effect.flip)
      expect(failure._tag).toBe("tenetkit/ag-ui/EventInvalid")
