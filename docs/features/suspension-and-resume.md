...
Baton stores the suspension token and authorization snapshot on the unresolved transformed call in Chat. Resume first derives the sole unresolved checkpoint and compares token, reason, call identity, parameters, active tools, and activated skills. Missing, stale, changed, fabricated, or repeated resumes fail with `ResumeMismatch` before skill loading, authorization, execution, model work, or persistence.
