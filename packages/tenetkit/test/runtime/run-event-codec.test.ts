...
import { ProgramCapabilities, ProgramHost, SandboxExecutor } from "tenetkit"
...
import { decodeEvent, encodeEvent } from "../../src/runtime/sql/codecs.js"
import {
  alternateAssistant,
...
it("rejects corrupted event payloads without fallback parsing", () => {
  ...
  expect(() =>
    decodeEvent(
      Schema.encodeSync(EventRecord)({ ...encoded, error: { _tag: "tenetkit/runtime/AgentExecutionFailure" } }),
    ),
  ).toThrow()
...
it.live("keeps memory and SQLite failure history and inspection in parity", () => {
  const failure = failures.find((candidate) => candidate._tag === "tenetkit/core/ProgramOperationUnknown")!
