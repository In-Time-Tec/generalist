...
A cell's namespace is not durable state. Baton operations, events, Session entries, and children remain the only truth, and nothing in `@batonfx/repl` is consulted to reconstruct a run.
...
This is why the pool holds no authority beyond the current epoch, and why `@batonfx/repl`'s root export has no process dependencies at all: a projection, a decoder, or a test host reads the contract without ever touching a worker.
