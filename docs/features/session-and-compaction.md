...
- An explicit `RunOptions.compaction.contextWindow` wins. Otherwise compaction resolves the active provider/model through `ModelCatalog`; bundled models need no catalog Layer, provided catalog overrides remain authoritative, and known but unlisted models use a conservative 32,768-token window after one warning. Identity-less custom `LanguageModel` services leave fallback ownership with the Compaction Layer. A configured `reserveTokens` at or above the resolved window is rejected, and an omitted reserve falls back to zero when the default 16,384-token headroom would exceed the window.
