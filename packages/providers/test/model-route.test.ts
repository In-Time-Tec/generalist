...
const isUnavailableWithUsage = (error: unknown): boolean =>
  AiError.isAiError(error) &&
  error.reason._tag === "InvalidOutputError" &&
  error.reason.description === "provider unavailable after accepting input"

const model = (streamText: () => Stream.Stream<Response.AnyPart, AiError.AiError>): LanguageModel.Service =>
  ({
    generateText: () => Effect.die("unexpected generateText"),
    generateObject: () => Effect.die("unexpected generateObject"),
    streamText,
  }) as LanguageModel.Service

const registration = (
  provider: string,
  streamText: () => Stream.Stream<Response.AnyPart, AiError.AiError>,
  calls: Array<string>,
