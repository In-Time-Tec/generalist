...
export const reconcileCancellationRequested = Effect.gen(function* () {
  ...
  yield* sql`UPDATE generalist_runs SET status = 'cancelling'
    WHERE ${cancellationRequested} AND status IN ('queued', 'running', 'waiting')`
})
...
export const sessionRoots = (sessionId: string) =>
  Effect.gen(function* () {
    ...
    const roots = yield* sql<{ run_id: string }>`
      SELECT run_id FROM generalist_runs WHERE root_run_id = run_id AND session_id = ${sessionId}
...
export const sessionRuns = (sessionId: string) =>
  Effect.gen(function* () {
    ...
    const runs = yield* sql<{ run_id: string }>`
      SELECT r.run_id FROM generalist_runs r
      JOIN generalist_runs root ON root.run_id = r.root_run_id
