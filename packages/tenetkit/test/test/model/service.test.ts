...
class Fixture extends Context.Service<Fixture, TestModel.Fixture>()("tenetkit/test/test/model/service.test/Fixture") {}
...
const jsonString = <A>(value: A) => Schema.encodeEffect(Schema.fromJsonString(Schema.Unknown))(value)

const eventTag = <A>(event: A): string => Schema.decodeUnknownSync(Schema.Struct({ _tag: Schema.String }))(event)._tag
...
layer(
  ...
  it.effect("runs the script and captures normalized prompts", () =>
    Effect.gen(function* () {
      ...
      expect(requests[0]?.tools.map((tool) => Schema.decodeUnknownSync(Schema.String)(tool.name))).toContain("echo")
    }),
  )
})
    ...
    Effect.gen(function* () {
      ...
      expect(() =>
        Schema.decodeSync(Schema.fromJsonString(Schema.Unknown))(
          delta?.type === "tool-params-delta" ? delta.delta : "",
        ),
      ).toThrow()
    }),
  )
...
layer(Layer.empty)("Agent runs over a truncated provider stream", (it) => {
  ...
  it.effect("fails the run when the stream is cut mid-text", () =>
    Effect.gen(function* () {
      ...
      expect(events.some((event) => eventTag(event) === "Completed")).toBe(false)
    }),
  )
  ...
  it.effect("fails the run when a tool call is cut mid-parameters and never runs the tool", () =>
    Effect.gen(function* () {
      ...
      expect(Exit.isFailure(exit)).toBe(true)
      expect(events.some((event) => eventTag(event) === "ToolExecutionStarted")).toBe(false)
      expect(events.some((event) => eventTag(event) === "Completed")).toBe(false)
    }),
  )
  ...
  it.effect("keeps fixture state across model registry resolution and layer rebuilds", () =>
    Effect.gen(function* () {
      ...
      const call = (prompt: string) =>
        ModelRegistry.withModel(fixture.selection, LanguageModel.generateText({ prompt })).pipe(
          Effect.provide(services),
        )
