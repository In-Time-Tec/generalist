...
`admissionOf` decodes tool call, key, and optional origin; `originOf` returns only the origin. Both origin fields travel in the invocation ID already recorded by `ChildLinked` and canonical child-tree events. Every string round-trips: an empty operation key encodes as `#<ordinal>` without losing the origin, an unpaired UTF-16 surrogate uses a reversible `%uXXXX` escape that percent-encoded data can never contain, and malformed percent escapes or non-canonical ordinal markers read back as `undefined` instead of raising.
...
- String fields in the invocation ID are percent-encoded; an empty operation key still carries its origin, an unpaired surrogate uses a reversible `%uXXXX` escape, and malformed percent escapes or non-canonical ordinal markers read as `undefined` rather than raising.
