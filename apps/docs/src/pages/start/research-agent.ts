...
export const researchAgent = Prose.definePage({
  ...
  content: [
    ...
    Prose.p(
      ...
      " resolves, the layer is a real OpenRouter model. When it does not, a scripted model stands in: it reads the growing prompt — no prior ",
    ...
    Prose.p(
      ...
      Prose.code("Pending"),
      " with a token — approval-gated tools park the run until a human resolves it. The routes serve the registry over HTTP: ",
      Prose.code("GET /ws"),
      " for WebSocket, ",
    ...
    Prose.p(
      ...
      " — the same wire frame Part 2 sent by hand.",
    ),
