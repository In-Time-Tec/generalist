...
import { SessionStorage } from "../../../../src/runtime/sql/session/storage.js"
  ...
  return Effect.gen(function* () {
    ...
    const payloads = database
      .query<
        { payload_json: string },
        []
      >("SELECT payload_json FROM generalist_session_entries WHERE tag = 'ModelResponse'")
      .all()
    const modelOperations = database
      .query<
        { result_json: string | null },
        []
      >("SELECT result_json FROM generalist_run_operations WHERE kind = 'model'")
      .all()
    const committedEvents = database
      .query<
        { event_json: string },
        []
      >("SELECT event_json FROM generalist_run_events WHERE event_json LIKE '%ModelResponseCommitted%'")
      .all()
    database.close()
