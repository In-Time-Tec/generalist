...
const makeRuntimeWith = (
  ...
  Effect.gen(function* () {
    ...
    const normalizeBudgetDelta = (delta: Parameters<RuntimeService["extendBudget"]>[0]["delta"]) =>
      Effect.try({
    ...
    const extendRunBudget: RuntimeService["extendBudget"] = (input) =>
      normalizeBudgetDelta(input.delta).pipe(Effect.flatMap((delta) => store.extendBudget({ ...input, delta })))
    const operatorRuns = Stream.paginate<string | undefined, RunInspection, RuntimeUnavailable | DurabilityFailure>(
      undefined,
      (afterRunId) =>
        store
          .list({
    ...
    const operator: RuntimeService["operator"] = {
      ...
      resolveUnknown: (runId, operationId, resolution, operatorIdentity, commandId) =>
      ...
      extendBudget: (runId, delta, operatorIdentity, commandId) =>
        Effect.gen(function* () {
          ...
          yield* store.extendBudgetRecovery({ runId, delta: normalized, operator: operatorIdentity, commandId })
        }),
    }
    ...
    const service: RuntimeService = {
      ...
      fork: (runId, input) =>
        Effect.gen(function* () {
          const newRunId = `run_${digest(["fork", runId, input.commandId])}`
          const receipt = yield* store.fork({ runId, newRunId, ...input })
          yield* store.activate({ runId: receipt.runId, commandId: `fork:${input.commandId}:activate` })
      ...
      rewind: (runId, input) =>
        Effect.gen(function* () {
          ...
          yield* store.activate({ runId, commandId: `rewind:${input.commandId}:activate` })
        }),
      fanOut: (input) =>
        Effect.gen(function* () {
