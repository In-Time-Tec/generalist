...
const scopedWith =
  <A, E, R0>(layer: Layer.Layer<A, E, R0>) =>
  <B, E2, R extends A | Scope.Scope | R0>(effect: Effect.Effect<B, E2, R>) =>
  ...
  return Effect.gen(function* () {
    ...
    yield* withObject(
      ...
      Effect.gen(function* () {
        ...
        expect(
          yield* Schema.encodeEffect(Schema.fromJsonString(Schema.Unknown))(entry).pipe(Effect.orDie),
        ).not.toContain("redaction-canary")
      }),
    )
  })
})
  ...
  return Effect.gen(function* () {
    ...
    yield* scopedWith(runtimeLayer)(
      Effect.gen(function* () {
        ...
        const encodedEntry = yield* Schema.encodeEffect(Schema.fromJsonString(Schema.Unknown))(entry).pipe(Effect.orDie)
        expect(encodedEntry).toContain("safe answer")
        expect(encodedEntry).not.toContain("persistence-canary")
        expect(encodedEntry).not.toContain("authorization")
        expect(encodedEntry).not.toContain("set-cookie")
