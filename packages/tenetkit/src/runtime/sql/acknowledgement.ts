...
export const acknowledge = (input: {
  ...
  Effect.gen(function* () {
    ...
    const boundary =
      input.sequence === -1
        ...
        : (yield* sql<{ event_json: string }>`
              SELECT event_json FROM tenetkit_run_events
    ...
    const rows = yield* sql<{ sequence: number | string }>`
      SELECT sequence FROM tenetkit_run_acknowledgements WHERE run_id = ${input.runId}
    `
    ...
    if (current === undefined) {
      yield* sql`
        INSERT INTO tenetkit_run_acknowledgements (run_id, sequence, acknowledged_at)
        VALUES (${input.runId}, ${input.sequence}, ${acknowledgedAt})
      `
      return
    }
    yield* sql`
      UPDATE tenetkit_run_acknowledgements
...
export const loadAcknowledged = (
  ...
  Effect.gen(function* () {
    ...
    const rows =
      yield* sql<AckRow>`SELECT sequence, acknowledged_at FROM tenetkit_run_acknowledgements WHERE run_id = ${runId}`
    const row = rows[0]
