...
import { Agent, ExecutableManifest, RunBudget, ToolExecutor } from "@batonfx/core"
import { Address, ExecutionHost, ExecutableResolver, Runtime, RunStore } from "../src/index.js"
import { defaultTreePolicy, TREE_POLICY_MAX } from "../src/tree-policy.js"
...
it("an unspecified agent budget is unbounded rather than a runtime-invented ceiling", () => {
  ...
  for (const dimension of ["modelCalls", "toolCalls", "totalTokens", "childRuns", "handoffs", "depth", "deadline"])
    expect((RunBudget.unbounded as Record<string, unknown>)[dimension]).toBeUndefined()
})
