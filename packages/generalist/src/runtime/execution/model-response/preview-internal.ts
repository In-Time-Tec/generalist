...
import {
  ...
  SubscriberCapacity,
  type Change as ModelPreviewChange,
  type Cleared as ModelPreviewCleared,
...
export class ModelPreviewLane extends Context.Service<ModelPreviewLane, Service>()(
  "generalist/runtime/execution/model-response/preview-internal/ModelPreviewLane",
) {}
...
const deltaOf = (part: ModelPart): readonly [Channel, string] | undefined => {
  if (part.part.type === "text-delta") return ["text", part.part.delta]
  if (part.part.type === "reasoning-delta") return ["reasoning", part.part.delta]
  return undefined
}

const appendDelta = <E, R>(
  initial: State,
  channel: Channel,
  delta: string,
  now: number,
...
const update = <E, R>(
  ...
  Effect.gen(function* () {
    ...
    if (delta !== undefined && delta[1].length > 0) {
      state = yield* appendDelta(state, delta[0], delta[1], now, emit)
      const oversized = delta[1].length > MaxPayloadCharacters
        ...
        offer: (part) =>
          serializer.withPermits(1)(
            Effect.gen(function* () {
              ...
              const now = yield* Clock.currentTimeMillis
              let emitted = false
              const next = yield* update(yield* Ref.get(state), part, now, (frame) => {
                emitted = true
                return emit(frame)
              })
