...
describe("Cell failure taxonomy", () => {
  ...
  it("tags every failure under the package scope", () => {
    for (const tag of Cell.failureTags) expect(tag.startsWith("@batonfx/repl/")).toBe(true)
  })

  it("rejects an unknown failure tag", () => {
    expect(() => Schema.decodeUnknownSync(Cell.CellFailure)({ _tag: "@batonfx/repl/Nope", message: "x" })).toThrow()
  })
