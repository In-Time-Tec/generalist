...
type StoredRun = RuntimeState["runs"] extends ReadonlyMap<string, infer Run> ? Run : never
...
export const recordOperation: {
  ...
  Effect.gen(function* () {
    ...
    let next: RuntimeState = { ...state, nextOperationCounter: state.nextOperationCounter + 1, operations, runs }
