...
export const cancelSession: {
  ...
  }): (state: RuntimeState) => Effect.Effect<readonly [ReadonlyArray<string>, RuntimeState], never, PreparedObservation>
  (
    state: RuntimeState,
    input: { readonly sessionId: string; readonly reason?: string },
  ): Effect.Effect<readonly [ReadonlyArray<string>, RuntimeState], never, PreparedObservation>
} = Function.dual(2, (state: RuntimeState, input: { readonly sessionId: string; readonly reason?: string }) =>
  Effect.gen(function* () {
