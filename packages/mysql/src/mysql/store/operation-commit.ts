...
const verifySteeringOwnership = (op: CompleteInput) =>
  Effect.gen(function* () {
...
const appendHandoffProjection = (loaded: DecodedRun, row: OperationRow, op: CompleteInput) =>
  Effect.gen(function* () {
    if (row.kind !== "handoff" || op.outcome._tag !== "Succeeded" || !isHandoffCommit(op.outcome.value)) return
    const entry = handoffSessionEntry({
      sessionId: loaded.message.sessionId,
...
const persistOutcome = (op: CompleteInput) =>
  Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient
...
const completeMysqlOperation = (
  ...
  import("tenetkit/runtime/driver/sql/operations").OperationRecord,
...
> =>
  Effect.gen(function* () {
    ...
    if (row.status === "succeeded" || row.status === "failed" || row.status === "unknown") {
      return yield* verifyCompletedRetry(loaded, current, op)
    }
    yield* verifySteeringOwnership(op)
    ...
    yield* appendHandoffProjection(loaded, row, op)
    yield* persistOutcome(op)
    yield* sql`
