...
const toolCallPart = (id: string, name: string, params: Schema.Json) =>
  Schema.decodeSync(Response.ToolCallPart(name, Schema.Json))({
    type: "tool-call",
    id,
    name,
    params,
...
const activeToolsSchema = Schema.Array(Schema.Struct({ name: Schema.String }))
const activeToolNames = (options: Parameters<ModelParams["streamText"]>[0]) =>
  Schema.decodeSync(activeToolsSchema)(options.tools).map((tool) => tool.name)

const request = (name: string, params: Schema.Json): ToolExecutor.Request => {
...
class AuthorizationDependency extends Context.Service<AuthorizationDependency, string>()(
  "generalist/test/core/agent/tool.test/AuthorizationDependency",
) {}

class PlacementPrefix extends Context.Service<PlacementPrefix, string>()(
  "generalist/test/core/agent/tool.test/PlacementPrefix",
) {}
...
layer(unusedToolHandlerLayer)("AgentTool", (it) => {
  ...
  it("exposes a stable schema-backed closed tool contract", () => {
    ...
    expect(requirementChildTool.invoke).toBeDefined()
  })
  ...
  ItLayer.make(it, "ToolExecutor.client validates placement results against the Effect AI tool schema", () => {
    ...
    const selectFileParams = Schema.Struct({ invalid: Schema.optionalKey(Schema.Boolean) })
      ...
      Layer.mergeAll(
        ToolExecutor.layerRouter([
          ToolExecutor.client({
            toolkit,
            execute: ({ call }) => {
              const params = Schema.decodeUnknownSync(selectFileParams)(call.params)
  ...
  ItLayer.make(it, "ToolExecutor.remote retries infrastructure failures without retrying tool failures", () => {
    ...
    const runCiParams = Schema.Struct({
      toolFailure: Schema.optionalKey(Schema.Boolean),
      infrastructureFailure: Schema.optionalKey(Schema.Boolean),
    })
    let attempts = 0
        ...
        ToolExecutor.layerRouter([
          ToolExecutor.remote({
            ...
            execute: ({ call }): Effect.Effect<ToolPlacement.PlacementResponse, string> =>
              Effect.gen(function* () {
                attempts += 1
                const params = yield* Schema.decodeUnknownEffect(runCiParams)(call.params).pipe(
                  Effect.mapError(() => "invalid run_ci parameters"),
                )
                if (params.toolFailure === true) {
                  return { _tag: "DomainFailure", failure: { message: "ci failed" } }
                }
                if (params.infrastructureFailure === true) {
  ...
  ItLayer.make(it, "ToolExecutor.remote bounds retries and excludes outcomes, defects, and interruption", () => {
    ...
    const toolkit = Toolkit.make(lookup)
    const lookupParams = Schema.Struct({ mode: Schema.String })
    ...
    const remote = ToolExecutor.remote({
      ...
      execute: ({ call }): Effect.Effect<ToolPlacement.PlacementResponse, string | AgentEvent.AgentError> =>
        Effect.gen(function* () {
          const { mode } = yield* Schema.decodeUnknownEffect(lookupParams)(call.params).pipe(
            Effect.mapError(() => "invalid mode"),
          )
          attempts[mode] = (attempts[mode] ?? 0) + 1
          if (mode === "domain") return { _tag: "DomainFailure", failure: { message: "not found" } }
          if (mode === "success") return { _tag: "Success", result: "found" }
          if (mode === "defect") return yield* Effect.die("broken adapter")
          if (mode === "interrupt") return yield* Effect.interrupt
          if (mode === "framework") return yield* AgentEvent.AgentError.make({ message: "invalid route", turn: 0 })
  ...
  ItLayer.make(it, "ToolExecutor.mcp and sandbox are placement routes over Effect AI tool names", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        const executor = yield* ToolExecutor.ToolExecutor
        const mcp = yield* executor.execute(request("github_search", { query: "generalist" }))
