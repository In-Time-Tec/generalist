...
describe("ToolAuthorization", () => {
  ...
  it.effect("routes asks through Approved, Denied, and Pending resolutions", () =>
    Effect.gen(function* () {
      ...
      for (const resolution of [
        ...
        let expected = "Suspend"
        if (resolution._tag === "Approved") expected = "Execute"
        if (resolution._tag === "Denied") expected = "Deny"
  ...
  it.effect("maps permission failures to AuthorizationError", () =>
    Effect.gen(function* () {
      ...
      expect(failure).toMatchObject({ _tag: "generalist/core/AuthorizationError", message: "policy unavailable" })
    }),
  )
