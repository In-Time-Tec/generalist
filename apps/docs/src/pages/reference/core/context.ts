import { code, command, definePage, h2, lead, link, p, table } from "../../../prose"
export const coreContextReference = definePage({
  path: "/docs/reference/core-context",
  title: "Context seams",
  navTitle: "Context seams",
  group: "Reference",
  description: "Instructions, SkillCatalog, Memory, Session, Compaction, Steering, and Handoff.",
  content: [
    lead(
      "Seven namespaces of tenetkit shape what the model sees and how a conversation persists. All are optional seams discovered per run; absent means default behavior.",
    ),
    command("Install", "bun add effect@4.0.0-rc.112 tenetkit@0.43.0"),
    h2("instructions", "Instructions"),
    p(
      "An ordered registry of ",
      code("Source"),
      " values: ",
      code("{ id, render }"),
      ". Every source renders once into the system message when an epoch opens.",
    ),
    table(
      ["Export", "Notes"],
      [
        [[code("staticSource(id, text)")], "A static baseline source; empty text renders nothing"],
        [[code("openEpoch(instructions, context)")], "Renders every source once and returns the joined baseline"],
        [
          [code("layer(sources)"), " / ", code("layerTest(implementation)")],
          "Explicit ordered registry; layer from a service",
        ],
      ],
    ),
    h2("skill-catalog", "SkillCatalog"),
    p(
      "The skill registry seam: ",
      code("{ all, get(name) }"),
      " over ",
      code("Skill = { name, description, instructions, tools, location? }"),
      ". The loop derives listings within a token budget and loads instructions lazily through the built-in ",
      code("activate_skill"),
      " tool.",
    ),
    table(
      ["Export", "Notes"],
      [
        [
          [code("Skill")],
          [
            code("name"),
            " and ",
            code("description"),
            ", lazy ",
            code("instructions"),
            ", and ",
            code("tools"),
            " required; optional ",
            code("whenToUse"),
            ", ",
            code("allowedTools"),
            ", ",
            code("disableModelInvocation"),
            ", ",
            code("userInvocable"),
            ", ",
            code("contextFork"),
            ", ",
            code("agent"),
            ", ",
            code("model"),
            ", ",
            code("paths"),
            ", and ",
            code("location"),
          ],
        ],
        [
          [code("selectListings(skills, budgetTokens, recentlyUsed)")],
          [
            "Startup listings within a token budget; skills with ",
            code("disableModelInvocation: true"),
            " are excluded, least-recently-used drop first",
          ],
        ],
        [
          [code("layerSkills(skills)"), " / ", code("layerEmpty"), " / ", code("layerTest")],
          "In-memory catalog, empty catalog, layer from a service",
        ],
        [[code("SkillCatalogError")], [code("{ source, message, cause? }")]],
      ],
    ),
    h2("memory", "Memory"),
    p(
      "The recall/remember seam keyed by ",
      code("Key = { agent, subject }"),
      ". ",
      code("recall(input)"),
      " returns ",
      code("Item"),
      " values (",
      code("{ id, parts, metadata? }"),
      ") injected into context; ",
      code("remember(input)"),
      " receives the transcript with a ",
      code("terminal"),
      " flag after each turn.",
    ),
    table(
      ["Export", "Notes"],
      [
        [[code("merge(first, second)")], "Concatenates recalls, fans out remembers"],
        [[code("layerNoop")], "Recalls nothing, remembers nothing"],
        [[code("layerTest(implementation)")], "Layer from an explicit service"],
        [[code("MemoryError")], [code("{ message }")]],
      ],
    ),
    p("Implementations live in ", link("/docs/reference/memory", "tenetkit/memory"), "."),
    h2("session", "Session"),
    p(
      "An append-only entry log with a leaf pointer. ",
      code("Entry"),
      " is the closed union ",
      code(
        "MessageEntry | ToolCallEntry | ToolResultEntry | MemoryEntry | SkillEntry | SteeringEntry | HandoffEntry | CompactionEntry | BranchSummaryEntry",
      ),
      "; the store service is ",
      code("{ reserveEntryId, append, appendCheckpoint, path, setLeaf, leaf }"),
      ".",
    ),
    table(
      ["Entry", "Fields beyond id/parentId/metadata", "Projection"],
      [
        [[code("MessageEntry")], [code("message: Ai.Prompt.Message")], "Included verbatim"],
        [[code("ToolCallEntry")], [code("part: Ai.Prompt.ToolCallPart")], "Assistant tool-call message"],
        [[code("ToolResultEntry")], [code("part: Ai.Prompt.ToolResultPart")], "Tool result message"],
        [[code("MemoryEntry")], [code("items: ReadonlyArray<string>")], [code("<memory>"), " system note"]],
        [[code("SkillEntry")], [code("name"), ", ", code("body")], [code("<skill>"), " system note"]],
        [[code("SteeringEntry")], [code("message: Ai.Prompt.Message")], "Included verbatim"],
        [[code("HandoffEntry")], [code("target"), ", ", code("summary")], [code("<handoff>"), " system note"]],
        [
          [code("CompactionEntry")],
          [code("projectedHistory"), ", ", code("telemetry"), ", optional ", code("summary")],
          ["Self-contained checkpoint; projection never reads entries before it"],
        ],
        [
          [code("BranchSummaryEntry")],
          [code("summary")],
          ["Rendered as an ", code("<abandoned-branch-summary>"), " system message"],
        ],
      ],
    ),
    p(
      code("Session.buildContext(path)"),
      " purely projects a root-to-leaf path into an ",
      code("Ai.Prompt.Prompt"),
      ". ",
      code("Session.layerMemory"),
      " is a keyed Ref-backed non-durable directory; ",
      code("Session.acquire(sessionId)"),
      " retains one exact store and its same-session lane for the current Scope; ",
      code("layerTest"),
      " wraps an explicit directory interface. Store failures are ",
      code("SessionStoreError{ message }"),
      "; stale leaves and reused checkpoint identities fail with ",
      code("SessionConflict"),
      ". Exact checkpoints append idempotently before their stored projection is applied to Chat.",
    ),
    h2("compaction", "Compaction"),
    p(
      "The seam consulted when context approaches the window: ",
      code("maybeCompact(request) => Effect<Option<Result>, CompactionError, LanguageModel>"),
      ". ",
      code("Result"),
      " is ",
      code("MicrocompactResult"),
      " (tool outputs bounded in place) or ",
      code("SummarizeResult"),
      " (summary checkpoint plus the kept tail).",
    ),
    table(
      ["Export", "Notes"],
      [
        [
          [code("defaultStrategy(options?)")],
          "Two-stage strategy: microcompact tool outputs first, then cut and summarize the head",
        ],
        [
          [code("strategy(parts, base?)")],
          "Compiles ordered capability parts onto a complete strategy; the last part for one capability wins",
        ],
        [[code("toolOutputBound({ maxBytes })")], "Lossless successful-tool-result bound backed by ToolOutputStore"],
        [
          [code("structuredSummary(options?)")],
          ["Validated ", code("AgentSummary"), " generation with deterministic string checkpoint rendering"],
        ],
        [
          [code("keepRecent({ tokens })")],
          "Token-denominated recent suffix target; TenetKit does not infer turns from message roles",
        ],
        [
          [code("layer(options?, strategy?)")],
          [
            "Service layer; ",
            code("DefaultOptions"),
            " are ",
            code("reserveTokens"),
            " (default 16384), ",
            code("keepRecentTokens"),
            " (default 20000), ",
            code("contextWindow"),
            ", ",
            code("summaryModel"),
            ", ",
            code("summaryPrompt"),
            " (default ",
            code("SUMMARY_TEMPLATE"),
            "), and an optional compiled ",
            code("strategy"),
          ],
        ],
        [
          [code("truncate(maxTokens)")],
          ["Truncate-only implementation over an ", code("Ai.Tokenizer"), " when one is provided"],
        ],
        [[code("make(strategy, options?)"), " / ", code("layerTest")], "Service from a strategy; layer from a service"],
      ],
    ),
    h2("steering", "Steering"),
    p(
      "Finite process-local inbox policy and errors for a scoped ",
      code("Agent.RunHandle"),
      ": ",
      code("steer"),
      " drains at model-turn boundaries mid-run, ",
      code("followUp"),
      " drains after the run would otherwise complete.",
    ),
    table(
      ["Export", "Notes"],
      [
        [
          [code("Options")],
          [
            "Configured under ",
            code("RunOptions.steering"),
            "; ",
            code("steering.mode"),
            " defaults to ",
            code('"all"'),
            ", ",
            code("followUp.mode"),
            " to ",
            code('"one-at-a-time"'),
            ", and overload defaults to typed fail-fast.",
          ],
        ],
        [
          [code("defaultCapacity"), " / ", code("defaultMaxPendingBytes")],
          ["64 entries in each lane and 1 MiB aggregate encoded prompt bytes per Run"],
        ],
        [[code("Receipt")], ["Accepted identity: ", code("{ runId, queue, sequence, bytes }")]],
        [[code("InboxFull"), " / ", code("RunClosed")], "Typed overload and terminal-admission failures"],
      ],
    ),
    h2("handoff", "Handoff"),
    table(
      ["Export", "Notes"],
      [
        [
          [code("transferTool(target, options?)")],
          ["Handled toolkit exposing ", code("transfer_to_<agent.name>"), " as a same-process handoff tool"],
        ],
        [
          [code("fanOut(children, options?)")],
          "Runs isolated child agents concurrently (default concurrency 4) and preserves input order",
        ],
        [
          [code("supervisor(options)")],
          "Builds a supervisor agent plus the handled toolkit of transfer tools for its specialists",
        ],
      ],
    ),
    p(
      "See ",
      link("/docs/guides/instructions", "How to compose instructions and context sources"),
      ", ",
      link("/docs/guides/skills", "How to add skills"),
      ", ",
      link("/docs/guides/memory", "How to add memory"),
      ", ",
      link("/docs/guides/compaction", "How to stay inside the context window"),
      ", ",
      link("/docs/guides/steering", "How to steer and interrupt a running agent"),
      ", ",
      link("/docs/guides/multi-agent", "How to coordinate multiple agents"),
      ", and ",
      link("/docs/learn/sessions-and-history", "Sessions, history, and persistence"),
      ".",
    ),
  ],
})
