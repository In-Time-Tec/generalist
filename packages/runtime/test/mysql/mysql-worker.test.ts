...
describeMysql("mysql worker cancellation", () => {
  it.live("rejects a stale worker commit after a replacement worker claims the Run", () => {
    const url = mysqlUrl!
    return Effect.gen(function* () {
      ...
      const [staleClaim] = yield* claims.claimReadyRuns({ workerId: "stale-worker", limit: 1, lease: "30 seconds" })
  ...
  it.live(
    ...
    () => {
      ...
      }).pipe((execute) => prepareMysql(url).pipe(Effect.andThen(execute)))
    },
  )
