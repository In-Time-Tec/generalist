...
const read: HostBindings.AnyOperation = {
  name: "read",
  input: ReadInput,
  output: ReadOutput,
  failure: ReadFailure,
...
const broken: HostBindings.AnyOperation = {
  name: "broken",
  input: ReadInput,
  output: ReadOutput,
  failure: ReadFailure,
...
describe("host modules", () => {
  it.effect("describes the mounted surface without exposing handlers", () =>
    Effect.gen(function* () {
      const registry = yield* HostBindings.make([workspace])
  ...
  it.effect("rejects an unmounted module", () =>
    Effect.gen(function* () {
      ...
      if (Schema.is(HostBindings.HostModuleNotFound)(failure)) {
  ...
  it.effect("rejects an unmounted operation and names it", () =>
    Effect.gen(function* () {
      ...
      expect(Schema.is(HostBindings.HostModuleNotFound)(failure)).toBe(true)
      if (Schema.is(HostBindings.HostModuleNotFound)(failure)) expect(failure.operation).toBe("delete")
    }),
  )
  ...
  it.effect("rejects a request that does not match the declared input schema", () =>
    Effect.gen(function* () {
      ...
      if (Schema.is(HostBindings.HostModuleSchemaFailure)(failure)) expect(failure.stage).toBe("decode-input")
    }),
  )
  ...
  it.effect("rejects a handler reply that does not match the declared output schema", () =>
    Effect.gen(function* () {
      ...
      expect(Schema.is(HostBindings.HostModuleSchemaFailure)(failure)).toBe(true)
      if (Schema.is(HostBindings.HostModuleSchemaFailure)(failure)) expect(failure.stage).toBe("encode-output")
    }),
  )

  it.effect("rejects two modules claiming the same name", () =>
    Effect.gen(function* () {
      const failure = yield* Effect.flip(HostBindings.make([workspace, workspace]))
      expect(Schema.is(HostBindings.HostModuleConflict)(failure)).toBe(true)
      if (Schema.is(HostBindings.HostModuleConflict)(failure)) expect(failure.module).toBe("workspace")
    }),
  )

  it.effect("rejects two operations claiming the same name", () =>
    Effect.gen(function* () {
      const duplicate: HostBindings.Module<never> = {
        name: "workspace",
        operations: [read, read],
      }
      const failure = yield* Effect.flip(HostBindings.make([duplicate]))
      expect(Schema.is(HostBindings.HostModuleConflict)(failure)).toBe(true)
      if (Schema.is(HostBindings.HostModuleConflict)(failure)) expect(failure.operation).toBe("read")
    }),
  )

  it.effect("mounts several modules side by side", () =>
    Effect.gen(function* () {
      const registry = yield* HostBindings.make([workspace, { name: "web", operations: [{ ...read, name: "search" }] }])
...
layer(HostBindings.layer([workspace]))("mounted HostBindings", (mounted) => {
