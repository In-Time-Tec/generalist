...
export const claimedStore = (options: {
  ...
  Effect.gen(function* () {
    ...
    const advance = (leafId: string | null, nextSeq: number, updated: string) => sql`
      UPDATE generalist_sessions SET leaf_id = ${leafId}, next_seq = ${nextSeq}, updated_at = ${updated}
          ...
          Effect.gen(function* () {
            ...
            if (id !== null) {
              const rows = yield* sql<{ readonly entry_id: string }>`
                  SELECT entry_id FROM generalist_session_entries
            ...
            yield* sql`
                UPDATE generalist_sessions SET leaf_id = ${id}, updated_at = ${updated} WHERE session_id = ${sessionId}
              `
          }),
        ).pipe(asReserveError),
