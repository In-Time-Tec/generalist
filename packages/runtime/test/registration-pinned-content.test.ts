...
import { AgentManifest, ExecutableManifest as CoreExecutableManifest, Pins } from "@batonfx/core"
import { Effect } from "effect"
import { ExecutableRegistration } from "../src/index.js"

const CODEC = "@batonfx/harness/snapshot"
const VERSION = "1"

const payload = { schemaVersion: "1", scope: "thread:alpha", entries: [{ id: "a", kind: "memory" }] }
const contentFor = (value: unknown): AgentManifest.PinnedContent => ({
  codec: CODEC,
  version: VERSION,
  digest: Pins.digest(value),
})
const pinFor = (value: unknown) => Pins.makeCapability({ codec: CODEC, version: VERSION, payload: value })
...
describe("pinned content in executable registrations", () => {
  ...
  it.effect("fails typed on a wrong codec", () =>
    Effect.gen(function* () {
      const failure = yield* validate([...others, registrationFor({ codec: "@batonfx/other" })]).pipe(Effect.flip)
      expect(failure._tag).toBe("@batonfx/runtime/ExecutableRegistrationInvalid")
