...
import { Chaos } from "generalist/unstable/transport"
import { JournalFault } from "../../../src/runtime/operation/journal-fault.js"
import { ConnectionFault } from "../../../src/unstable/transport/connection-fault.js"

it.effect("interruptAfter interrupts exactly at the declared journaled operation", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const context = yield* Layer.build(Chaos.layerInterruptAfter(3))
...
it.effect("dropConnection fails exactly after the declared admitted event", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const context = yield* Layer.build(Chaos.layerDropConnection(2))
...
it.effect("flakyModel fails every declared request", () =>
  Effect.scoped(
    Layer.build(Chaos.layerFlakyModel({ failEvery: 3 })).pipe(
