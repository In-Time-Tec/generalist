...
function cancelDescendants(
  ...
  return Effect.gen(function* () {
    ...
    const linked = yield* sql<{ child_run_id: string }>`
      SELECT l.child_run_id FROM generalist_run_links l
      LEFT JOIN generalist_fan_out_members m ON m.child_run_id = l.child_run_id
    ...
    const owned = yield* sql<{ child_run_id: string }>`
      ...
      FROM generalist_fan_outs f JOIN generalist_fan_out_members m ON m.fan_out_id = f.fan_out_id
...
const finishCancellation = (
  ...
  Effect.gen(function* () {
    ...
    const running = yield* sql<{ fan_out_id: string }>`
      SELECT fan_out_id FROM generalist_fan_outs WHERE parent_run_id = ${run.runId} AND status = 'running' LIMIT 1
    `
...
const cancelRun = (
  ...
  Effect.gen(function* () {
    ...
    if (marked > 0 && current.status === "needs-resolution") {
      yield* sql`UPDATE generalist_runs SET status = 'cancelling' WHERE run_id = ${run.runId}`
      current = (yield* loadRun(run.runId))!
    }
    ...
    yield* sql`UPDATE generalist_external_child_placements SET cancel_requested = ${cancellationRequested}
...
export const respondApproval: {
  ...
  approvalResponse(input).pipe(
    ...
    Effect.mapError((error) =>
      error._tag === "generalist/runtime/ResponseConflict" ||
      error._tag === "generalist/runtime/WaitNotOpen" ||
      error._tag === "generalist/runtime/RunTerminal"
  ...
  Effect.gen(function* () {
    ...
    if (run.cancellationRequested) {
      ...
      if (running.length > 0 || (yield* hasPendingCancellationWork(run.runId))) {
        yield* sql`UPDATE generalist_runs SET owner_worker_id = NULL${clearLease} WHERE run_id = ${run.runId}`
    ...
    if (running.length > 0) {
      yield* sql`
        UPDATE generalist_runs SET status = 'waiting', owner_worker_id = NULL${clearLease}, suspension_json = NULL,
