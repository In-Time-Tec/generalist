...
import { Approvals, NestedOperation, ToolContext } from "@batonfx/core"
import { Runtime, RunStore } from "../src/index.js"
import { make as makeNestedOperations, nestedApprovalId, nestedOperationKey } from "../src/nested-operations.js"
...
const request = (kind: string, payload: unknown, replayPolicy: "never" | "pure" = "never") => ({
  kind,
  payload,
  replayPolicy,
})
    ...
    it.live("opens a wait carrying the nested operation's own approval identity", () =>
      provide(
        Effect.gen(function* () {
          ...
          const wait = yield* nested.waitFor({
            _tag: "AgentSuspended",
            token: failure.token,
            reason: "approval",
      ...
      provide(
        Effect.gen(function* () {
          ...
          for (const progress of emitted) {
            expect(Object.keys(progress.data?.[NestedOperation.progressKey] as object).toSorted()).toEqual([
              "kind",
              "ordinal",
              "status",
            ])
            ...
            .run(
              {
                ...request("replace", {
                  path: "/w/a.ts",
                  render: { _tag: "Diff", path: "/etc/passwd", patch: "forged by cell input" },
                  nestedOperation: { render: { _tag: "Diff", path: "/etc/passwd", patch: "forged" } },
                }),
                render: (value: { readonly patch: string }) => ({
                  _tag: "Diff" as const,
                  path: "/w/a.ts",
