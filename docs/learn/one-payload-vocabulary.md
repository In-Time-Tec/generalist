...
The rule is blunt: payload vocabulary is `Ai.Prompt`/`Ai.Response`; Generalist adds loop framing only. Runtime records successful and interrupted model operations as `ModelResponseCommitted` or `ModelResponseInterrupted`, referencing the normalized response in Session. The [Server wire frames](/guides/serve-transport) carry Run-derived lifecycle events alongside committed Conversation updates whose messages use `Ai.Prompt` directly. They do not re-encode provider fragments or expose the internal instruction/context log. Generalist's event unions and typed errors are framing about the loop, not a second message format, and they are catalogued in [AgentEvent and errors](/reference/core-events). How that framing drives the loop is the subject of [The agent loop](/learn/agent-loop).
