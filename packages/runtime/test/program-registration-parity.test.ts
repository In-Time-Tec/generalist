...
describe("Program registration parity", () => {
  for (const backend of ["memory", "sqlite"] as const) {
    it.live(`${backend} gives each Program fan-out child only its required registrations`, () => {
      ...
      return Effect.gen(function* () {
        ...
        Effect.provide(
          backend === "memory" ? Runtime.layerMemory(options) : Runtime.layerSqlite({ ...options, filename }),
        ),
        Effect.scoped,
      )
    })
