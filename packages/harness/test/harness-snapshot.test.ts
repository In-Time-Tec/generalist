...
import { HarnessSnapshot, HarnessState } from "../src/index"
...
describe("HarnessSnapshot", () => {
  ...
  it.effect("fails when the payload does not reconstruct the pinned snapshot", () =>
    Effect.gen(function* () {
      ...
      if (failure._tag !== "@batonfx/harness/SnapshotMismatch") return
  ...
  it("names a stable codec and version for durable registrations", () => {
    expect(HarnessSnapshot.CODEC).toBe("@batonfx/harness/snapshot")
