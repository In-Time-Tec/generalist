...
import { closedTestAgent, pinnedTestAgent, pinnedTestExecutable } from "../run/identity.js"
import {
  Address,
  ChildRuns,
  Cursor,
...
const waitParameters = Schema.Struct({ question: Schema.String })
const summaryRoute = Schema.Struct({ route: Schema.Literal("summary-route:v2") })
...
const acknowledgementLayer = (
  ...
  observeClaim: (claim: ExecutionClaim) => void = () => {},
) => {
...
const scopedWith =
  <A, E, R>(layerValue: Layer.Layer<A, E, R>) =>
  <B, E2, R2 extends A | Scope.Scope | R>(effect: Effect.Effect<B, E2, R2>) =>
...
describe("RunExecutor", () => {
  it.live("resumes the exact unacknowledged event tail after an object-host reopen", () => {
    ...
    const claims: Array<ExecutionClaim> = []
      ...
      Effect.gen(function* () {
        ...
        yield* host.execute(
          yield* store.claimExecution({
            commandId: "runtime-execution-run-executor-test-ts-claim-1",
            runId,
  ...
  it.live("replays the full event stream after an object-host reopen when the host never acknowledges", () => {
    ...
    const executeWithoutAcknowledging = scopedWith(layerObject())(
      Effect.gen(function* () {
        ...
        yield* host.execute(
          yield* store.claimExecution({
            commandId: "runtime-execution-run-executor-test-ts-claim-2",
            runId,
            ownerId: objectWorkerId,
          }),
        )
    ...
    Effect.gen(function* () {
      ...
      yield* scopedWith(runtimeLayer)(
        Effect.gen(function* () {
          ...
          yield* host.execute(
            yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-3",
              runId: receipt.runId,
              ownerId: objectWorkerId,
            }),
          )
  ...
  it.effect("keeps a valid conversation after every execution record is dropped", () =>
    Effect.gen(function* () {
      ...
      const layerObject = () => objectRuntimeLayer({ addresses: [] }, storage).pipe(Layer.provide(resolverLayer))
        ...
        Effect.scoped(
          Effect.gen(function* () {
            ...
            const claim = yield* store.claimExecution({
              commandId: `runtime-execution-run-executor-test-ts-claim-4:${++writerCommand}`,
              runId,
              ownerId: objectWorkerId,
            })
            ...
            return yield* body(session.value).pipe(Effect.ensuring(store.releaseExecution(claim).pipe(Effect.orDie)))
          }).pipe((effect) => provideScoped(layerObject(), effect), Effect.scoped),
        )

      yield* withWriter((session) =>
        session
          .append({ _tag: "Message", message: user("m1") }, { commandId: "session-independence:m1" })
          .pipe(Effect.orDie),
      )
      yield* withWriter((session) =>
        session
          .append({ _tag: "Message", message: user("m2") }, { commandId: "session-independence:m2" })
          .pipe(Effect.orDie),
      )
  ...
  it.effect("round-trips a checkpoint whose telemetry carries absent usage fields", () =>
    Effect.gen(function* () {
      ...
      const withWriter = <A>(body: (session: Session.SessionStore) => Effect.Effect<A>) =>
        Effect.scoped(
          Effect.gen(function* () {
            ...
            const claim = yield* store.claimExecution({
              commandId: `runtime-execution-run-executor-test-ts-claim-5:${++writerCommand}`,
              runId,
              ownerId: objectWorkerId,
            })
            const session = yield* store.claimedSessionStore(claim)
      ...
      const id = yield* withWriter((session) => session.reserveEntryId("session-telemetry:reserve").pipe(Effect.orDie))
      yield* withWriter((session) =>
        session
  ...
  it.effect("appends and reads one durable Session across store reopens", () =>
    Effect.gen(function* () {
      ...
      const withWriter = <A>(body: (session: Session.SessionStore) => Effect.Effect<A>) =>
        Effect.scoped(
          Effect.gen(function* () {
            ...
            const claim = yield* store.claimExecution({
              commandId: `runtime-execution-run-executor-test-ts-claim-6:${++writerCommand}`,
              runId,
              ownerId: objectWorkerId,
            })
            const session = yield* store.claimedSessionStore(claim)
      ...
      const first = yield* withWriter((session) =>
        session.append({ _tag: "Message", message: user("m1") }, { commandId: "session-direct:m1" }).pipe(Effect.orDie),
      )
      const second = yield* withWriter((session) =>
        session.append({ _tag: "Message", message: user("m2") }, { commandId: "session-direct:m2" }).pipe(Effect.orDie),
      )
      const path = yield* Effect.gen(function* () {
        const store = yield* RunStore
      ...
      const turn = (idempotencyKey: string, prompt: string) =>
        Effect.scoped(
          Effect.gen(function* () {
            ...
            yield* host.execute(
              yield* store.claimExecution({
                commandId: `runtime-execution-run-executor-test-ts-claim-7-${idempotencyKey}`,
                runId: receipt.runId,
                ownerId: objectWorkerId,
              }),
            )
    ...
    Effect.gen(function* () {
      ...
      const resolver = ExecutableResolver.ExecutableResolver.of({
        resolve: (input) =>
          Effect.sync(() => {
            ...
            if (Option.isNone(Schema.decodeUnknownOption(summaryRoute)(summaryRegistration?.payload))) {
      ...
      yield* scopedWith(layerObject())(
        Effect.gen(function* () {
          ...
          yield* host.execute(
            yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-8",
              runId: receipt.runId,
              ownerId: objectWorkerId,
            }),
          )
    ...
    Effect.gen(function* () {
      ...
      yield* scopedWith(runtimeLayer)(
        Effect.gen(function* () {
          ...
          yield* host.execute(
            yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-9",
              runId: receipt.runId,
              ownerId: objectWorkerId,
            }),
          )
      ...
      yield* scopedWith(runtimeLayer)(
        Effect.gen(function* () {
          ...
          const abandoned = yield* store.claimExecution({
            commandId: "runtime-execution-run-executor-test-ts-claim-10",
            runId: receipt.runId,
            ownerId: objectWorkerId,
          })
          ...
          const replacement = yield* store.claimExecution({
            commandId: "runtime-execution-run-executor-test-ts-claim-11",
            runId: receipt.runId,
            ownerId: objectWorkerId,
          })
    ...
    Effect.gen(function* () {
      ...
      yield* scopedWith(
        ...
        Effect.gen(function* () {
          ...
          yield* host.execute(
            yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-12",
              runId: receipt.runId,
              ownerId: objectWorkerId,
            }),
          )
  ...
  it.effect("persists typed missing and mismatched executable failures", () => {
    ...
    const verify = (
      ...
      Effect.gen(function* () {
        ...
        yield* host.execute(
          yield* store.claimExecution({
            commandId: "runtime-execution-run-executor-test-ts-claim-13",
            runId: receipt.runId,
            ownerId: objectWorkerId,
          }),
        )
  ...
  it.effect("persists operations and resumes a suspended Agent in the same Run", () => {
    ...
    const model = Layer.effect(
      ...
      LanguageModel.make({
        ...
        streamText: () => {
          modelCalls += 1
          let parts: ReadonlyArray<Response.StreamPartEncoded>
          if (modelCalls <= 9) {
            parts = [
              Response.makePart("tool-call", {
            ...
            parts = [Response.makePart("text-delta", { id: "answer", delta: "continued" }), finish]
          }
          const response = Stream.fromIterable(parts)
    ...
    const executor = ToolExecutor.layerTest({
      execute: (request) =>
        Schema.decodeUnknownSync(waitParameters)(request.call.params).question.startsWith("Advance ") ||
        phase === "resume"
    ...
    return scopedWith(runtimeLayer)(
      Effect.gen(function* () {
        ...
        const firstClaim = yield* store.claimExecution({
          commandId: "runtime-execution-run-executor-test-ts-claim-14",
          runId: receipt.runId,
          ownerId: objectWorkerId,
        })
        ...
        expect(
          persisted.suspension?._tag === "generalist/core/AgentSuspended"
        ...
        const resumeExecution = Effect.gen(function* () {
          phase = "resume"
          lifecycle.length = 0
          ...
          const resumeClaim = yield* store.claimExecution({
            commandId: "runtime-execution-run-executor-test-ts-claim-15",
            runId: receipt.runId,
            ownerId: objectWorkerId,
          })
          ...
          expect(terminal?.result).not.toHaveProperty("transcript")
          ...
          for (const event of replay.filter((candidate) => candidate._tag === "ModelResponseCommitted")) {
            ...
            const result = Schema.decodeUnknownOption(operationMessages)(operation?.result)
            if (Option.isSome(result)) inlineMessageCounts.push(result.value.messages.length)
          }
          ...
          const resumedTool = replay.find(
            (event): event is Extract<RunEvent.RunEvent, { readonly _tag: "ToolExecutionCompleted" }> =>
              event._tag === "ToolExecutionCompleted" && event.call.id === "wait-call-10",
          )
          expect(resumedTool?.turn).toBe(9)
          const nextCall = replay.find(
            (event): event is Extract<RunEvent.RunEvent, { readonly _tag: "ModelCallStarted" }> =>
              event._tag === "ModelCallStarted" && event.turn === 10,
          )
          expect(nextCall?.turn).toBe(10)
        ...
        const cancelExecution = Effect.gen(function* () {
          ...
          yield* host.execute(
            yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-16",
          ...
          yield* runtime.cancel({
            commandId: "runtime-execution-run-executor-test-ts-cancel-1",
            runId: cancelled.runId,
            reason: "stop while suspended",
          })
  ...
  it.effect("hosts blocking run_child_group across a object storage reopen and resumes the same Run", () => {
    ...
    const model = Layer.effect(
      ...
      LanguageModel.make({
        ...
        streamText: (options) => {
          modelCalls += 1
          advertisedTools.push(options.tools.map((tool) => Schema.decodeUnknownSync(Schema.String)(tool.name)))
          prompts.push(JSON.stringify(options.prompt.content))
          let parts: ReadonlyArray<Response.StreamPartEncoded>
          if (modelCalls === 1) {
            parts = [
              Response.makePart("tool-call", {
                ...
                params: {
                  concurrency: 2,
                  members: [
                    { key: "research", selection: "researcher", label: "Research card", prompt: "research" },
                    { key: "analysis", selection: "analyst", label: "Analysis card", prompt: "analyze" },
                  ],
                },
                providerExecuted: false,
              }),
    ...
    const hooks = Hooks.layer([
      Hooks.onChildStart({
        key: "test.runtime.execution.run.executor.onChildStart.1",
        version: "1",
        replayPolicy: "never",
        hook: ({ child }) =>
          Effect.sync(() => {
      ...
      Hooks.onChildEnd({
        key: "test.runtime.execution.run.executor.onChildEnd.1",
        version: "1",
        replayPolicy: "never",
        hook: ({ child }) =>
          Effect.sync(() => {
    ...
    return Effect.gen(function* () {
      const admitted = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* host.execute(
            yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-17",
              runId: parent.runId,
              ownerId: objectWorkerId,
            }),
          )
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* store.fail({
            ...(yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-18",
              runId: group.members[1]!.childRunId,
              ownerId: objectWorkerId,
            })),
          ...
          yield* store.complete({
            commandId: "child-group:first-complete",
            ...(yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-19",
              runId: group.members[0]!.childRunId,
              ownerId: objectWorkerId,
            })),
            result: {
              text: "first child complete",
              output: "first child complete",
              turns: 1,
          ...
          yield* host.execute(
            yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-20",
  ...
  it.effect("auto-installs nonblocking child admission for the root but not its depth-limited leaf", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* host.execute(
        yield* store.claimExecution({
          commandId: "runtime-execution-run-executor-test-ts-claim-21",
          runId: parent.runId,
          ownerId: objectWorkerId,
        }),
      )
      ...
      yield* host.execute(
        yield* store.claimExecution({
          commandId: "runtime-execution-run-executor-test-ts-claim-22",
  ...
  it.effect("advertises recursive child tools only while persisted tree capacity remains", () => {
    ...
    return Effect.gen(function* () {
      ...
      const execute = (runId: string) =>
        Effect.flatMap(
          store.claimExecution({
            commandId: `runtime-execution-run-executor-test-ts-claim-23:${runId}:${claimSequence++}`,
            runId,
            ownerId: objectWorkerId,
          }),
          host.execute,
      ...
      yield* runtime.cancel({
        commandId: "runtime-execution-run-executor-test-ts-cancel-3",
        runId: group.children[0]!.childRunId,
        reason: "settle admitted group",
      })
      yield* execute(exhausted.runId)
  ...
  it.effect("persists distinct suspension checkpoints when one turn suspends twice after a resume", () => {
    ...
    const executor = ToolExecutor.layerTest({
      execute: (request) =>
        Schema.decodeUnknownSync(waitParameters)(request.call.params).question === "First?"
  ...
  it.effect("preserves structured run-budget exhaustion details", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* host.execute(
        yield* store.claimExecution({
          commandId: "runtime-execution-run-executor-test-ts-claim-25",
          runId: receipt.runId,
          ownerId: objectWorkerId,
        }),
      )
  ...
  it.effect("atomically commits a paid model response before a token overrun stops execution", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* host.execute(
        yield* store.claimExecution({
          commandId: "runtime-execution-run-executor-test-ts-claim-26",
          runId: receipt.runId,
          ownerId: objectWorkerId,
        }),
      )
    ...
    Effect.gen(function* () {
      ...
      yield* scopedWith(runtimeLayer)(
        Effect.gen(function* () {
          ...
          expect(lifecycle).toEqual(["admission resolver acquired", "admission resolver finalized"])
          const claim = yield* store.claimExecution({
            commandId: "runtime-execution-run-executor-test-ts-claim-27",
            runId: receipt.runId,
            ownerId: objectWorkerId,
          })
          ...
          yield* runtime.cancel({
            commandId: "runtime-execution-run-executor-test-ts-cancel-4",
            runId: receipt.runId,
            reason: "stop",
          })
    ...
    Effect.gen(function* () {
      ...
      const runId = yield* scopedWith(
        ...
        Effect.gen(function* () {
          ...
          const claim = yield* store.claimExecution({
            commandId: "runtime-execution-run-executor-test-ts-claim-28",
            runId: receipt.runId,
            ownerId: objectWorkerId,
          })
    ...
    Effect.gen(function* () {
      ...
      yield* scopedWith(runtimeLayer)(
        Effect.gen(function* () {
          ...
          const claim = yield* store.claimExecution({
            commandId: "runtime-execution-run-executor-test-ts-claim-29",
            runId: receipt.runId,
            ownerId: objectWorkerId,
          })
          const fiber = yield* host.execute(claim).pipe(Effect.forkChild({ startImmediately: true }))
          ...
          yield* runtime.cancel({
            commandId: "runtime-execution-run-executor-test-ts-cancel-5",
            runId: receipt.runId,
            reason: "stop",
          })
          const exit = yield* Fiber.await(fiber)
    ...
    Effect.gen(function* () {
      ...
      yield* scopedWith(runtimeLayer)(
        Effect.gen(function* () {
          ...
          const claim = yield* store.claimExecution({
            commandId: "runtime-execution-run-executor-test-ts-claim-30",
            runId: receipt.runId,
            ownerId: objectWorkerId,
          })
          ...
          yield* runtime.cancel({
            commandId: "runtime-execution-run-executor-test-ts-cancel-6",
            runId: receipt.runId,
            reason: "stop",
          })
        ...
        yield* scopedWith(layer())(
          Effect.gen(function* () {
            ...
            const fiber = yield* host
              .execute(
                yield* store.claimExecution({
                  commandId: "runtime-execution-run-executor-test-ts-claim-31",
                  runId: receipt.runId,
                  ownerId: objectWorkerId,
                }),
              )
            ...
            yield* Deferred.await(started)
            yield* runtime.cancel({
              commandId: "runtime-execution-run-executor-test-ts-cancel-7",
              runId: receipt.runId,
              reason: "stop after commit",
            })
  ...
  it.effect("rejects stale execution checkpoint writers", () =>
    Effect.gen(function* () {
      ...
      const first = yield* store.claimExecution({
        commandId: "runtime-execution-run-executor-test-ts-claim-32",
        runId: receipt.runId,
        ownerId: objectWorkerId,
      })
      ...
      yield* store.startOperation({
        commandId: "runtime-execution-run-executor-test-ts-startOperation-8",
        ...first,
        operationId: operation.operationId,
      })
      yield* store.releaseExecution(first)
      const replacement = yield* store.claimExecution({
        commandId: "runtime-execution-run-executor-test-ts-claim-33",
        runId: receipt.runId,
        ownerId: objectWorkerId,
      })
      ...
      expect(yield* Effect.flip(firstSession.reserveEntryId("fenced:stale-reserve"))).toMatchObject({
        message: "Session write claim is stale",
      })
      expect(yield* replacementSession.reserveEntryId("fenced:replacement-reserve")).toMatch(/^\d+$/)
      expect(yield* Effect.flip(firstSession.setLeaf(firstEntry.id, "fenced:stale-leaf"))).toMatchObject({
        message: "Session write claim is stale",
      })
      yield* replacementSession.setLeaf(firstEntry.id, "fenced:replacement-leaf")
      expect(
      ...
      const stale = yield* store
        .saveExecution({
          commandId: "fenced:stale-save",
          ...first,
        })
        .pipe(Effect.flip)
      expect(stale._tag).toBe("generalist/runtime/StaleClaim")
      const staleRecovery = yield* store
        .expireRunningOperation({
          commandId: "fenced:stale-expire",
          ...first,
          operationId: operation.operationId,
        })
        .pipe(Effect.flip)
      expect(staleRecovery._tag).toBe("generalist/runtime/StaleClaim")
  ...
  it.effect("exposes only pre-commit or post-commit operation and checkpoint states", () =>
    Effect.gen(function* () {
      ...
      const claim = yield* store.claimExecution({
        commandId: "runtime-execution-run-executor-test-ts-claim-34",
        runId: receipt.runId,
        ownerId: objectWorkerId,
      })
      const operation = yield* store.recordOperation({
      ...
      yield* store.startOperation({
        commandId: "runtime-execution-run-executor-test-ts-startOperation-9",
        ...claim,
        operationId: operation.operationId,
      })
  ...
  it.effect("atomically imports memory handoff projections with exact retry and divergent rollback", () =>
    Effect.gen(function* () {
      ...
      const claim = yield* store.claimExecution({
        commandId: "runtime-execution-run-executor-test-ts-claim-35",
        runId: receipt.runId,
        ownerId: objectWorkerId,
      })
      ...
      yield* store.startOperation({
        commandId: "runtime-execution-run-executor-test-ts-startOperation-10",
        ...claim,
        operationId: operation.operationId,
      })
      ...
      const commit: Commit = {
        ...
        projectedHistory,
        targetAgentPin: yield* Schema.decodeEffect(Pins.AgentPin)(researcherRef.ref.active),
      }
      const checkpoint = {
        driverVersion: "1" as const,
        executable: researcherRef.ref,
      ...
      yield* session.value.append(
        { _tag: "Message", message: Prompt.make("descendant").content[0]! },
        { commandId: "atomic-operation:descendant" },
      )
      yield* store.completeOperation(completion)
      ...
      const divergentCheckpoint = yield* store
        ...
        .pipe(Effect.flip)
      expect(divergentCheckpoint).toBeInstanceOf(DurabilityFailure)
      if (!Schema.is(DurabilityFailure)(divergentCheckpoint)) return yield* Effect.die("expected receipt conflict")
      expect(divergentCheckpoint.reason).toBe("input-conflict")
      expect(divergentCheckpoint.commandId).toMatch(/^completeOperation:/)
      const divergent = yield* store
        ...
        .pipe(Effect.flip)
      expect(divergent).toBeInstanceOf(DurabilityFailure)
      if (!Schema.is(DurabilityFailure)(divergent)) return yield* Effect.die("expected receipt conflict")
      expect(divergent.reason).toBe("input-conflict")
      expect(divergent.commandId).toBe(divergentCheckpoint.commandId)
  ...
  it.effect("atomically commits failed, unknown, and suspended execution state", () =>
    Effect.gen(function* () {
      ...
      for (const outcome of [{ _tag: "Failed" as const, error: { message: "failed" } }, { _tag: "Unknown" as const }]) {
        ...
        const claim = yield* store.claimExecution({
          commandId: `runtime-execution-run-executor-test-ts-claim-36:${receipt.runId}`,
          runId: receipt.runId,
          ownerId: objectWorkerId,
        })
        const operation = yield* store.recordOperation({
        ...
        yield* store.startOperation({
          commandId: `runtime-execution-run-executor-test-ts-startOperation-11:${receipt.runId}:${operation.operationId}`,
          ...claim,
          operationId: operation.operationId,
        })
      ...
      const claim = yield* store.claimExecution({
        commandId: "runtime-execution-run-executor-test-ts-claim-37",
        runId: receipt.runId,
        ownerId: objectWorkerId,
      })
  ...
  for (const earlyFailure of [
    {
      ...
      expectedTag: "generalist/runtime/ExecutablePinMissing",
      expectedMessage: undefined,
      opensService: false,
    },
    {
      name: "executable identity mismatch",
      expectedTag: "generalist/runtime/ExecutableIdentityMismatch",
      expectedMessage: undefined,
      opensService: false,
    },
    {
      name: "compaction-options mismatch",
      expectedTag: "generalist/runtime/AgentExecutionFailure",
    ...
    it.effect(`defers Program map child ${earlyFailure.name} until scoped finalizers close`, () => {
      ...
      return scopedWith(
        ...
        Effect.gen(function* () {
          ...
          yield* host.execute(
            yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-38",
              runId: parent.runId,
              ownerId: objectWorkerId,
            }),
          )
          ...
          yield* host.execute(
            yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-39",
              runId: childRunId,
              ownerId: objectWorkerId,
            }),
          )
  ...
  const programBackend = "object" as const
  {
    const storage = makeObjectStorage()
    it.effect(`${programBackend} finalizes a real Program map child before its RunFailed commit`, () => {
      let modelCalls = 0
      ...
      return scopedWith(runtimeLayer)(
        Effect.gen(function* () {
          ...
          const receipt = yield* runtime.send({
            to: address,
            sessionId: `session:failed-agent-map:${programBackend}`,
            idempotencyKey: "failed-agent-map",
            prompt: "run the worker map",
          })
          yield* host.execute(
            yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-40",
              runId: receipt.runId,
              ownerId: objectWorkerId,
            }),
          )
          ...
          yield* host.execute(
            yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-41",
              runId: childRunId,
              ownerId: objectWorkerId,
            }),
          )
          ...
          expect(failedOperation).toMatchObject({
            ...
            status: "failed",
            error: { _tag: "generalist/runtime/AgentExecutionFailure" },
          })
          ...
          yield* host.execute(
            yield* store.claimExecution({
              commandId: "runtime-execution-run-executor-test-ts-claim-42",
              runId: receipt.runId,
              ownerId: objectWorkerId,
            }),
          )
          ...
          expect(programOperation).toMatchObject({
            ...
            fanOutId,
            childRunIds: [childRunId],
            error: { _tag: "generalist/core/ProgramAgentFailure", operation: "workers", selection: "worker" },
          })
          ...
          if (parentFailed?._tag === "RunFailed") {
            expect(parentFailed.error).toMatchObject({
              _tag: "generalist/core/ProgramAgentFailure",
              operation: "workers",
              selection: "worker",
            })
          }
  ...
  it.effect("resolves the checkpoint active pin when recovering a Run", () => {
    ...
    return Effect.gen(function* () {
      ...
      const claim = yield* store.claimExecution({
        commandId: "runtime-execution-run-executor-test-ts-claim-43",
        runId: receipt.runId,
        ownerId: objectWorkerId,
      })
