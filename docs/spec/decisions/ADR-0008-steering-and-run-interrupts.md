...
ADR-0036 refines the transcript order: completed tool results are checkpointed in Chat before policy evaluation, then drained steering becomes new input after those results. Steering remains after tool results and before the next model call, but it is no longer concatenated ahead of a pending tool-result prompt.
