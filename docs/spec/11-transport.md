...
`seq` numbers server frames, not agent events. Every allocated frame sequence and request or replay cursor is a non-negative safe integer in `[0, Number.MAX_SAFE_INTEGER]`, enforced by one shared wire schema at frame, WebSocket `afterSeq`, SSE cursor, and client decode boundaries; `Snapshot(seq: -1)` is the sole pre-history replay-boundary exception described below. Allocated sequences are 0-based and monotonic per session. Replay cursors compare against frame `seq`; transports such as SSE use the same value as their resume cursor. Sequence allocation, ring insertion, and live subscriber delivery are serialized per transport session so ring replay and live delivery observe the same order; publication in one session does not serialize publication in another.

Wire encoding is lazy and effectful. The toolkit-parameterized wire codec encodes client and server frames to JSON text and maps schema encoding failures to `WireEncodeError`; request and socket handlers handle that typed failure without converting it to a defect. Existing valid JSON frame fields and tags are unchanged.
