...
const measureDurableJson = (filename: string): DurableJsonMeasurement => {
  ...
  const operations = readRows(
    database,
    `SELECT run_id || ':' || operation_id || ':input' AS rowKey, input_json AS payload FROM generalist_run_operations
     UNION ALL SELECT run_id || ':' || operation_id || ':result', result_json FROM generalist_run_operations
     UNION ALL SELECT run_id || ':' || operation_id || ':error', error_json FROM generalist_run_operations
     UNION ALL SELECT run_id || ':' || operation_id || ':resolution', resolution_json FROM generalist_run_operations`,
  )
  ...
  const checkpoints = readRows(
    database,
    `SELECT run_id || ':message' AS rowKey, message_json AS payload FROM generalist_runs
  ...
  const tables = database
    .query<
      ...
      []
    >("SELECT name FROM sqlite_master WHERE type = 'table' AND name LIKE 'generalist_%'")
    .all()
  let allJsonBytes = 0
...
const inspectFourChildReferences = (filename: string, expectedMarkers: ReadonlyArray<string>) => {
  ...
  const operations = readRows(
    database,
    "SELECT run_id || ':' || operation_id AS rowKey, result_json AS payload FROM generalist_run_operations WHERE result_json IS NOT NULL",
  ...
  const sessions = readRows(
    database,
    "SELECT session_id || ':' || entry_id AS rowKey, payload_json AS payload FROM generalist_session_entries",
  )
  const sessionCount = database
    .query<{ readonly count: number }, []>("SELECT COUNT(DISTINCT session_id) AS count FROM generalist_session_entries")
    .get()!.count
  database.close()
