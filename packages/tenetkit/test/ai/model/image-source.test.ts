...
const makePrompt = (messages: Parameters<typeof Prompt.make>[0]) => Prompt.make(messages)

const decodeBody = (request: HttpClientRequest.HttpClientRequest): Schema.Json | undefined =>
    ...
    operation === "streamText"
      ...
      : ModelRegistry.operate(
          ...
          operation === "generateObject"
            ...
            : LanguageModel.generateText({ prompt: input }),
        )
...
const adapters = [
  ...
  },
  {
    ...
    layer: layerOrDeterministic({ model: "fallback-openai-test", fallbackModel: "fallback-test", apiKey }),
    inline: imageDataUri,
  },
]
    ...
    it.effect(`${adapter.name} rejects malformed and unsupported images before transport`, () =>
      Effect.gen(function* () {
        const malformed = yield* capture(
          ...
          adapter.model,
          adapter.layer,
          makePrompt([{ role: "user", content: [{ type: "file", mediaType: "image/png", data: "not base64" }] }]),
        )
        const mismatched = yield* capture(
          adapter.provider,
          adapter.model,
          adapter.layer,
          makePrompt([
        ...
        const unsupported = yield* capture(
          adapter.provider,
          adapter.model,
          adapter.layer,
          makePrompt([
        ...
        const noncanonical = yield* capture(
          adapter.provider,
          adapter.model,
          adapter.layer,
          makePrompt([{ role: "user", content: [{ type: "file", mediaType: "image/png", data: "AB==" }] }]),
        )
        const uppercase = yield* capture(
          adapter.provider,
          adapter.model,
          adapter.layer,
          makePrompt([
    ...
    Effect.gen(function* () {
      ...
      const captured = yield* Effect.forEach(mediaTypes, (mediaType) =>
        capture(
          ...
          "mime-test",
          layer,
          makePrompt([{ role: "user", content: [{ type: "file", mediaType, data: new Uint8Array([1]) }] }]),
        ),
      )
