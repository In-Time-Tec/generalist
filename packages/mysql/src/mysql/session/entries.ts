...
/** Append or verify one exact completed assistant projection in the caller's MySQL transaction. */
...
/** Append or verify one exact handoff projection in the caller's MySQL transaction. */
...
export const mysqlSessionStore = (options: {
  ...
  const existingAppend = (
    entry: AppendInput,
    appendOptions: AppendOptions & { readonly id: string },
    session: SessionRow,
  ) =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient
  ...
  const availableSequence = (initial: number) =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient
      ...
      while (true) {
        ...
        `
        if (collision[0] === undefined) return sequence
        sequence += 1
      }
    })
  ...
  const append = (entry: AppendInput, appendOptions?: AppendOptions) =>
    Effect.gen(function* () {
      ...
      const generatedSequence =
        appendOptions?.id === undefined ? yield* availableSequence(session.next_seq) : session.next_seq
