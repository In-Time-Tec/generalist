...
const observedLockMetric = (snapshots: MetricSnapshots, lock: string, transition: string) =>
  snapshots.find(
    (snapshot) =>
      snapshot.id === "generalist_runtime_sql_lock_wait_duration" &&
      snapshot.attributes?.backend === "sqlite" &&
...
const expectTransitionOutcomes = (snapshots: MetricSnapshots) => {
  const snapshot = observedMetric(snapshots, "generalist_runtime_sql_transition_outcomes", "commitModelResponse")
    ...
    Effect.gen(function* () {
      ...
      const transitions = spans.filter(
        (span) =>
          ...
          span.attributes.get("generalist.runtime.sql.transition") === "commitModelResponse",
      )
      expect(transitions.map((span) => span.attributes.get("generalist.runtime.sql.outcome"))).toEqual([
        "committed",
        "exact-retry",
        "divergent-retry",
        "stale-claim",
      ])
      expect(transitions[1]?.attributes.get("generalist.runtime.sql.retry.classification")).toBe("exact-retry")
      expect(transitions[2]?.attributes.get("generalist.runtime.sql.retry.classification")).toBe("divergent-retry")
      expect(
        transitions.every(
          (span) =>
            span.attributes.get("generalist.runtime.sql.backend") === "sqlite" &&
            span.attributes.get("generalist.runtime.run_id") === receipt.runId &&
            span.attributes.get("generalist.runtime.operation_id") === operation.operationId &&
            span.attributes.get("generalist.runtime.operation_kind") === "model",
        ),
      ).toBe(true)
      ...
      expect(
        spans.some(
          (span) =>
            span.name === "Generalist.Runtime.sqlLock" &&
            span.attributes.get("generalist.runtime.sql.backend") === "sqlite" &&
            span.attributes.get("generalist.runtime.sql.lock") === "fence" &&
            span.attributes.get("generalist.runtime.sql.transition") === "commitModelResponse",
        ),
      ).toBe(true)
      ...
      expect(migration?.attributes.get("generalist.runtime.sql.schema.version")).toBe(7)
      ...
      expectMetricCount(snapshots, "generalist_runtime_sql_transition_duration", 4, "commitModelResponse")
      ...
      const schemaStatus = snapshots.find(
        (snapshot) =>
          snapshot.id === "generalist_runtime_sql_schema_statuses" && snapshot.attributes?.backend === "sqlite",
      )
      ...
      const checkpoints = [
        {
          ...
          trigger: (runId: string, sessionId: string) => `
            ...
            BEFORE INSERT ON generalist_session_entries
        ...
        },
        {
          ...
          trigger: (runId: string) => `
            ...
            BEFORE UPDATE ON generalist_run_operations
        ...
        },
        {
          ...
          trigger: (runId: string) => `
            ...
            BEFORE UPDATE ON generalist_runs
            WHEN NEW.run_id = '${quote(runId)}' AND EXISTS (
              SELECT 1 FROM generalist_run_operations
        ...
        },
        {
          ...
          trigger: (runId: string) => `
            ...
            BEFORE INSERT ON generalist_run_events
        ...
        },
        {
          ...
          trigger: (runId: string) => `
            ...
            BEFORE UPDATE ON generalist_tree_roots
            WHEN EXISTS (
    ...
    Effect.gen(function* () {
      ...
      const operationRow = databaseAfter
        .query<
          { result_json: string },
          [string, string]
        >("SELECT result_json FROM generalist_run_operations WHERE run_id = ? AND operation_id = ?")
        .get(receipt.runId, operation.operationId)
      const eventRows = databaseAfter
        .query<
          { event_json: string },
          [string]
        >("SELECT event_json FROM generalist_run_events WHERE run_id = ? AND event_json LIKE '%ModelResponseCommitted%'")
        .all(receipt.runId)
      databaseAfter.close()
    ...
    Effect.gen(function* () {
      ...
      const row = database
        .query<
          ...
          [string, string]
        >("SELECT session_id, entry_id, parent_id, tag, payload_json FROM generalist_session_entries WHERE session_id = ? AND entry_id = ?")
        .get(event.sessionId, event.sessionEntryId)
      if (row === null) return yield* Effect.die("expected persisted completed Session entry")

      database
        .query("UPDATE generalist_session_entries SET entry_id = ? WHERE session_id = ? AND entry_id = ?")
      ...
      database
        .query("UPDATE generalist_session_entries SET parent_id = NULL WHERE session_id = ? AND entry_id = ?")
      ...
      database
        .query("UPDATE generalist_session_entries SET tag = ? WHERE session_id = ? AND entry_id = ?")
      ...
      for (const mutated of [
        ...
        database
          .query("UPDATE generalist_session_entries SET payload_json = ? WHERE session_id = ? AND entry_id = ?")
