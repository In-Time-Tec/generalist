 ...
 */
export const probe = (store: Service, prefix: string): Effect.Effect<void, ObjectStoreFailure> =>
  Effect.gen(function* () {
    ...
    if (initial === undefined || initial.bytes.length !== first.length || !first.every((byte, index) => initial.bytes[index] === byte)) {
      return yield* Effect.fail(new ObjectStoreFailure({
        operation: "probe", key, reason: "invalid-response",
    ...
    if (result !== "conflict" || retained === undefined || retained.bytes.length !== first.length || !first.every((byte, index) => retained.bytes[index] === byte)) {
      return yield* Effect.fail(new ObjectStoreFailure({
        operation: "probe", key, reason: "invalid-response",
