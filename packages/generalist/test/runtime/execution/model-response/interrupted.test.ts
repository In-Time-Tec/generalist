...
import { assistantAddress, memoryLayer, registrationsFor, textPrompt } from "../fixtures.js"
...
const interrupted = (
  ...
  return {
    ...
    ...identity,
    response,
    digest: Pins.digest(Schema.decodeUnknownSync(Schema.Json)(JSON.parse(jsonText(unsigned)))),
  }
}
      ...
      yield* scopedWith(hosted.layer)(
        Effect.gen(function* () {
          ...
          expect(
            ...
            status: "failed",
            error: { _tag: "generalist/runtime/AgentExecutionFailure", message: "execution interrupted" },
          })
          ...
          const laterText = jsonText(prompts[1])
...
it.live("rejects mutated interrupted model response references and Session storage", () => {
  ...
  return scopedWith(sqliteLayer(filename))(
    Effect.gen(function* () {
      ...
      if (payload._tag !== "ModelResponse") return yield* Effect.die("expected persisted ModelResponse payload")
