...
layer(memoryGroupLayer)("model-facing durable child groups", (suite) => {
  suite.effect("narrows child selections in authority-derived tool schemas", () =>
    Effect.sync(() => {
      const tools = ChildRuns.Tools.make({ children: [{ selection: "researcher" }] })
      const groupInput = {
        concurrency: 1,
        members: [{ key: "one", selection: "researcher", label: "Research card", prompt: "work" }],
      }
      expect(Schema.is(tools.startChildGroup.parametersSchema)(groupInput)).toBe(true)
      const allowed = Schema.decodeOption(ChildRuns.StartGroupParameters)(groupInput).pipe(Option.getOrThrow)
      ...
      expect(Schema.is(tools.runChild.parametersSchema)(singletonInput)).toBe(true)
      const singleton = Schema.decodeOption(ChildRuns.Parameters)(singletonInput).pipe(Option.getOrThrow)
      expect(singleton.label).toBe("Singleton card")
      expect(
        Schema.is(tools.runChild.parametersSchema)({
          selection: "researcher",
          label: "x".repeat(257),
          prompt: "work",
        }),
      ...
      expect(
        Schema.is(tools.startChildGroup.parametersSchema)({
          concurrency: 1,
      ...
      Effect.flatMap(Layer.build(objectGroupLayer(storage)), (context) =>
        Effect.gen(function* () {
          ...
          const singleton = yield* children.invoke({
            ...
            operationKey: "turn:0:object-run-child",
            selection: "researcher",
          ...
          const input = {
            parentRunId: parent.runId,
            toolCallId: "object-run-group",
            operationKey: "turn:0:object-run-group",
            concurrency: 3,
            members: groupMembers,
          }
          const outcome = yield* children.runGroup(input)
          ...
          yield* store.suspend({
            ...parentClaim,
            waits: [openGroupWait("object-run-group")],
            suspension: suspension({
              waitId: "object-run-group",
              token: groupId,
              toolCallId: "object-run-group",
              toolName: ChildRuns.runGroupToolName,
              toolParams: { concurrency: 3, members: groupMembers },
      ...
      Effect.flatMap(Layer.build(objectGroupLayer(storage)), (context) =>
        Effect.gen(function* () {
          ...
          expect(yield* runtime.inspect(admitted.parentRunId)).toMatchObject({
            status: "waiting",
            waits: [{ waitId: "object-run-group", status: "open" }],
          })
      ...
      Effect.flatMap(Layer.build(objectGroupLayer(storage)), (context) =>
        Effect.gen(function* () {
          ...
          expect(
            ...
            childDepth: 1,
            origin: {
              parentToolCallId: "object-run-child",
              operationKey: "turn:0:object-run-child",
            },
          })
          expect(
    ...
    const admitted = yield* Effect.scoped(
      Effect.flatMap(Layer.build(objectGroupLayer(storage)), (context) =>
        Effect.gen(function* () {
          ...
          const parent = yield* runtime.send({
            to: assistantAddress,
            sessionId: "child-singleton:object",
            idempotencyKey: "parent",
            prompt: "parent",
          })
          const claim = yield* store.claimExecution({
          ...
          const input = {
            ...
            operationKey: "turn:4:object-child-call",
            selection: "researcher",
