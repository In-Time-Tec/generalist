# Host TenetKit Runtime in Rivet Actors through the raw SDK

TenetKit provides `@tenetkit/rivet/actors` over raw `rivetkit@2.3.10`. One Rivet Actor owns one Runtime partition, and
...
disposed on sleep and destroy. Public actions use Standard Schema inputs and direct TenetKit Runtime values, not an RPC
...
Every authoritative Runtime transaction updates `tenetkit_activations` through the shared SQL lifecycle kernel. A
...
A fresh wake atomically increments the actor-local `tenetkit_rivet_host` incarnation row, uses it as the new Runtime
...
Rivet's public actor context does not expose generation, so TenetKit cannot manufacture its own actor-generation token.
...
RIVET_TEST_DATABASE=filesystem RIVET_TEST_PUBSUB=memory \
  cargo test -p rivet-engine --test tenetkit_generation \
...
The package exposes only ESM `@tenetkit/rivet/actors`. Raw RivetKit advertises CommonJS outputs, but that path is not part
of TenetKit's support claim; clean Node and Bun ESM consumers are verified, and package smoke proves CommonJS resolution
...
shutdown. TenetKit does not close `RawAccess`, and its durable activation recovery does not depend on that private sync;
