...
import { SSE, Wire } from "../../../src/unstable/transport/index.js"
...
class BodyReadError extends Schema.TaggedError<BodyReadError>()("generalist/transport/BodyReadError", {
...
describe("SSE", () => {
  it.live("replays events strictly after Last-Event-ID", () =>
    Effect.gen(function* () {
      ...
      expect(response.headers["generalist-run-event-version"]).toBe("1")
    }),
  )
  ...
  layer(runtimeLayer())("rejects malformed cursors instead of silently replaying from origin", (suite) => {
    suite.effect("rejects malformed cursors instead of silently replaying from origin", () =>
      SSE.respond({ runId: "run-1", request: request("http://test/runs/run-1/events?cursor=wat") }).pipe(
        Effect.flip,
        Effect.map((error) => expect(error._tag).toBe("generalist/transport/InvalidCursor")),
      ),
    )
  })
})
