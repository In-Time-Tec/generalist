...
const recoveryDuration = Metric.timer("generalist_runtime_sqlite_exclusive_recovery_duration", {
  description: "Exclusive SQLite Runtime incarnation recovery duration",
  attributes: { backend: "sqlite-exclusive-host" },
})

const recoveredClaims = Metric.counter("generalist_runtime_sqlite_exclusive_recovered_claims", {
        ...
        .withTransaction(
          Effect.gen(function* () {
            ...
            const rows = yield* sql<ClaimedRow>`
              ...
              LEFT JOIN generalist_sessions s ON s.session_id = r.session_id
            ...
            for (const row of selected) {
              ...
              const released = yield* sql<{ readonly run_id: string }>`
                UPDATE generalist_runs SET owner_worker_id = NULL, attempt_fence = attempt_fence + 1
            ...
            if (selected.length > 0) {
              const final = yield* sql<{ run_id: string; status: string; attempt_fence: number }>`
                SELECT run_id, status, attempt_fence FROM generalist_runs
            ...
            Metric.update(recoveredClaims, result.recovered).pipe(
              Effect.andThen(
                Effect.annotateCurrentSpan({
                  "generalist.runtime.sql.backend": "sqlite-exclusive-host",
                  "generalist.runtime.sql.recovered_claims": result.recovered,
                  "generalist.runtime.sql.recovery_has_more": result.continuation !== undefined,
                }),
              ),
            ),
          ),
          Effect.trackDuration(recoveryDuration),
          Effect.withSpan("Generalist.Runtime.sqlExclusiveRecovery"),
        ),
  }),
)
