...
describe("RunBudget Agent.stream integration", () => {
    ...
    layer(
      ...
      it.effect("stops before tool execution", () =>
        Effect.gen(function* () {
          ...
          expect(failure._tag).toBe("generalist/core/RunBudgetExhausted")
  ...
  standalone.effect("rejects child grant wider than parent remaining", () =>
    Effect.gen(function* () {
      ...
      expect(error._tag).toBe("generalist/core/RunBudgetGrantWidened")
    }),
  )
        ...
        Effect.gen(function* () {
          ...
          expect(
            Schema.is(AgentEvent.ResumeMismatch)(error) ||
              Schema.is(Schema.TaggedStruct("generalist/core/DriverError", {}))(error),
          ).toBe(true)
        }),
      )
    })
