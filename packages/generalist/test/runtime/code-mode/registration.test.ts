...
import { Context, Deferred, Effect, Layer, Result, Schedule, Schema, Stream } from "effect"
...
describe("CodeMode registration", () => {
  ...
  it.live("reopens an approved CodeMode Tool with its retained operation identity and dispatches once", () =>
    Effect.gen(function* () {
      const storage = makeObjectStorage()
      ...
      const root = Agent.make({
        ...
        codeMode: {
          ...
          executor: CodeExecutor.testIdentity,
          maxSourceBytes: 4_096,
          budget: { ...revisionBudget, toolCalls: 1 },
        },
      })
      const model = Layer.effect(
        LanguageModel.LanguageModel,
        LanguageModel.make({
          generateText: () => Effect.succeed([{ type: "text", text: "unused" }]),
          streamText: (request) =>
            JSON.stringify(request.prompt.content).includes("approved-result")
              ? text("approved-finished")
              : Stream.make(
                  Response.makePart("tool-call", {
                    id: "code-mode-approved",
                    name: "code_mode",
                    params: {
                      source: "return await program(input)",
      ...
      const executor = CodeExecutor.makeTest((request) =>
        Effect.gen(function* () {
          executorCalls += 1
          programRunId = request.requestId.split(":")[0]!
          const capabilities = yield* ProgramCapabilities.ProgramCapabilities
          const result = yield* capabilities.callTool({
            operation: "approved-operation",
            tool: "approved_tool",
            input: { value: "approved-result" },
          })
      ...
      const services = Layer.mergeAll(
        model,
        Toolkit.make(approvedTool).toLayer({
          approved_tool: ({ value }) =>
            Effect.gen(function* () {
              dispatches += 1
      ...
      const runId = yield* Effect.gen(function* () {
        ...
        const runtime = Context.get(context, Runtime.Runtime)
        const run = yield* runtime.start(root, "request approval", { idempotencyKey: "code-mode-approval" })
        yield* Deferred.await(approvalRequested).pipe(Effect.timeout("5 seconds"))
        const inspection = yield* runtime.inspect(programRunId).pipe(
          Effect.filterOrFail(
            ({ waits }) => waits.length > 0,
            () => "approval wait is not committed yet",
          ),
          Effect.retry({ times: 1_000, schedule: Schedule.spaced("5 millis") }),
          Effect.timeout("5 seconds"),
        )
        expect(inspection.waits[0]).toMatchObject({
          id: "approval:approved-tool",
          kind: "approval",
        })
        expect({ authorizations, dispatches, executorCalls }).toEqual({
          authorizations: 1,
          dispatches: 0,
          executorCalls: 1,
        })
      ...
      const output = yield* Effect.gen(function* () {
        const context = yield* Layer.build(layer())
        const runtime = Context.get(context, Runtime.Runtime)
        const engine = yield* engineFor(runtime)
        yield* engine.respondApproval({
          runId: programRunId,
          approvalId: "approval:approved-tool",
          commandId: "approve:code-mode-tool",
        ...
        return yield* (yield* engine.getRun(runId)).await.pipe(Effect.timeout("10 seconds"))
      }).pipe(Effect.scoped)

      expect(output).toBe("approved-finished")
      expect({ authorizations, dispatches, executorCalls }).toEqual({
        authorizations: 1,
        dispatches: 1,
        executorCalls: 2,
