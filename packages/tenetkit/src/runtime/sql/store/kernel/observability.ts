...
const transitionDuration = Metric.timer("tenetkit_runtime_sql_transition_duration", {
  description: "Runtime SQL semantic transition duration",
})

const transitionOutcomes = Metric.frequency("tenetkit_runtime_sql_transition_outcomes", {
...
const lockWaitDuration = Metric.timer("tenetkit_runtime_sql_lock_wait_duration", {
  description: "Runtime SQL semantic lock acquisition duration",
})

const readyClaimBatchSize = Metric.histogram("tenetkit_runtime_sql_ready_claim_batch_size", {
  description: "Runtime SQL ready claims returned per batch",
  boundaries: Metric.exponentialBoundaries({ start: 1, factor: 2, count: 16 }),
})

const leaseTakeovers = Metric.counter("tenetkit_runtime_sql_lease_takeovers", {
  description: "Runtime SQL ready claims that replaced a prior execution claim",
  incremental: true,
})

const schemaStatuses = Metric.frequency("tenetkit_runtime_sql_schema_statuses", {
  ...
  currentTransition.pipe(
    Effect.flatMap((observation) =>
      effect.pipe(
        ...
        Effect.withSpan("TenetKit.Runtime.sqlLock", {
          attributes: {
            "tenetkit.runtime.sql.backend": backend,
            "tenetkit.runtime.sql.lock": lock,
            "tenetkit.runtime.sql.transition": observation.transition,
          },
        }),
      ),
    ),
  ...
  Metric.update(Metric.withAttributes(readyClaimBatchSize, { backend }), batchSize).pipe(
    ...
    Effect.andThen(
      Effect.annotateCurrentSpan({
        "tenetkit.runtime.sql.claim.batch_size": batchSize,
        "tenetkit.runtime.sql.claim.lease_takeovers": takeovers,
      }),
    ),
  )
...
const observeSqlTransition = <A, E, R>(
  ...
  const spanAttributes = {
    "tenetkit.runtime.sql.backend": backend,
    "tenetkit.runtime.sql.transition": transition,
    ...(attributes.runId === undefined ? undefined : { "tenetkit.runtime.run_id": attributes.runId }),
    ...(attributes.operationId === undefined ? undefined : { "tenetkit.runtime.operation_id": attributes.operationId }),
    ...(attributes.operationKind === undefined
      ? undefined
      : { "tenetkit.runtime.operation_kind": attributes.operationKind }),
  }
    ...
    return effect.pipe(
      Effect.onExit((exit) => {
        ...
        return Effect.annotateCurrentSpan({
          "tenetkit.runtime.sql.outcome": outcome,
          ...(outcome === "exact-retry" || outcome === "divergent-retry"
            ? { "tenetkit.runtime.sql.retry.classification": outcome }
            : undefined),
        }).pipe(Effect.andThen(Metric.update(outcomes, outcome)))
      }),
      Effect.trackDuration(duration),
      Effect.withSpan("TenetKit.Runtime.sqlTransition", { attributes: spanAttributes }),
      Effect.ensuring(
        Effect.sync(() => {
  ...
  return observeSqlTransition(
    ...
    effect.pipe(
      Effect.onExit((exit) => {
        ...
        return Effect.annotateCurrentSpan({
          "tenetkit.runtime.sql.schema.version": SQL_SCHEMA_VERSION,
          "tenetkit.runtime.sql.schema.checksum": checksum,
          "tenetkit.runtime.sql.schema.status": status,
