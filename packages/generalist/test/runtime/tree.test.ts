...
const blockRootOnChild = (sessionId: string, invocationId: string) =>
  Effect.gen(function* () {
    ...
    const childClaim = yield* store.claimExecution({
      commandId: `runtime-tree-test-ts-claim-1:${sessionId}`,
      runId: child.runId,
      ownerId: objectWorkerId,
    })
    yield* store.suspend({
      ...(yield* store.claimExecution({
        commandId: `runtime-tree-test-ts-claim-2:${sessionId}`,
        runId: root.runId,
        ownerId: objectWorkerId,
      })),
      runId: root.runId,
...
layer(objectLayer)("RunTree", (it) => {
  it.effect("inspects exact active Runs and stable mixed terminal outcomes", () =>
    Effect.gen(function* () {
      ...
      const rootClaim = yield* store.claimExecution({
        commandId: "runtime-tree-test-ts-claim-3",
        runId: root.runId,
        ownerId: objectWorkerId,
      })
      yield* store.complete({
        commandId: "runtime-tree-test-ts-complete-1",
        ...rootClaim,
  ...
  it.effect("awaits terminal from a checkpoint cursor without losing the transition", () =>
    Effect.gen(function* () {
      ...
      const claim = yield* store.claimExecution({
        commandId: "runtime-tree-test-ts-claim-5",
        runId: root.runId,
        ownerId: objectWorkerId,
      })
      yield* store.complete({
        commandId: "runtime-tree-test-ts-complete-3",
        ...claim,
  ...
  it.effect("watches a tree until its terminal checkpoint cursor is drained", () =>
    Effect.gen(function* () {
      ...
      const rootClaim = yield* store.claimExecution({
        commandId: "runtime-tree-test-ts-claim-6",
        runId: root.runId,
        ownerId: objectWorkerId,
      })
      yield* store.complete({
        commandId: "runtime-tree-test-ts-complete-4",
        ...rootClaim,
        result: completedResult("root result"),
      })
      yield* TestClock.adjust("50 millis")
  ...
  it.effect("projects explicit model and tool call identities", () =>
    Effect.gen(function* () {
      ...
      const claim = yield* store.claimExecution({
        commandId: "runtime-tree-test-ts-claim-7",
        runId: root.runId,
        ownerId: objectWorkerId,
      })
  ...
  it.effect("atomically places a concurrent append in the checkpoint or its replay tail", () =>
    Effect.gen(function* () {
      ...
      const claim = yield* store.claimExecution({
        commandId: "runtime-tree-test-ts-claim-8",
        runId: root.runId,
        ownerId: objectWorkerId,
      })
      const [checkpoint] = yield* Effect.all(
        [
          ...
          store.emitAgentEvent({
            commandId: "runtime-tree-test-ts-emitAgentEvent-7",
            ...claim,
  ...
  it.effect("rejects malformed, wrong-root, future, and invalid-limit cursors", () =>
    Effect.gen(function* () {
      ...
      const unsupported = RunTree.TreeCursor.make(
        `generalist-tree:${encodeURIComponent(encodeJson({ version: 2, projection: "run-tree", rootRunId: first.runId, position: 0 }))}`,
      )
      ...
      expect(future._tag).toBe("generalist/runtime/TreeCursorFuture")
      if (future._tag === "generalist/runtime/TreeCursorFuture") {
      ...
      if (expired._tag === "generalist/runtime/TreeCursorExpired") {
      ...
      expect(yield* Effect.flip(RunTree.replay({ rootRunId: first.runId, limit: 0 }))).toMatchObject({
        _tag: "generalist/runtime/TreeReplayLimitInvalid",
        received: "0",
        minimum: 1,
        maximum: 1000,
      })
  ...
  it.effect("resumes the live stream strictly after a tree cursor", () =>
    Effect.gen(function* () {
      ...
      const claim = yield* store.claimExecution({
        commandId: "runtime-tree-test-ts-claim-9",
        runId: root.runId,
        ownerId: objectWorkerId,
      })
      yield* store.emitAgentEvent({
        commandId: "runtime-tree-test-ts-emitAgentEvent-8",
        ...claim,
        event: { _tag: "TurnStarted", turn: 1 },
      })
      yield* TestClock.adjust("50 millis")
      yield* store.emitAgentEvent({
        commandId: "runtime-tree-test-ts-emitAgentEvent-9",
        ...claim,
  ...
  it.effect("hands a checkpoint to live changes without a snapshot gap", () =>
    Effect.gen(function* () {
      ...
      const checkpoint = yield* RunTree.checkpoint(root.runId)
      const claim = yield* store.claimExecution({
        commandId: "runtime-tree-test-ts-claim-10",
        runId: root.runId,
        ownerId: objectWorkerId,
      })
      yield* store.emitAgentEvent({
        commandId: "runtime-tree-test-ts-emitAgentEvent-10",
        ...claim,
        event: { _tag: "TurnStarted", turn: 42 },
      })
  ...
  it.effect("settles a root-blocked watch on an open approval wait at the root", () =>
    Effect.gen(function* () {
      ...
      yield* store.suspend({
        ...(yield* store.claimExecution({
          commandId: "runtime-tree-test-ts-claim-11",
          runId: root.runId,
          ownerId: objectWorkerId,
        })),
        runId: root.runId,
  ...
  it.effect("settles a root-blocked watch when a descendant needs operation resolution", () =>
    Effect.gen(function* () {
      ...
      yield* store.startOperation({
        commandId: "runtime-tree-test-ts-startOperation-11",
        ...claim,
        operationId: operation.operationId,
      })
      yield* store.expireRunningOperation({
        commandId: "runtime-tree-test-ts-expireRunningOperation-12",
        ...claim,
        operationId: operation.operationId,
      })
  ...
  it.effect("keeps a root-blocked watch open when the root completes before its child", () =>
    Effect.gen(function* () {
      ...
      yield* store.claimExecution({
        commandId: "runtime-tree-test-ts-claim-12",
        runId: child.runId,
        ownerId: objectWorkerId,
      })
      const claim = yield* store.claimExecution({
        commandId: "runtime-tree-test-ts-claim-13",
        runId: root.runId,
        ownerId: objectWorkerId,
      })
      yield* store.complete({
        commandId: "runtime-tree-test-ts-complete-13",
        ...claim,
        result: completedResult("root result"),
      })
      const watching = yield* watchBlocked(root.runId)
  ...
  it.effect("requires an explicit open wait and an equal cursor before a root-blocked watch settles", () =>
    Effect.gen(function* () {
      ...
      yield* store.suspend({
        ...(yield* store.claimExecution({
          commandId: "runtime-tree-test-ts-claim-14",
          runId: root.runId,
          ownerId: objectWorkerId,
        })),
        runId: root.runId,
