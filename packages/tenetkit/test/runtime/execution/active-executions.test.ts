...
layer(activeExecutionsLayer)("ActiveExecutions", (it) => {
  ...
  it.effect("observes grace expiry without releasing ownership", () => {
    ...
    return Effect.gen(function* () {
      ...
      const cancellation = tracing.spans.find((span) => span.name === "TenetKit.Runtime.cancel.localExit")
      expect(cancellation?.attributes.get("tenetkit.runtime.run_id")).toBe("observed-teardown")
      expect(cancellation?.events.map(([name]) => name)).toEqual([
        "tenetkit.runtime.cancel.interrupt_sent",
        "tenetkit.runtime.cancel.grace_exceeded",
      ])
      ...
      expect(cancellation?.events.map(([name]) => name)).toEqual([
        "tenetkit.runtime.cancel.interrupt_sent",
        "tenetkit.runtime.cancel.grace_exceeded",
        "tenetkit.runtime.cancel.local_exit_acknowledged",
      ])
