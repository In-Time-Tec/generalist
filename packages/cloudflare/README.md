...
Cloudflare Workers and Durable Objects adapters for Generalist.

```bash
bun add effect@4.0.0-rc.112 generalist@0.44.0 @generalist/cloudflare@0.44.0
```

The package root is intentionally not exported. Import only `@generalist/cloudflare/workers`, `@generalist/cloudflare/durable-objects`, or `@generalist/cloudflare/dynamic-workers`; these are Worker entrypoints, not Node or Bun runtime claims.

## Durable Run activation

`@generalist/cloudflare/durable-objects` stores only its scheduling projection in
`generalist_activations`. Run authority remains in the existing `generalist_*` tables.
Import `SqliteRunActivation` and `makeExclusiveExecutionRecovery` from
`generalist/runtime/sql-driver`. Construct `SqliteRunActivation.makeProjection(sql, rearm)` with the same full-storage SQLite client
...
runtime schema is one baseline, migration 1 `generalist_runtime` at version 1. The
...
- `@generalist/cloudflare/workers`
- `@generalist/cloudflare/durable-objects`
- `@generalist/cloudflare/dynamic-workers`

## Dynamic Program Workers

`@generalist/cloudflare/dynamic-workers` provides `make(options)` and `layer(options)` for a configured Cloudflare
...
The exported `generalist/test` provider suite and the VM Worker Loader fixture prove adapter protocol semantics, fresh
...
`GENERALIST_CLOUDFLARE_DYNAMIC_WORKER_CONFORMANCE_URL` and
`GENERALIST_CLOUDFLARE_DYNAMIC_WORKER_CONFORMANCE_TOKEN` for a deployed harness that exposes
