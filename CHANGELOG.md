...
- **Breaking:** Sessions now own a bounded, editable durable input queue, and Host registration resolves named Agent profiles for bounded delegation. Queue edits and removals use observed revisions and immutable command receipts; authenticated Host and Server routes cannot widen the pinned policy.
...
- Add sleep-time memory consolidation with versioned, revertible memory. `Memory.Service` gains `history(entryId)` and `revert(entryId, { to })`; `RememberInput` gains `entryId`, `supersedes`, and required `evidence: OperationRef[]`; `MemoryError.reason` adds `version` and `unsupported`. `Learning.consolidate({ schedule, window, model, maxProposals, budget? })` registers a scheduled consolidation Agent that reads recent successful Run journals and semantic memory and submits `RefineInstruction`, `Remember`, and `Forget` proposals through the existing Learning approval path; `ApplyHandlers` gains `Forget`. `SemanticRecall` over `VectorStore.layerMemory` and pgvector implement versioning (pgvector adds a `<table>_history` table and backfills existing rows to version 1); `WorkingMemory` and Supermemory report `unsupported`. `ScheduleOptions.scheduleId` makes schedule registration idempotent, and RRULE `FREQ=DAILY;BYHOUR=n` (UTC) is accepted. (#360)
- Add `generalist/unstable/rl-export`. `dag(runtime, runId)` projects a root Run, its retained forks, and linked child Runs into an operation DAG of `ModelCall`, `ToolCall`, `ChildLink`, `Compaction`, and `Terminal` nodes joined by `parent`, `fork`, `child`, and `compaction` edges. `export(dag, { format: "verifiers-v1", include, reward })` streams one JSONL `VerifiersV1Record` per leaf; `Reward.fromGates`, `Reward.fromEval(scorers)`, and `Reward.make` supply the scalar reward, which is journaled as a `Rewarded { leaf, value, source }` lifecycle event. Token ids and log probabilities come only from persisted provider metadata; `ModelCatalog.Metadata` gains `logprobs`, `false` for every bundled adapter, and `generalist/testing/model` steps accept `tokens` and `logprobs`. (#359)
- Add offline runnable `examples/a2a` and `examples/ag-ui`, and fix two adapter bugs they exposed: the A2A and AG-UI handlers now `activate` the Run they `send` to instead of leaving it staged, and the AG-UI stream ends after `RunWaiting`, `RunCompleted`, `RunFailed`, `RunCancelled`, or `OperationUnknown` instead of staying subscribed. `AGUI.RunError` includes `ActivateError`. (#342)
- Add the `generalist/testing/model` subpath so `TestModel` (`layer`, `text`, `object`, `toolCall`, `turn`) imports without `@effect/vitest`; `generalist/testing` is unchanged and `examples/five-minutes` no longer installs `@effect/vitest`. (#439)

## 0.60.0

- Add `Tasks`, a journaled task list tool. `Tasks.layer()` gives every Agent run or Runtime registration in its environment `tasks_read` and complete-replacement `tasks_write`; the list is stored in the Run's driver checkpoint (`LoopDriverState.tasks`) in the same transition as the tool result, so recovery, fork, and rewind carry it without a separate store, and it is rendered as one canonical system message that compaction retains. A successful write projects as the `TasksUpdated { sessionId, cursor, runId, items }` Host event in place of that completion's `ToolCall`. `Tasks.update([{ id, title?, status?, note? }])` builds a steering prompt for `runtime.send(runId, prompt, { policy: "steer" })`. `Agent.Inheritance` gains `tasks: "read" | "none"` (default `none`); `read` gives a child an immutable read-only snapshot of the parent list. `Tasks` is exported from `generalist` and `generalist/tasks`. (#353)
- Make `Runtime.inspect` a superset of the process-local Inspector snapshot. `RuntimeInspection` now extends `RunInspection` with `turn`, aggregated `usage: { inputTokens, outputTokens }`, `usageFacts`, `activeTools`, optional `lastEvent`, `elapsed` (milliseconds since `RunAccepted`), `budget`, `gates`, `children`, and `suspension`. The raw provider usage arrays on `RunSnapshot`, run-tree inspection, and `Trajectory.Turn` are renamed from `usage` to `usageFacts`. `Host.runs.inspect` returns `RuntimeInspection`, `GET /runs/:id` serves the expanded schema, and `docs/openapi.json` is regenerated. (#389)
- Restore a branch Run's Sandbox from the forked journal. At execution bootstrap the durable host derives the newest `SandboxSnapshot` from the retained journal; the first `CellTool` acquisition restores it with `sandbox.fork(snapshotId, { key: branchSessionId })` and later cells reacquire the keyed branch Sandbox, so fork and rewind continue from the copied filesystem image after a SQL close/reopen. `SandboxService.fork(snapshotId, options?: { key })` is implemented by the Bun, worktree, E2B, and Modal leaves. (#398)
- Rewrite the README around the five-minute path: `Agent.run` with the scripted `TestModel`, then the same Agent under the SQLite Runtime surviving a Layer close/reopen. `examples/five-minutes` is that code and runs offline in `bun run test`, and `scripts/readme-check.ts` typechecks every README TypeScript block. (#340)

## 0.59.0

- Add Run mailbox admission policies and run-tree messaging. `runtime.send(runId, prompt, { policy?, from?, idempotencyKey? })`, `RunHandle.send`, `host.runs.send`, and process-local `Agent.send(handle, message, policy)` admit one message to an existing Run under `steer` (next turn boundary), `enqueue` (after the current work), `interrupt` (interrupt in-flight tools; ambiguous never-replay tools become `Unknown` obligations), `rollback` (rewind to the last committed prefix; Runtime only), or `reject` (fail with `RunBusy` while the Run is claimed); `runtime.steer`, `runtime.followUp`, `RunHandle.steer`, and `RunHandle.followUp` are removed. Addressed `runtime.sendMessage` uses the same durable inbox, accepts `policy`, and authorizes a Run source by derived family (self, parent, direct child, sibling) before `MessagingPolicy.allow`; user and system sources bypass the family check. `Steering.toolkit()` gives a model `send_to_child`, `send_to_parent`, and `list_inbox`, `Steering.layer` provides it, and `onSteer` hooks now run for enqueue, rollback, and replayed continuations with their original lane. `MessagingUnauthorized`, `MailboxFull`, `MailboxRateLimited`, `MessageConflict`, and `LayerOptions.mailboxBounds` are removed; `RunBusy`, `NotInFamily`, and `RollbackRequiresRuntime` are added. `Testing.runtimeDriver` gains `steering`. (#347)
- Add child inheritance policy. `Agent.child(agent, input, { inherit })` and `AgentTool.fanOut({ agents: { name: { agent, inherit? } } })` take an `Agent.Inheritance` record: `history: "none" | "summary" | "full"` (full forks the live provider prompt prefix and closes pending tool calls), `tools: "attenuate" | "same"`, `permissions: "inherit" | "fresh"`, `budget?`, `sandbox: "share" | "fork" | "fresh"`, `instructions: "inherit" | "own"`, and `memory: "inherit" | "fresh"`, with defaults `none`/`attenuate`/`inherit`/`fork`/`inherit`/`inherit`; `Agent.inheritance` and `Agent.defaultInheritance` normalize it. A child that would hold tools, a custom authorizer, or a sandbox its parent lacks fails at spawn with `ChildExceedsParent { field }` before any child Run or model call. Model-authored fan-out members are now only `{ agent, input }`; a member budget lives on the declaration under `inherit.budget`. The normalized record is journaled on `ChildLinked` (now a required field, so journals with 0.58.0 `ChildLinked` events do not decode) and in the fan-out admission digest, and restart reapplies it. (#346)
- Reject unknown Sessions and Runs with a typed 404 before the response commits. `Host.events.subscribe` now returns `Effect<Stream<HostEvent, SessionEventsError>, SessionError>` so `GET /sessions/:id/events` and `GET /sessions/:id/ws` fail with `SessionNotFound` instead of answering HTTP 200 and then emitting a failure event, and every `Server.api` endpoint declares its status-annotated error schemas so `RunNotFound` and the other typed errors map to 400/403/404/409/503 instead of 500. Failures after an SSE response has committed remain one terminal `effect/httpapi/stream/failure` event. `docs/openapi.json` is regenerated. (#414)
- Make the `generalist/server` declaration emit reproducible. `Server.api`, its groups, `EventStreamItem`, and the `Server` namespace are explicitly typed so `dist/server/api.d.ts` and `dist/server/index.d.ts` no longer depend on inferred union member order (19,550 and 15,108 lines down to 4,554 and 34), which made the 0.58.0 local proof tarball differ from the published one; `bun run package` now rejects any packed `.d.ts` over 400,000 bytes. (#430)

## 0.58.0

- Add `generalist/server`, the HTTP API over a `Generalist.create` Host, and remove the `generalist/unstable/transport`, `/errors`, `/replay`, `/run-client`, `/snapshot`, `/sse`, `/websocket`, and `/wire` exports together with the Cloudflare hibernating per-Run replay adapter. `Server.api` is the schema-first `HttpApi`; `Server.layer({ host, auth, operator? })` mounts `POST /sessions`, `GET /sessions`, `GET /sessions/:id`, `POST /sessions/:sessionId/runs`, `GET /sessions/:sessionId/runs`, `GET /runs/:id`, `POST /runs/:id/cancel`, `GET /sessions/:id/events` (SSE; `Last-Event-ID` is the cursor), `GET /sessions/:id/ws`, `POST /runs/:id/approvals/:token`, `GET /runs/:id/explain`, and the operator writes `retry`, `wake`, `resolve-unknown`, and `extend-budget`, which return a typed 403 `OperatorDisabled` unless `operator: true`; `/openapi.json` is public and every other route is authenticated through `Server.Authentication`, with `Server.authBearer(token)` as the bearer Layer. `Server.client({ baseUrl })` is the typed client with `sessions`, `runs`, `events.subscribe`/`events.connect`, `approvals.resolve`, and `operator` calls, and `Server.HostEvent`/`Server.eventCodec` are the shared stream contract. Host filters internal model-response records, so the FoldKit chat projection shows connection, turn, tool, approval, cancellation, and terminal state but no incremental assistant rows. `docs/openapi.json` is generated and checked in `bun run test`. (#335)
- Add typed Agent fan-out. `Agent.fanOut([Agent.child(agent, input), ...], { concurrency, onFailure: "collect" | "failFast" })` runs children in-process with bounded fibers and returns one `Exit` per child in authored order, preserving each Agent's output type; collect keeps failed Exits and fail-fast interrupts siblings. `AgentTool.fanOut({ name, description, agents, maxChildren })` declares a model-callable fan-out with no static handler; under a Runtime it admits members through the existing child-group journal under the parent RunId, reserves `parent remaining / maxChildren` per member (a member `budget` may only narrow that share), refunds unused reservation on settlement, and reattaches the parent to the same group after restart. Registered Agents capture their reachable fan-out graph so child pins resolve on recovery, `RuntimeInspection.children` is now required, and `Testing.runtimeDriver` gains `child-runs`. (#321)

## 0.57.0

- Add triggers that wake suspended Runs. `Agent.awaitEvent(filter, { timeout })` is a terminal tool-handler Effect that suspends the durable tool call until a matching `WakeEvent` (`Timer`, `Webhook`, `ChildCompleted`, `FileChanged`, or `ApprovalResolved`, each with a per-Run `dedupeKey`) or the deadline; resume injects `AwaitEventResult` (`Event | TimedOut`) as the tool result and strict replay does not redispatch. `runtime.wake(runId, event)` journals one validated event and resumes one matching wait at most once, returning `Resumed | Duplicate | Ignored`; `runtime.schedule(agent, input, { rrule, sessionId, budget? })` registers fresh recurring Runs for the fixed UTC `FREQ=SECONDLY|MINUTELY|HOURLY|DAILY[;INTERVAL=n]` subset and fails other rules with `ScheduleInvalid`. `Triggers.ingestWebhook({ source, body, headers })` verifies GitHub, Slack, generic HMAC-SHA256, or unsigned sources into typed `Webhook` events, and `Watcher.watch({ path, recursive? })` streams `FileChanged` events over the Effect `FileSystem`; the host chooses the Run and calls `runtime.wake`. The shared SQL schema baseline moves from v8 to v9, so existing SQL Runtime databases must be recreated. `Testing.runtimeDriver` gains `await-event` and `schedules`. `POST /triggers/:source` is deferred to #335. (#349)
- Add completion gates. `Agent.make({ gates, onGateFailure?, sandbox? })` runs ordered `Gate.command({ name, run })`, `Gate.verifier({ name, agent, threshold })`, and `Gate.predicate({ name, check })` checks against the decoded output before `onRunEnd`; each result is journaled as a checkpoint and published as a `GateResult { turn, name, verdict, evidence }` event, `onGateFailure: "retry"` feeds the failure back to the model while the default `"fail"` ends the Run with `GateFailed`, and `RuntimeInspection.gates`, `RunSnapshot.gates`, `Trajectory.gates`, and `Eval.gatesPassed()` expose the results. A verifier is a fresh process-local child Run without the proposer's history, tools, memory, or hooks. (#350)
- Add `generalist/unstable/rlm`. `Rlm.layer({ root, leaf, maxDepth, maxSubCalls })` is a `LanguageModel` Layer that places the prompt in a Sandbox as data, gives the root model `exec` and `llm_query` tools to slice and inspect that context and delegate sub-questions to the leaf model recursively up to `maxDepth`, aggregates root and leaf usage into one finish, and decodes structured output with the caller schema; `Compaction.rlmOffload({ keepRecentTokens })` is a strategy part that moves older context into the same sandbox instead of summarizing it. Requires a `Sandbox` with files plus TypeScript commands, or Process commands with Bun. (#331)
- Fix a suspended Run looping forever after the second same-turn approval: `resumeBatch` no longer treats waits resolved by the current resume as unresolved, so approving the second of two approval-gated tool calls executes it and the Run advances instead of emitting `TurnCompleted` every checkpoint. `ToolExecutionStarted` is now emitted only inside the live durable operation, so committed replay after recovery or fork emits no extra start and budget inspection counts one start per dispatch. (#404, #408)
- Fail fresh and durable Runs with the original `PermissionDenied` when an approval is denied, instead of an authorization-stage `FrameworkFailure` or feeding the denial back to the model; durable Runs carry it under `AgentExecutionFailure.failure`. (#401)
- Fix `Session` synchronization after a same-run handoff to a target Agent with instructions: reconciliation now skips only the transient derived system messages at turn boundaries instead of failing with `Session projection is not a prefix of live Chat history`. (#400)
- Drop empty streamed text deltas before middleware and events, and normalize OpenRouter finish usage whose reasoning detail exceeds the completion total (`text` is clamped to zero, raw values stay at `metadata.openrouter.usage`, and `metadata.generalist.usageInconsistent` is set). (#402)
- Fix `Trajectory.export` on a Run reopened from a durable store: projection now restores committed usage as `Response.Usage`. (#407)
- Run inline `AgentTool` children with an isolated process-local driver journal so a child model completion no longer fails against the parent's Runtime journal; the parent tool operation remains the durable boundary. (#406)
- Fix `rewind` leaving the discarded Session suffix in the source Session, which made strict replay fail before the next model call; the suffix is copied to the branch first and the source Session is trimmed to the retained prefix. (#405)
- Give `AgentError` targeted hints for a missing ambient `LanguageModel`, provider-rejected model IDs, and authentication failures, and include the first schema issue (bounded to 300 characters) in `InvalidOutput`, `ContextInvalid`, and `InvalidRuleFile` messages. (#403)
- Fix the package README example (`Agent.run`, not the removed `Agent.generate`), make every shipped example and executable docs snippet complete with a terminal `finish` part, and verify all scripted examples and snippets in `bun run test`. The docs app derives its version and export count from the package manifest, the research tutorial reads `RunWaiting.wait.waitId` instead of a hard-coded ID, and the OpenRouter guide documents the `@effect/ai-openrouter` TS2411 `skipLibCheck` workaround. (#383, #412, #413, #415)

## 0.56.0

- Add `generalist/unstable/learning`, a post-run proposal seam. `Learning.declaration({ propose, apply })` builds a `Hooks.onRunEnd` declaration (and `Learning.layer` wraps it as the only hook) that hands the finished run's `Trajectory` to a proposer and journals the returned `Proposal` list (`RefineInstruction`, `AuthorSkill`, `Remember`, `ExportTrajectory`, all `Schema` values) as a nested operation. Each proposal is then applied through its `apply` handler as a non-idempotent approval-gated nested operation with capability `learning`; denials are journaled with their reason, pending approvals persist across restarts as an operator-resolvable suspension, and `proposeWithModel({ model, maxProposals })` is the built-in structured-output proposer. Nested approval identities now include the run ID (`runtime-approval:<runId>:nested-approval:<key>`) so `runtime.operator.resolveApproval` can resolve them. Requires a hosted Runtime; nothing changes model weights. (#333)

## 0.55.0

- Add Run forking, rewind, and counterfactual replay. `runtime.fork(runId, { atSequence, substitute? })` starts a new Run from one committed journal prefix, optionally replacing one completed tool result (`substitute: { operationId, result }`, journaled as `Substituted`), and returns a `RunHandle`; `runtime.rewind(runId, { toSequence })` continues the same Run from an earlier prefix and keeps the discarded suffix as a branch; `runtime.inspect(runId).branches` lists `{ runId, forkedAt }`; `host.sessions.fork` and `host.runs.rewind` expose both through the Host. Errors are `ForkSequenceInvalid`, `SubstitutionInvalid`, and `NoSnapshot` (only when the latest sandbox marker in the prefix is `SandboxSnapshotUnavailable`; Runs without sandbox state fork freely). `Sandbox.layerWorktree({ repo })` is a process-host sandbox whose snapshots are hidden Git commits and whose forks are worktrees; restoring a branch's Sandbox from the copied journal is deferred to #398. The shared SQL schema baseline moves from v7 to v8 with `forked_from`/`fork_sequence` on `generalist_runs` and `checkpoint_json`/`completed_sequence` on run events and operations, so existing SQL Runtime databases must be recreated. `Testing.runtimeDriver` gains the `fork-rewind` capability. (#345)
...
- Generate the host certification matrix. `docs/features/hosts.md` is rendered from `docs/features/hosts-report.json`, which records the capabilities each runtime driver passed in `Testing.runtimeDriver`; `bun run test` refreshes a row only when that driver's whole suite ran and passed, then fails on drift between the report and the page. Cloudflare and Rivet appear as unstable until they register the shared suite. (#323)
- Add typed recovery decisions and `runtime.operator`. `RecoveryDecision` is `Resume | RetryOperation | AwaitApproval | AwaitBudget | Unknown | Failed`; `runtime.operator.explain(runId)` and `verify(runId)` project it from the journal with its obligations and drift, `scanObligations()` streams every Run whose primary decision is not `Resume`, and `retry`, `wake`, `resolveUnknown`, `resolveApproval`, and `extendBudget` take a required operator identity, check the action is legal for the current decision (otherwise `IllegalOperatorAction`), and journal an `operator` audit operation. `Approvals.resolve` accepts `{ operator }` and now lives in the Runtime module. `Testing.runtimeDriver` gains `operator-explain`, `operator-retry`, `operator-resolve-unknown`, and `operator-scan`. (#320)
- Add `generalist/memo`. `Memo.pure({ ttl, dependsOn? })` marks a Tool as side-effect free so successful results are reused across Runs from a `Memo.Store` (`layerMemory()` or `layerSql()` over the Runtime SQL schema); keys cover the tool name, canonical arguments, declared dependency versions, and the tenant and capability scope supplied by `Memo.layerDependencies`. `Memo.models({ enabled: true })` opts temperature-zero model calls into prompt-keyed reuse. A hit is a normal journaled tool result carrying `memoized: { fromRun, fromOperation }`; replay never reads the store. The shared SQL schema baseline moves from v6 to v7 with a `generalist_memo_entries` table, so existing SQL Runtime databases must be recreated. `Testing.memo` certifies a store. (#356)

## 0.52.0

- Add `generalist/host`. `Generalist.create({ agents, plugins? })` builds a `Host` over the ambient `Runtime`, `LanguageModel`, `Approvals`, and `Permissions`: `host.sessions.create({ id?, title? })`, `get`, and `list` manage durable Host Sessions; `host.runs.start(sessionId, agent, input, { idempotencyKey? })` starts a typed Run in a Session and returns `{ id, await, events, steer, followUp }`; `runs.list`, `inspect`, and `cancel` read and control Runs; `host.events.subscribe(sessionId, cursor?)` streams `RunStarted`, `Turn`, `ToolCall`, `ApprovalRequested`, `Compacted`, and `Completed` wrappers that each carry the raw Runtime event and one durable per-Session cursor. `Generalist.plugin({ name, tools?, instructions?, skills? })` adds tools, instructions, and skills to every configured Agent; duplicate plugin names or tool names fail `create` with `PluginNameConflict` or `PluginToolConflict`. The shared SQL Runtime schema baseline moves from v5 to v6 with a `generalist_host_sessions` table and `host_session_id`/`host_session_sequence` columns on `generalist_run_events`; existing SQL Runtime databases must be recreated. `Testing.runtimeDriver` gains the `host-sessions` capability. Lifecycle hooks are deferred to #348. (#334)
- Add durable run budgets. `RunBudget.make({ tokens?, usd?, duration?, toolCalls?, children? })` from `generalist` builds a budget; `runtime.start(agent, input, { budget })` attaches it to a Run. Spend is derived from the journal on every inspect or resume, so `runtime.inspect(runId).budget` reports the remaining amounts (`usd` is `"unknown"` for a model without catalog pricing) and `inspect(runId).suspension` reports why a Run is waiting. Exhausting tokens, USD, or elapsed time suspends the Run as `waiting` with `{ _tag: "BudgetExhausted", budget }` instead of failing it, preserving the pending model operation so replay after a top-up does not redispatch; exceeding tool calls or children fails admission with `RunBudgetExhausted`. Children reserve from the parent's remainder before admission and settlement returns unused reservation. `runtime.extendBudget(runId, delta)` journals a top-up and resumes a Run waiting on the replenished dimension. `ModelCatalog.cost(selection, usage)` prices a `Response.Usage` from catalog metadata, and `TurnPolicy` receives the remaining numeric limits as `info.budget`. (#319)

## 0.51.0

- Start a Run from a registered Agent. `runtime.register(agent)` records an Agent for the Runtime lifetime and fails with `DuplicateAgent` on a repeated name; `runtime.start(agent, input, { sessionId?, idempotencyKey? })` validates the typed input and returns a `RunHandle` with `runId`, `await` (typed output or `RunFailed`/`RunCancelled`/`InvalidOutput`), `events`, `steer`, and `followUp`; `runtime.inspect(runId)` returns a `RuntimeInspection` with `turn` and raw `usage` facts on top of the Run snapshot. Recovery of a Run whose Agent is not registered waits on `agent-registration` and fails with `UnknownAgent` when started. The former `runtime.start({...})` shape is removed. (#318)
- Add Daytona and Fly Sprites sandbox providers under `generalist/unstable/sandbox/daytona` and `generalist/unstable/sandbox/fly-sprites`. Daytona takes `{ apiKey, image, sandboxClass: "container" | "linux-vm", autoPauseAfter? }` and reports `container` or `microvm` isolation from the returned class; Fly Sprites takes `{ token, app }` and reports `microvm`. Both support Process, files, and wall-clock limits over Effect `HttpClient`; CPU and memory limits, TypeScript modules, snapshot, and fork return typed `Unsupported`. Fresh sandboxes are deleted on scope close. Modal and agentOS remain open in #369. (#369)
- Add approval adapters to `generalist/approvals`. `Approved({ remember? })` and `Denied({ reason? })` build decisions; `layerConsole()` asks on the Effect `Terminal` and fails closed on quit or input failure; `layerTiered({ askAbove, ask })` auto-approves below a `Permissions.Level` and delegates above it; `layerDurable({ notify })` records the pending request in the Runtime and calls `notify` with `{ runId, tool, args, level, reason, token }`; `resolve(token, decision)` closes a durable wait from any process, writes a remembered rule first, and requires `Runtime` and `Permissions.RuleStore`. Hosted approval tokens are `runtime-approval:<runId>:<token>`, `Approvals.Pending` carries `level` and `reason`, and `Testing.runtimeDriver` gains the `approval-suspend` capability. `layerMemory`, `layerSqlite`, and the SQL Runtime layers now build their Agent registration per Runtime instance instead of sharing one map across every build of the same Layer value. (#325)
- Make every framework error actionable. Each `Schema` tagged error under `packages/generalist/src` carries a `hint` and its identifying fields; `error.name` stays the tag; errors without a declared `message` field derive their `message` from the identifying fields and hint so `Cause.pretty` prints `Tag: runId="…". Hint: …`; errors with a `message` field keep it and expose `hint` separately. Older encoded errors decode with the current default hint. `Agent.Inspector` is a `Context.Service` with `layerMemory` and `layerTest(implementation)`; `snapshot(runId)` returns `{ runId, turn, usage: { inputTokens, outputTokens }, activeTools, lastEvent?, elapsed }` for Runs whose event stream is consumed with the Layer present, and fails with `InspectorRunNotFound` otherwise. `docs/features/errors.md` lists every error tag. (#317)
- Add `generalist/trajectory` and `generalist/eval`. `Trajectory.fromJournal(runtime, runId)` projects a durable Run into a `Trajectory` (input prompt, per-turn session prompt, normalized response, tool calls and results, raw usage, last compaction, terminal output, stop reason, agent, optional budget) using only the cross-driver `snapshot`, `history`, `sessionEntry`, and `resolveModelResponse` reads; `Trajectory.export(trajectory, { format: "jsonl" })` streams one `{ schemaVersion: "1", trajectory }` line. `Eval.score`, `outputMatches(schema)`, `toolCalledAtMost(tool, n)`, `usageUnder({ usd?, tokens? })`, `judge({ rubric, model })`, and `runSuite(agent, dataset, scorers, { concurrency })` score trajectories; `runSuite` registers the Agent, runs each case through the Runtime, prints a plain-text table, and returns a `SuiteResult`. (#332)
