...
import { ProgramCapabilities, SandboxExecutor } from "../../../src/index.js"
...
const request = (overrides: Partial<SandboxExecutor.Request> = {}): SandboxExecutor.Request => {
  ...
  return {
    ...
    sourceDigest: SandboxExecutor.sourceDigest(source),
    ...source,
    input: { value: 1 },
...
const enforcedIdentity = (overrides: Partial<SandboxExecutor.Identity> = {}): SandboxExecutor.Identity =>
  SandboxExecutor.declareIdentity({
...
it.effect("constructs the canonical request and trusted exact result envelope", () =>
  Effect.gen(function* () {
    ...
    const normalized = SandboxExecutor.makeRequest({
      requestId: "run-1",
    ...
    const executor = SandboxExecutor.makeTest(() => Effect.succeed({ value: 2 }))
    expect(executor.identity).toBe(SandboxExecutor.testIdentity)
...
it("persists immutable bounded identity facts without free-form fields", () => {
  ...
  const encoded = Schema.encodeSync(SandboxExecutor.Identity)(identity)
  expect(Schema.decodeSync(SandboxExecutor.Identity, { onExcessProperty: "error" })(encoded)).toEqual(identity)
  expect(Object.isFrozen(identity)).toBe(true)
  expect(Object.isFrozen(identity.limits)).toBe(true)
  expect(Object.isFrozen(identity.knownLimitations)).toBe(true)
  expect(() =>
    Schema.decodeUnknownSync(SandboxExecutor.Identity, { onExcessProperty: "error" })({
      ...encoded,
      secure: true,
    }),
  ).toThrow()
...
it.effect("admits only fresh isolated requests whose exact guarantees are enforced", () =>
  Effect.gen(function* () {
    yield* SandboxExecutor.admit(enforcedIdentity(), request(), 1_000)

    const trusted = yield* SandboxExecutor.admit(SandboxExecutor.testIdentity, request(), 1_000).pipe(Effect.flip)
    ...
    const unsupported = yield* SandboxExecutor.admit(identity, request(), 1_000).pipe(Effect.flip)
    expect(unsupported).toMatchObject({ guarantee: "cpuMillis", message: "fixture has no CPU governor" })

    const oversized = yield* SandboxExecutor.admit(
      enforcedIdentity(),
...
it.effect("strictly rejects malformed and mismatched result envelopes", () =>
  Effect.gen(function* () {
    ...
    const exact = {
      ...
      output: { value: 2 },
    } satisfies SandboxExecutor.Result
    expect(yield* SandboxExecutor.validateResult(input, exact)).toEqual(exact)
    ...
    for (const malformed of [
      ...
      expect(yield* SandboxExecutor.validateResult(input, malformed).pipe(Effect.flip)).toBeInstanceOf(
        SandboxExecutor.SandboxProtocolViolation,
      )
    }
  }),
)
