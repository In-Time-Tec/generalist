...
const priorOperation = (sql: SqlClient.SqlClient, input: RecordOperationInput, prior: OperationRow) =>
  Effect.gen(function* () {
    ...
    }>`SELECT entry_id FROM tenetkit_run_steering WHERE run_id = ${input.runId} AND consumed_operation_id = ${prior.operation_id} ORDER BY sequence`
...
const persistOperationOutcome = (sql: SqlClient.SqlClient, input: CompleteOperationInput, finished: string) =>
  Effect.gen(function* () {
    if (input.outcome._tag === "Succeeded") {
      yield* sql`UPDATE tenetkit_run_operations SET status = 'succeeded', result_json = ${encodeJsonValue(input.outcome.value)}, finished_at = ${finished} WHERE run_id = ${input.runId} AND operation_id = ${input.operationId} AND status IN ('requested', 'running')`
    } else if (input.outcome._tag === "Failed") {
      yield* sql`UPDATE tenetkit_run_operations SET status = 'failed', error_json = ${encodeJsonValue(input.outcome.error)}, finished_at = ${finished} WHERE run_id = ${input.runId} AND operation_id = ${input.operationId} AND status IN ('requested', 'running')`
    } else {
      yield* sql`UPDATE tenetkit_run_operations SET status = 'unknown', finished_at = ${finished} WHERE run_id = ${input.runId} AND operation_id = ${input.operationId} AND status IN ('requested', 'running')`
    }
  })
...
const verifySteeringOwnership = (sql: SqlClient.SqlClient, input: CompleteOperationInput) =>
  Effect.gen(function* () {
    for (const entryId of new Set(input.steeringEntryIds ?? [])) {
      const rows =
        yield* sql<SteeringConsumptionRow>`SELECT entry_id, consumed_operation_id FROM tenetkit_run_steering WHERE run_id = ${input.runId} AND entry_id = ${entryId}`
...
export const recordOperation: {
  ...
  Effect.gen(function* () {
    ...
    const pending = yield* sql<SteeringConsumptionRow>`
      SELECT entry_id, sequence, consumed_operation_id, discarded_reason FROM tenetkit_run_steering
    ...
    const rows = yield* sql<OperationRow>`
      SELECT * FROM tenetkit_run_operations WHERE run_id = ${input.runId} AND operation_id = ${operationId}
    `
...
export const getOperationByKey = (input: { readonly runId: string; readonly operationKey: string }) =>
  Effect.gen(function* () {
    ...
    yield* requireRun(input.runId)
    const rows =
      yield* sql<OperationRow>`SELECT * FROM tenetkit_run_operations WHERE run_id = ${input.runId} AND operation_key = ${input.operationKey}`
...
export const operationCancellations = (input: { readonly runId: string }) =>
  Effect.gen(function* () {
    ...
    yield* requireRun(input.runId)
    const rows =
      yield* sql<OperationRow>`SELECT * FROM tenetkit_run_operations WHERE run_id = ${input.runId} AND status = 'cancelling' ORDER BY operation_id`
