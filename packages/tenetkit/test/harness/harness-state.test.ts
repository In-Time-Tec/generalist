...
import { HarnessEntry, HarnessState } from "../../src/harness/index"
import { at, entry, scope } from "./harness-fixtures"
...
describe("HarnessState", () => {
  ...
  it("round-trips one state through its schema", () => {
    ...
    expect(Schema.decodeUnknownSync(HarnessState.HarnessState)(encoded)).toEqual(state)
  })

  it("rejects an out-of-contract entry", () => {
    expect(() =>
      Schema.decodeUnknownSync(HarnessEntry.HarnessEntry)({ ...entry({ id: "a", kind: "memory" }), version: 0 }),
    ).toThrow()
    expect(() =>
      Schema.decodeUnknownSync(HarnessEntry.HarnessEntry)({ ...entry({ id: "a", kind: "memory" }), kind: "other" }),
    ).toThrow()
    expect(() =>
      Schema.decodeUnknownSync(HarnessEntry.HarnessEntry)({
