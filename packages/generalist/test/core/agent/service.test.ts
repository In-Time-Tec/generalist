...
class AuthorizationDependency extends Context.Service<AuthorizationDependency, string>()(
  "generalist/test/core/agent/service.test/AuthorizationDependency",
) {}
...
layer(unusedToolHandlerLayer)("Agent", (it) => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        expect(failure).toMatchObject({
          ...
          cause: {
            _tag: "generalist/core/ContextInvalid",
  ...
  ItLayer.make(it, "restores an activated skill from its original call when the persisted result is bounded", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        if (failure._tag !== "generalist/core/AgentSuspended" || suspendedTranscript === undefined) {
    ...
    () =>
      [
        ...
        Effect.gen(function* () {
          ...
          expect(events.some((event) => event._tag === "Completed")).toBe(false)
          expect(failure).toMatchObject({ _tag: "generalist/core/AgentError", turn: 0 })
          expect(
            failure._tag === "generalist/core/AgentError" && Schema.is(ModelStreamTermination.Truncated)(failure.cause),
          ).toBe(true)
        }),
      ] as const,
  )
  ...
  ItLayer.make(it, "provides ToolContext to default toolkit handlers", () => {
    ...
    const handledTool = Tool.make("handled-context", {
      description: "Reads Generalist ToolContext from a toolkit handler",
      parameters: Schema.Struct({ text: Schema.String }),
      success: Schema.Unknown,
  ...
  ItLayer.make(it, "spills large successful tool results by default before re-feeding them", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        const toolSpan = tracing.spans.find((span) => span.name === "Generalist.Agent.tool")
        expect(toolSpan?.attributes.get("generalist.tool.output.original_bytes")).toBe(60 * 1024 + 2)
        expect(toolSpan?.attributes.get("generalist.tool.output.max_bytes")).toBe(50 * 1024)
        expect(toolSpan?.attributes.get("generalist.tool.output.truncated")).toBe(true)
        expect(toolSpan?.attributes.get("generalist.tool.output.spill")).toBe("stored")
        expect(toolSpan?.attributes.get("generalist.tool.output.path_count")).toBe(1)
        expect(toolSpan?.attributes.get("generalist.tool.output.digest")).toMatch(/^[0-9a-f]{64}$/)
        expect(toolSpan?.events.at(-1)?.[0]).toBe("generalist.tool.output.bound")
  ...
  ItLayer.make(it, "attributes a direct terminal structured call to its own ordered turn span", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        expect(turnSpans.map((span) => span.attributes.get("generalist.turn"))).toEqual([0, 1])
  ...
  ItLayer.make(it, "does not start the terminal structured turn before it is consumed", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        const turnSpans = spans.filter((span) => span.name === "Generalist.Agent.turn")
        expect(turnSpans.map((span) => span.attributes.get("generalist.turn"))).toEqual([0])
  ...
  ItLayer.make(it, "fails typed on in-band stream error parts", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        expect(failure._tag === "generalist/core/AgentError" && failure.turn).toBe(0)
        if (failure._tag === "generalist/core/AgentError") {
  ...
  ItLayer.make(it, "fails typed when the stream channel fails", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        expect(failure._tag === "generalist/core/AgentError" && failure.cause).toBe(streamFailure)
      }),
    ] as const
  })
  ...
  ItLayer.make(it, "does not retry model failures when ModelResilience.none is provided", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        expect(calls).toBe(1)
        expect(failure._tag).toBe("generalist/core/AgentError")
        if (failure._tag === "generalist/core/AgentError") expect(failure.cause).toBe(transientModelError)
      }),
    ] as const
  })
  ...
  ItLayer.make(it, "propagates policy requirements and explicit non-limit stop reasons", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        if (failure._tag === "generalist/core/PolicyStopped") {
  ...
  ItLayer.make(it, "checkpoints completed sibling tool results before suspension and preserves them on resume", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        if (suspension._tag !== "generalist/core/AgentSuspended" || suspendedTranscript === undefined) {
  ...
  ItLayer.make(it, "resolves simultaneous authored-order waits without replaying siblings", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        if (afterFirst._tag !== "generalist/core/AgentSuspended" || checkpointAfterFirst === undefined) {
        ...
        if (afterThird._tag !== "generalist/core/AgentSuspended") {
  ...
  ItLayer.make(it, "settles two approvals and one tool wait before crossing a later exclusive barrier", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        if (first._tag !== "generalist/core/AgentSuspended" || checkpoint === undefined) {
        ...
        if (afterPartial._tag !== "generalist/core/AgentSuspended" || checkpointAfterPartial === undefined) {
  ...
  ItLayer.make(it, "rejects a resume token that differs from the authoritative suspension checkpoint", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        expect(mismatch._tag).toBe("generalist/core/ResumeMismatch")
        expect(executions).toBe(1)
      }),
    ] as const
  })
  ...
  ItLayer.make(it, "resumes an authorization suspension with its active-tool snapshot", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        if (failure._tag !== "generalist/core/AgentSuspended" || checkpoint === undefined) {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        for (const received of mismatches) {
          ...
          expect(failure).toMatchObject({
            _tag: "generalist/core/ResumeMismatch",
  ...
  ItLayer.make(it, "rejects an authorization resume whose params differ from the checkpoint", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        expect(resumeFailure._tag).toBe("generalist/core/ResumeMismatch")
      }),
    ] as const
  })
  ...
  ItLayer.make(it, "resumes a framework call that reuses a provider-executed call id", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        if (suspended._tag !== "generalist/core/AgentSuspended" || checkpoint === undefined) {
          return expect.unreachable()
        }
  ...
  ItLayer.make(it, "fails typed when durable telemetry delivery rejects an exact stable batch", () => {
    ...
    return [
      ...
      Effect.gen(function* () {
        ...
        expect(failure._tag).toBe("generalist/core/SinkFailed")
