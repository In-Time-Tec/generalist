...
import { makeAgentExecutionFailure } from "../../src/runtime/agent-execution-failure.js"

const messageFor = (error: unknown): string => makeAgentExecutionFailure(Cause.fail(error)).message
...
describe("terminal agent failure messages", () => {
  ...
  it("keeps the structured budget failure attached for callers that branch on it", () => {
    const error = RunBudget.RunBudgetExhausted.make({ dimension: "modelCalls", requested: 2, remaining: 1 })
