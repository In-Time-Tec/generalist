import "./suites/agent-stream-no-replay-suite.js"
  ...
  (effect) =>
    provideScoped(
      ProgramRunner.layerDirect({
        executor: CodeExecutor.makeTest(fixture),
        handlers: live,
      }),
      effect,
    )
      ...
      runWith(
        (request) =>
          Effect.gen(function* () {
            ...
            const input = yield* Schema.decodeUnknownEffect(Input)(request.input).pipe(Effect.orDie)
            ...
            const output = yield* capabilities
              .callStep({ operation: "shape", step: "double", input: input.value + 1 })
              .pipe(Effect.flatMap(Schema.decodeUnknownEffect(Schema.Finite)), Effect.orDie)
...
it.effect("keeps host authorization and handler failures distinct", () =>
  Effect.scoped(
    Effect.gen(function* () {
      ...
      if (Schema.is(ProgramCapabilities.ProgramToolFailure)(failure)) expect(failure.cause).toBe(cause)
    }),
  ),
)
...
it("rejects blank, ambiguous, and unbounded operation and member keys", () => {
  for (const invalid of ["", "two words", "a/b", "a.b", "_hidden", `a${"x".repeat(64)}`]) {
    expect(() => Schema.decodeSync(ProgramCapabilities.ProgramOperationName)(invalid)).toThrow()
    expect(() => Schema.decodeSync(ProgramCapabilities.ProgramMemberKey)(invalid)).toThrow()
  }
  expect(Schema.decodeSync(ProgramCapabilities.ProgramOperationName)("step_1-ok")).toBe("step_1-ok")
})
