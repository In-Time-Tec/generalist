...
const duplicateStart = (
  ...
  Effect.gen(function* () {
    ...
    const links = yield* sql<{
      ...
      FROM generalist_run_links links JOIN generalist_runs runs ON runs.run_id = links.child_run_id
    ...
    for (const fanOut of input.initialFanOuts) {
      const rows = yield* sql<{ fan_out_id: string }>`SELECT fan_out_id FROM generalist_fan_outs
      ...
      const members = yield* sql<{ child_run_id: string }>`SELECT child_run_id FROM generalist_fan_out_members
  ...
  (hub: EventHub, input: AdmitStartInput, options?: { readonly activate?: boolean }) =>
    ...
    `
      ...
      if (input.runId !== undefined) {
        const byId = yield* sql<RunRow>`SELECT * FROM generalist_runs WHERE run_id = ${input.runId}`
        if (byId[0] !== undefined)
      ...
      if (input.initialChildren.length === 0 && options?.activate !== false) {
        ...
        yield* sql`UPDATE generalist_runs SET attempt_fence = 1 WHERE run_id = ${runId}`
...
export const admitProgramChild: {
  ...
  Effect.gen(function* () {
    ...
    const byId = yield* sql<RunRow>`SELECT * FROM generalist_runs WHERE run_id = ${input.childRunId}`
    if (byId[0] !== undefined) {
