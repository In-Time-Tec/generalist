...
const nextGeneratedId = (sql: SqlClient.SqlClient, sessionId: string, firstSequence: number) =>
  Effect.gen(function* () {
    ...
    while (true) {
      ...
      `
      if (collision[0] === undefined) return { id: String(sequence), sequence }
      sequence += 1
    }
  })
...
/** Dialect-native durable PostgreSQL Session authority bound to one storage-issued claim. */
