...
type EventPartial = Pick<
  Extract<RunEvent, { readonly _tag: "ChildReadinessChanged" }>,
  "_tag" | "childRunId" | "readiness"
>
type AppendFn<E, R> = (
  hub: EventHub,
  run: DecodedRun,
...
export const loadChildReadiness = (
  ...
  Effect.gen(function* () {
    ...
    const rows = yield* sql<{ readiness: ChildReadiness }>`
      SELECT readiness FROM generalist_run_links WHERE child_run_id = ${childRunId}
    `
...
export const readinessForAdmission = (
  ...
  Effect.gen(function* () {
    ...
    const rows = yield* sql<{ count: number | string }>`
      SELECT
        (SELECT COUNT(*) FROM generalist_run_links WHERE parent_run_id = ${parent.runId} AND readiness = 'ready') +
        (SELECT COUNT(*) FROM generalist_external_child_placements
...
export const activeChildCount = (parentRunId: string): Effect.Effect<number, SqlError, SqlClient.SqlClient> =>
  Effect.gen(function* () {
    ...
    const rows = yield* sql<{ count: number | string }>`
      SELECT
        (SELECT COUNT(*) FROM generalist_run_links WHERE parent_run_id = ${parentRunId} AND readiness = 'ready') +
        (SELECT COUNT(*) FROM generalist_external_child_placements
...
export const promoteChildCapacity = <E, R>(input: {
  ...
  Effect.gen(function* () {
    ...
    const queued = yield* sql<{
      ...
      FROM generalist_run_links l
      JOIN generalist_runs r ON r.run_id = l.child_run_id
      LEFT JOIN generalist_fan_out_members m ON m.child_run_id = l.child_run_id
      LEFT JOIN generalist_fan_outs f ON f.fan_out_id = m.fan_out_id
    ...
    for (const candidate of queued) {
      ...
      if (candidate.fan_out_id !== null) {
        ...
        const groupActiveRows = yield* sql<{ count: number | string }>`
          ...
          FROM generalist_fan_out_members m
          JOIN generalist_run_links l ON l.child_run_id = m.child_run_id
      ...
      yield* sql`
        UPDATE generalist_run_links SET readiness = 'ready'
      ...
      if (candidate.fan_out_id !== null) {
        yield* sql`
          UPDATE generalist_fan_out_members SET status = 'running'
