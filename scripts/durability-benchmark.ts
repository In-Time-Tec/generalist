...
import { Array, Console, Duration, Effect, Layer, Order, Schema } from "effect"
...
const program = Effect.gen(function* () {
  ...
  const context = yield* Layer.build(
    layerRunStore(options).pipe(Layer.provide(Layer.succeed(ObjectStore, measured(simulator.store)))),
  )
  const store = yield* RunStore.pipe(Effect.provide(context))
  const latencies: number[] = []
  for (let index = 0; index < count; index++) {
    const [duration] = yield* store
      .admitSend({
        message: {
          id: `message-${index}`,
          to: address,
  ...
  const [recoveryDuration, recovered] = yield* Effect.gen(function* () {
    const freshContext = yield* Layer.build(
      layerRunStore(options).pipe(Layer.provide(Layer.succeed(ObjectStore, measured(freshClient.store)))),
    )
  ...
  const sorted = Array.sort(latencies, Order.Number)
  const [first, last, p50, p95, max] = yield* Schema.decodeUnknownEffect(
    Schema.Tuple([Schema.Finite, Schema.Finite, Schema.Finite, Schema.Finite, Schema.Finite]),
  )([latencies[0], latencies.at(-1), sorted[49], sorted[94], sorted.at(-1)])
  const report = {
    schemaVersion: 1,
    ...
    fixture: {
      ...
      concurrency: 1,
      pageSize: 32,
      snapshotPolicy: "engine defaults",
      seed: "deterministic authored messages",
    },
    admissionMillis: {
      first,
      last,
    ...
    admissionRequests,
    recovery: {
      millis: Duration.toMillis(recoveryDuration),
    ...
    limitations: [
      "No real S3/R2 latency",
      "No cross-host execution",
      "No pre-refactor comparison",
      "No warmup or statistical confidence interval",
      "Partition state is still eagerly materialized",
    ],
