...
const load = (placementId: string) =>
  Effect.gen(function* () {
    ...
    const rows = yield* sql<Row>`SELECT * FROM generalist_external_child_placements WHERE placement_id = ${placementId}`
...
export const reserve: {
  ...
  Effect.gen(function* () {
    ...
    const conflicting = yield* sql<{ placement_id: string }>`
      SELECT placement_id FROM generalist_external_child_placements
    ...
    yield* sql`INSERT INTO generalist_external_child_placements
...
export const acknowledge = (placementId: string) =>
  Effect.gen(function* () {
    ...
    yield* sql`UPDATE generalist_external_child_placements SET acknowledged = 1 WHERE placement_id = ${placementId}`
...
export const cancel = (placementId: string) =>
  Effect.gen(function* () {
    ...
    yield* sql`UPDATE generalist_external_child_placements SET cancel_requested = ${sqlBool(sql, true)}
...
const settle = (
  ...
  Effect.gen(function* () {
    ...
    yield* sql`UPDATE generalist_external_child_placements SET settlement_id = ${input.settlementId},
...
const loadExternalRoot = (placementId: string) =>
  Effect.gen(function* () {
    ...
    const rows = yield* sql<ExternalRootRow>`
      SELECT * FROM generalist_external_roots WHERE placement_id = ${placementId}
    `
    return rows[0]
  })
...
const admitExternalRoot = (hub: EventHub, input: AdmitRootInput) =>
  Effect.gen(function* () {
    ...
    const conflicts = yield* sql<{ placement_id: string }>`
      SELECT placement_id FROM generalist_external_roots
    ...
    yield* sql`INSERT INTO generalist_external_roots
...
const activateExternalRoot = (hub: EventHub, placementId: string) =>
  Effect.gen(function* () {
    ...
    yield* sql`UPDATE generalist_external_roots SET activated = 1 WHERE placement_id = ${placementId}`
...
export const acknowledgeExternalRootSettlement = (input: {
  ...
  Effect.gen(function* () {
    ...
    yield* sql`UPDATE generalist_external_roots SET settlement_acknowledged = 1
