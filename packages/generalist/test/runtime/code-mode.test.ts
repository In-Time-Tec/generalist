    ...
    return withLayer(Runtime.layerMemory({ addresses: [] }).pipe(Layer.provide(resolverLayer)))(
      Effect.gen(function* () {
        ...
        expect(result).toMatchObject({
          _tag: "DomainFailure",
          failure: {
            _tag: "generalist/runtime/ProgramAuthorityExceeded",
            dimension: "tools",
    ...
    withLayer(Runtime.layerMemory({ addresses: [] }).pipe(Layer.provide(fixture().resolverLayer)))(
      Effect.gen(function* () {
        ...
        expect(source).toMatchObject({
          _tag: "DomainFailure",
          failure: { _tag: "generalist/runtime/ProgramAuthorityExceeded", dimension: "sourceBytes" },
        })
        ...
        expect(overBudget).toMatchObject({
          _tag: "DomainFailure",
          failure: { _tag: "generalist/runtime/ProgramAuthorityExceeded", dimension: "toolCalls" },
        })
      }),
    ),
  )
