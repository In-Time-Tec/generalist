...
import { Agent, AgentEvent, Approvals, Guardrail, ModelMiddleware, ToolExecutor } from "../../../src/core/index"
...
layer(unusedToolHandlerLayer)("Guardrail", (it) => {
  ...
  it.effect("redactInput rewrites text-bearing prompt fields without corrupting tool payloads", () => {
    ...
    const verifyUser = (message: Prompt.Message) => {
      if (message.role !== "user") return
      const [text, preservedFile] = message.content
      expect(text?.type === "text" && text.text).toBe("user MASK")
      expect(preservedFile).toBe(file)
    }
    const verifyAssistant = (message: Prompt.Message) => {
      if (message.role !== "assistant") return
      const [text, reasoning, preservedToolCall] = message.content
    ...
    return middleware.transformPrompt!(
      ...
      Effect.map((redacted) => {
        ...
        if (user !== undefined) verifyUser(user)
        if (assistant !== undefined) verifyAssistant(assistant)
        if (tool !== undefined) verifyTool(tool)
      }),
    )
  })
