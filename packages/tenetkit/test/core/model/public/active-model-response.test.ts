...
import { controller } from "../../../../src/core/model/result/active-model-response.js"
      ...
      const provider = modelLayer(() =>
        Stream.suspend(() => {
          ...
          if (attempts === 1) {
            ...
            return Stream.make(metadata).pipe(Stream.concat(Stream.fail(transient)))
          }
          const metadata = {
            type: "response-metadata",
          ...
          return Stream.make(metadata, Response.makePart("text-delta", { id: "answer", delta: "recovered" })).pipe(
