...
describe("ExecutableResolver.makeDynamic", () => {
  ...
  it.effect("fails typed when a required registration pin is missing", () =>
    Effect.gen(function* () {
      ...
      expect(failure).toMatchObject({ _tag: "tenetkit/runtime/ExecutableRegistrationMissing", pin: sandboxPin })
    }),
  )
  ...
  it.effect("fails typed instead of throwing when the reference is not owned by the manifest", () =>
    Effect.gen(function* () {
      ...
      expect(failure).toMatchObject({ _tag: "tenetkit/runtime/ExecutablePinMissing", ref })
    }),
  )
})
