...
export const decision = {
  continue: (overrides?: TurnOverrides): Continue =>
    overrides === undefined ? { _tag: "Continue" } : { _tag: "Continue", overrides },
...
export const recurs = (n: number): Policy => {
  ...
  return Number.isFinite(n) ? { ...policy, snapshot: { _tag: "Recurs", count: n } } : policy
}
...
export const both: {
  ...
  return first.snapshot === undefined || second.snapshot === undefined
    ? policy
    : { ...policy, snapshot: { _tag: "Both", first: first.snapshot, second: second.snapshot } }
})
