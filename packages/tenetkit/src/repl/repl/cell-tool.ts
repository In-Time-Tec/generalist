...
/** @experimental The only name a TenetKit REPL host advertises to a model. */
...
/** @experimental The one Effect AI tool a conversational TenetKit agent advertises. */
...
const identity = (event: CellEvent): Record<string, unknown> => ({
  _tag: event._tag,
  cellId: event.cellId,
  sequence: event.sequence,
})

const encodedSize = (encoded: unknown): number => new TextEncoder().encode(JSON.stringify(encoded)).byteLength
  ...
  encodeEvent(event).pipe(
    Effect.match({
      ...
      onSuccess: (encoded) => {
        ...
        return bytes > maxProgressBytes
          ? { ...identity(event), withheldBytes: bytes }
