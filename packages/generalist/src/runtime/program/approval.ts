...
export const approvedFor: {
  ...
  const token = claimed.suspension?._tag === "generalist/core/ProgramSuspended" ? claimed.suspension.token : undefined
  const wait = programWait(token === undefined ? waitInput : { ...waitInput, token })
  return (
    claimed.suspension?._tag === "generalist/core/ProgramSuspended" &&
...
export const deniedFor: {
  ...
  return claimed.suspension?._tag === "generalist/core/ProgramSuspended" &&
    claimed.suspension.operation === operation &&
    claimed.suspension.reason === "approval" &&
