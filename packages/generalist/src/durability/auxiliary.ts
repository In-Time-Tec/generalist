...
import { type Options as JournalOptions, type State, make } from "./internal/journal.js"
import { decode } from "./internal/protocol.js"
 ...
 * @experimental
 */
export type Options = JournalOptions
...
const verifyPartition = (state: State): Effect.Effect<void, DurabilityFailure> => {
 ...
 * @experimental
 */
export const layerMemo = (options: MemoOptions): Layer.Layer<Store, DurabilityFailure, ObjectStore | Crypto.Crypto> =>
  Layer.effect(
    Store,
    Effect.gen(function* () {
      const journal = yield* make(options)
      ...
      return Store.of({
        ...
        get: (key) =>
          Effect.gen(function* () {
            ...
            const entry = yield* decode(Entry, state[stateKey], "corruption", stateKey)
            ...
            Effect.mapError((cause) =>
              MemoError.make({
                operation: "get",
                key,
                message: `Memo read failed: ${cause.message}`,
                cause,
              }),
            ),
          ),
        put: (key, entry) =>
          Effect.gen(function* () {
            const encoded = yield* decode(Entry, entry, "encoding", `memo:${key}`)
            ...
            yield* journal.commit({ id, input }, (state) =>
              verifyPartition(state).pipe(
                Effect.as({
                  patches: [
            ...
            Effect.mapError((cause) =>
              MemoError.make({
                operation: "put",
                key,
                message: `Memo write failed: ${cause.message}`,
                cause,
              }),
            ),
          ),
...
): Layer.Layer<RuleStore, DurabilityFailure, ObjectStore | Crypto.Crypto> =>
  Layer.effect(
    RuleStore,
    Effect.gen(function* () {
      const journal = yield* make(options)
      ...
      return RuleStore.of({
        rules: Effect.gen(function* () {
          const { state } = yield* journal.read
          yield* verifyPartition(state)
          return Object.hasOwn(state, key) ? yield* decode(Rules, state[key], "corruption", key) : []
        }).pipe(
          Effect.mapError((cause) =>
            PermissionError.make({
              message: `Permission rule read failed: ${cause.message}`,
              cause,
            }),
          ),
        ),
        remember: (rule) =>
          Effect.gen(function* () {
            const encoded = yield* decode(RuleSchema, rule, "encoding", key)
            const id = yield* crypto.randomUUIDv4
            yield* journal.commit({ id: `rule:${id}`, input: { scope, rule: encoded } }, (state) =>
              Effect.gen(function* () {
                yield* verifyPartition(state)
                const rules = Object.hasOwn(state, key) ? yield* decode(Rules, state[key], "corruption", key) : []
                return {
                  patches: [
                    { op: "set" as const, path: [marker], value: 1 },
                    {
                      op: "set" as const,
                      path: [key],
                      value: [...rules.filter((existing) => existing.pattern !== encoded.pattern), encoded],
                    },
                  ],
                  receipt: null,
                }
            ...
            Effect.mapError((cause) =>
              PermissionError.make({
                message: `Permission rule write failed: ${cause.message}`,
                cause,
              }),
            ),
          ),
