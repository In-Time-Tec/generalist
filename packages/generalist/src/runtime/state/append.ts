...
export const appendEvent: {
  ...
  (
    ...
    Effect.gen(function* () {
      ...
      const publication: Types.Mutable<RuntimePublication> = {
        runId,
        event,
...
export const requireOpenRun: {
  (runId: string): (state: RuntimeState) => Effect.Effect<StoredRun, RuntimeUnavailable>
  (state: RuntimeState, runId: string): Effect.Effect<StoredRun, RuntimeUnavailable>
} = Function.dual(2, (state: RuntimeState, runId: string): Effect.Effect<StoredRun, RuntimeUnavailable> => {
